...
class TerminalCoreRendererBoundaryTest(unittest.TestCase):
    ...
    def test_explicit_staging_and_submission_path_cannot_be_bypassed(self) -> None:
        for path, marker in (
            ...
            (guard.GPU, 'self.submit_terminal_upload_chunk(device, queue, &mut |submission| { target.submitted(submission) })?'),
            (guard.GPU, 'target.submitted(submission)'),
            (guard.GPU, 'target.acquire()?'),
            ...
            (guard.UPLOAD, 'self.textures[index].consume_chunk(count)'),
    ...
    def test_acquisition_before_upload_continuation_fails(self) -> None:
        self.assertTrue(self.mutate(guard.GPU, "self.submit_terminal_upload_chunk(",
                                    "target.acquire()?; self.submit_terminal_upload_chunk("))
