...
`pin_pad_maps[*].part` plus canonical `pin_pad_maps.mappings` pad keys with
`{gate, pin}` values against the referenced part. Historical pin-keyed rows
with pad values are migrated only when the pin resolves to exactly one entity
gate; multi-gate ambiguity is reported instead of inferred. Footprint geometry
