...
- `eda project inspect <dir>` validates that native scaffold and reports the resolved schema, UUIDs, paths, current object counts, and each declared `project.json` pool reference with its priority, resolved path, and current existence state.
- `eda project query <dir> summary` reports the current native schematic/board/rule summary from the scaffold, including the declared pool refs with priority, resolved path, and existence state.
