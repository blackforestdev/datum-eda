...
- `eda project inspect <dir>` validates that native scaffold and reports the resolved schema, UUIDs, paths, current object counts, each declared `project.json` pool reference with its priority, resolved path, and current existence state, plus aggregate primitive counts and component-presence counts for persisted component silkscreen and mechanical subsets.
- `eda project query <dir> summary` reports the current native schematic/board/rule summary from the scaffold, including the declared pool refs with priority, resolved path, and existence state plus aggregate primitive counts and component-presence counts for the currently materialized board-level component silkscreen subset and persisted component-mechanical subset.
...
- `eda project query <dir> board-components` reports the current native placed-package inventory from `board/board.json`, including per-component presence flags plus the currently materialized silkscreen and mechanical subset counts.
...
- `eda project export-gerber-silkscreen-layer <dir> --layer <id> --out <path>` writes a narrow RS-274X Gerber file for authored native board text plus persisted explicit component-silkscreen text, line, arc, circle, closed-polygon, and open-polyline geometry on one selected silkscreen layer; board text and component text are currently rendered through the fixed built-in stroke-font contract using stored `height_nm` and `stroke_width_nm`, component silkscreen lines remain explicit stored line strokes, component silkscreen arcs are currently emitted through deterministic 15-degree chordization, component silkscreen circles are currently emitted through deterministic 24-segment chordization, component silkscreen polygons are currently emitted as deterministic stroked loops, and component silkscreen polylines are currently emitted as deterministic stroked chains. During `place-board-component` and `set-board-component-package`, the current native mutation flow now resolves packages from `project.json` pool roots and persists only the supported package silkscreen non-text subset into those explicit component-silkscreen maps; `component_silkscreen_texts` remains schema-only and empty by design in this slice because the source package model does not yet carry truthful native text sizing for this contract.
...
- `eda project export-gerber-mechanical-layer <dir> --layer <id> --out <path>` writes a narrow RS-274X Gerber file for persisted native board keepout polygons, board-dimension span lines, authored board text, plus explicit component-mechanical text, line, closed-polygon, open-polyline, circle, and arc geometry on one selected mechanical layer; closed keepouts and component polygons are currently emitted as filled regions, component lines/polylines/circles as circular-aperture strokes, component arcs through deterministic 15-degree chordization, board-dimension spans as fixed-width baseline strokes only, and authored board text plus explicit component-mechanical text through the same fixed stroke-font contract used by the current silkscreen subset.
- `eda project inspect-gerber <path>` inspects a narrow RS-274X Gerber file and reports deterministic stroke, flash, and region counts plus normalized geometry entries for the currently supported subset.
...
- `eda project validate-gerber-silkscreen-layer <dir> --layer <id> --gerber <path>` validates that a narrow RS-274X silkscreen-layer Gerber still matches the current native board text plus explicit component-silkscreen text, line, arc, circle, closed-polygon, and open-polyline geometry on that layer exactly.
...
- `eda project validate-gerber-mechanical-layer <dir> --layer <id> --gerber <path>` validates that a narrow RS-274X mechanical-layer Gerber still matches the current native keepout polygons, board-dimension span lines, authored board text, plus explicit component-mechanical text, line, closed-polygon, open-polyline, circle, and arc geometry on that layer exactly.
...
- `eda project compare-gerber-silkscreen-layer <dir> --layer <id> --gerber <path>` compares that narrow RS-274X silkscreen-layer Gerber semantically against the current native board text plus explicit component-silkscreen text, line, arc, circle, closed-polygon, and open-polyline geometry on that layer.
...
- `eda project compare-gerber-mechanical-layer <dir> --layer <id> --gerber <path>` compares that narrow RS-274X mechanical-layer Gerber semantically against the current native keepout polygons, fixed-width board-dimension span lines, authored board-text strokes, explicit component-mechanical text strokes, plus explicit component-mechanical line, closed-polygon, open-polyline, circle, and arc geometry on that layer.
...
- `eda project place-board-dimension <dir> --from-x-nm <i64> --from-y-nm <i64> --to-x-nm <i64> --to-y-nm <i64> --layer <i32> [--text <text>]` places a native board dimension on an explicit board layer.
- `eda project edit-board-dimension <dir> --dimension <uuid> [--from-x-nm <i64> --from-y-nm <i64>] [--to-x-nm <i64> --to-y-nm <i64>] [--layer <i32>] [--text <text>] [--clear-text]` edits an existing native board dimension, including its stored layer.
