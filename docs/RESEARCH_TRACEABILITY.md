...
| Datum's scalable Project, continuous domain Model Spaces, universal Paper Space, and documentation object model must be derived from recorded owner intent plus internal and external research rather than inherited page-based EDA assumptions. | `research/workspace-architecture/WORKSPACE_ARCHITECTURE_RESEARCH.md` → DOC-SYSTEM-SPEC → `docs/decisions/PRODUCT_MECHANICS_020_PAPER_SPACE_AND_VIEWPORTS.md` | Owner-question ledger in progress under DOC-C01; no implementation mechanism authorized | The original 20 questions are answered and recorded one at a time before research synthesis, Claude visual studies, owner disposition, and final ratification. Distributed collaboration is separately forced to re-enter through `dat-distributed-collaboration-architecture-lt1`. |
