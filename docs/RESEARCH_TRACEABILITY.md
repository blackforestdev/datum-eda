...
| Datum's scalable Project, continuous Design Space, universal Publish Space, and documentation object model must be derived from recorded owner intent plus internal and external research rather than inherited page-based EDA assumptions. | `research/workspace-architecture/WORKSPACE_ARCHITECTURE_RESEARCH.md` → DOC-SYSTEM-SPEC → `docs/decisions/PRODUCT_MECHANICS_020_PAPER_SPACE_AND_VIEWPORTS.md` | DOC-C01 current-state inventory complete; DOC-C02 vocabulary and authority mapping is next; no implementation mechanism authorized | The 20-question owner ledger and code/decision/research/prototype inventory now expose the legacy schematic `Sheet*`, Model/Paper, pane/Viewport, and revision collisions before architecture is specified. Claude visual studies, owner disposition, and final ratification remain gated later steps. Distributed collaboration is separately forced to re-enter through `dat-distributed-collaboration-architecture-lt1`. |
