...
| Global Preferences must persist typed machine/user contributions and managed generations without flattening Project authority, losing unknown data, trusting arrival order, or requiring a network. | `research/preferences-system/GP_C01_INTERNAL_AUTHORITY_AUDIT.md` + `GP_C02_EXTERNAL_AND_STANDARDS_RESEARCH.md` + `GP_C02B_DOMAIN_PEER_PREFERENCES_RESEARCH.md` + `GP_C03_TYPED_AUTHORITY_DECISION_PACKET.md` + `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md` + `docs/gui/prototypes/preference-store-states-study.html` → `GP_C04_STORAGE_MIGRATION_EXCHANGE_RECOVERY_CONTRACT.md` / `dat-global-preferences-engine-qcv` | GP-C03 and GP-C04 are complete. GP-C04 reconciles Claude commit `586eb0d` into the persistence, recovery, migration, exchange, conflict, audit, typed operation/refusal, Project-seed seam, and proof contract; GP-C05 is next. | One locally complete generation-addressed repository uses authority-separated partitions, expected-generation transactions, immutable staged generations, explicit preserved-corrupt recovery, opaque unknown envelopes, atomic migrations that never invent a replacement value, quarantined typed exchange, previewed reversible restore, and explicit three-way conflicts. Capability authority refuses portable sources; every store operation leaves Project policy, design data, and authoring untouched. No implementation, dependency, or sync/account service is authorized. |
