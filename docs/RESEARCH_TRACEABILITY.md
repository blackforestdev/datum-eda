...
| Datum's scalable Project, continuous Design Space, universal Publish Space, and documentation object model must be derived from recorded owner intent plus internal and external research rather than inherited page-based EDA assumptions. | `research/workspace-architecture/WORKSPACE_ARCHITECTURE_RESEARCH.md` → `docs/gui/prototypes/publish-space-study.html` → DOC-SYSTEM-SPEC → `docs/decisions/PRODUCT_MECHANICS_020_PAPER_SPACE_AND_VIEWPORTS.md` | DOC-C01 inventory, DOC-C02 authority map, DOC-C03 object/mutation contract, and DOC-C04 visual study complete; DOC-C05 owner disposition is next; no implementation authorized | The contract now specifies stable Publish objects, typed operations, projection/write fences, Revision Engine configuration/baseline integration, failures, the narrow schematic proof, and Frames A-H with independently reviewable OR-1..OR-8 candidates. Owner disposition and final DOC-C06 ratification remain gated. Distributed collaboration is separately forced to re-enter through `dat-distributed-collaboration-architecture-lt1`. |
| Engineering revision must govern complete product configurations and controlled documents without equating Git commits, technical model revisions, or title-block counters with approved releases. | `research/documentation-system/PRODUCT_REVISION_ENGINE_RESEARCH.md` → `dat-product-revision-engine-k9f` → future governed decision/specification | Standards and internal-authority research opened; Questions 15/16 of DOC-SYSTEM-SPEC are suspended pending the release/baseline contract | The research spans Design, libraries, rules, Publish, manufacturing artifacts, approvals, effectivity, Git/offline collaboration, status accounting, audits, and quiet-by-default UX. No revision engine implementation is authorized. |
