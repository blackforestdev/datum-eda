...
| Engineering revision must govern complete product configurations and controlled documents without equating Git commits, technical model revisions, or title-block counters with approved releases. | `research/documentation-system/PRODUCT_REVISION_ENGINE_RESEARCH.md` + `REV_C01_INTERNAL_AUTHORITY_AUDIT.md` + `PRODUCT_REVISION_ENGINE_STANDARDS_MATRIX.md` + `PRODUCT_REVISION_ENGINE_AUTHORITY_MODEL.md` + `REVISION_SEQUENCING_AND_ORIGIN_RESEARCH.md` + `REV_C03_REVISION_IDENTITY_DECISION_PACKET.md` + `REV_C04_OFFLINE_GIT_INTEGRATION_CONTRACT.md` + `REV_C05_IMPACT_STALENESS_REPRODUCIBILITY_CONTRACT.md` + `REV_C06_UX_VISUAL_STUDY_BRIEF.md` + `REV_C07_PRODUCT_REVISION_ENGINE_RATIFICATION_PACKET.md` + `docs/gui/prototypes/revision-identity-study.html` + `docs/gui/prototypes/revision-authority-study.html` + `revision-ux-shell-study.html` + `revision-ux-impact-study.html` + `revision-ux-release-study.html` + `revision-ux-evidence-study.html` + `revision-ux-study-index.html` → `docs/decisions/PRODUCT_MECHANICS_034_PRODUCT_REVISION_ENGINE.md` + `specs/PRODUCT_REVISION_ENGINE_SPEC.md` / `dat-product-revision-engine-k9f` | REV-C01–C07 are complete. Product Mechanics 034 and the governed Product Revision Engine specification carry the owner-ratified mechanism. REV-C08 must now place bounded implementation, migration, conformance, audit-witness, reproduction, and production-acceptance slices without implicitly authorizing execution. No dependency, certification claim, implementation, or Publish Space development is authorized. | Datum uses one profile-driven Revision Engine. Lightweight post-release divergence automatically opens identity-free Draft successor Change work; explicit preparation later enters V2-P. Sequential Alphanumeric is the factory preference; ISO 19650 keeps revision/status separate in V3d title-block cells. Revision-bearing CIs exclusively own EngineeringRevisions; baselines own composition and one Release may issue several affected CIs' independent revisions. Mutable Publish Sets remain separate from CI-designated ControlledDocuments and immutable DocumentIssues. REV-C04 requires complete local authority and keeps Git optional/non-authoritative. REV-C05 separates changed, affected, stale, orphaned, standing, and non-reproducible facts and requires byte-identical release reproduction. REV-C06 Q1–Q6 govern the visible interaction. Under any profile without explicitly adopted earlier control, Design authoring remains non-blocking; ignoring the revision engine is a supported first-class workflow. Presentation-only hiding remains deferred to Global Preferences without weakening Project policy or engine truth. |
| Global Preferences must persist typed machine/user contributions and managed generations without flattening Project authority, losing unknown data, trusting arrival order, or requiring a network. | `research/preferences-system/GP_C01_INTERNAL_AUTHORITY_AUDIT.md` + `GP_C02_EXTERNAL_AND_STANDARDS_RESEARCH.md` + `GP_C02B_DOMAIN_PEER_PREFERENCES_RESEARCH.md` + `GP_C03_TYPED_AUTHORITY_DECISION_PACKET.md` + `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md` + `docs/gui/prototypes/preferences-window.html` + `preferences-accessibility-study.html` + `preference-store-states-study.html` → `GP_C04_STORAGE_MIGRATION_EXCHANGE_RECOVERY_CONTRACT.md` + `GP_C05_INTERACTION_AND_VISUAL_CONTRACT.md` / `dat-global-preferences-engine-qcv` | GP-C03, GP-C04, and GP-C05 are complete. GP-C05 records the owner's refined Option A at `a061fca`, accessibility/context states at `e2127fa`, and unchanged store states at `586eb0d`; GP-C08 is next and GP-C05A remains its historical alias. | One two-column Preferences window uses a section list plus pinned in-pane search as the primary discovery path. Every row retains label, description, real control, and provenance; search spans current/retired vocabulary and stable keys, then returns to the canonical row. One resolver explanation includes effective truth, every contribution and absence, Q4 reason, descriptor facts, and actions. Narrow, keyboard, reduced-motion, non-color, and announcement behavior is exact. The generation store and Project-policy seam remain unchanged. No implementation, dependency, or sync/account service is authorized. |
