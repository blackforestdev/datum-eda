...
| Global Preferences must persist typed machine/user contributions and managed generations without flattening Project authority, losing unknown data, trusting arrival order, or requiring a network. | `research/preferences-system/GP_C01_INTERNAL_AUTHORITY_AUDIT.md` + `GP_C02_EXTERNAL_AND_STANDARDS_RESEARCH.md` + `GP_C02B_DOMAIN_PEER_PREFERENCES_RESEARCH.md` + `GP_C03_TYPED_AUTHORITY_DECISION_PACKET.md` + `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md` → `GP_C04_STORAGE_MIGRATION_EXCHANGE_RECOVERY_CONTRACT.md` / `dat-global-preferences-engine-qcv` | GP-C03 authority is complete. GP-C04's non-visual repository, atomicity, recovery, migration, exchange, conflict, audit, and Project-seed seam is drafted; user-visible recovery, import/sync collision, backup/restore, and migration-result clauses await Claude-owned real-window renders before extraction. | One locally complete generation-addressed repository uses authority-separated partitions, expected-generation transactions, immutable staged generations, retained last-known-good recovery, opaque unknown envelopes, deterministic all-or-nothing migrations, quarantined typed exchange, and explicit three-way conflicts. Existing Projects retain their copied policy and receipt. No implementation, dependency, sync/account service, or owner disposition is authorized by the draft. |
