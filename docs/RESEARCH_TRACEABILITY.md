...
| Engineering revision must govern complete product configurations and controlled documents without equating Git commits, technical model revisions, or title-block counters with approved releases. | `research/documentation-system/PRODUCT_REVISION_ENGINE_RESEARCH.md` + `REV_C01_INTERNAL_AUTHORITY_AUDIT.md` + `PRODUCT_REVISION_ENGINE_STANDARDS_MATRIX.md` + `PRODUCT_REVISION_ENGINE_AUTHORITY_MODEL.md` + `REVISION_SEQUENCING_AND_ORIGIN_RESEARCH.md` + `REV_C03_REVISION_IDENTITY_DECISION_PACKET.md` + `docs/gui/prototypes/revision-identity-study.html` → `PRODUCT-REVISION-SPEC` / `dat-product-revision-engine-k9f` → future governed decision/specification | REV-C01 factual baseline and REV-C03 EngineeringChange/Q1 identity-profile decisions are owner-approved; REV-C02 supplies the standards evidence; the V1–V7 prototype is reviewed; ID-Q2 onward remain pending; no implementation is authorized | Datum uses one profile-driven Revision Engine. Sequential Alphanumeric (Legacy) is the factory system preference, ISO 19650 and later standards are selectable profiles, and a future Global Preferences engine (`dat-global-preferences-engine-qcv`) seeds new Project policy without becoming live authority over existing Projects. Per-CI namespaces, baseline-manifest composition, identity-free successor work, and REV-C02 source-strength gates remain controlling. |
