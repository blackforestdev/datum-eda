...
| Datum's embedded terminal must be a fully fledged daily-driver terminal—not a console lane or homegrown partial emulator—and must run arbitrary shells, TUIs, Codex, Claude Code, Cursor-compatible CLI agents, and local agents through normal PTY semantics plus governed Datum CLI/MCP context. | `docs/decisions/PRODUCT_MECHANICS_027_FULL_NATIVE_TERMINAL.md`; `docs/gui/DATUM_NATIVE_TERMINAL_SPEC.md`; Active Frontier T0–T4 | Ratified product contract; execution starts at T0 shell truth | Focus, PTY, parser, renderer, protocol, and agent-integration slices remain individually incomplete until the T4 production matrix passes. |
