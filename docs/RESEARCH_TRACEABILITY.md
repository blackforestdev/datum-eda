...
| Engineering revision must govern real adopted configurations without turning Revision into permanent chrome or fictional project data. | The full registered workspace/documentation/Revision/Preferences route → `docs/decisions/PRODUCT_MECHANICS_034_PRODUCT_REVISION_ENGINE.md` + `docs/decisions/PRODUCT_MECHANICS_038_REVISION_RECOVERY_AND_PRODUCT_BASELINE.md` + `specs/PRODUCT_REVISION_ENGINE_SPEC.md` / `dat-revision-product-recovery-dap` | REV-I10 was rejected on 2026-08-30. PM-038 is the controlling corrective baseline; the old production-acceptance epic and successors are suspended while bounded recovery proceeds. | Technical and human revision identity remain distinct, but an unmanaged Project has no default Revision UI or fictional records. Revision appears only through explicit intent or deliberately adopted real Project authority. Broad authority/public/enterprise implementation is quarantined, reusable integrity and preference primitives are retained, and no corrective runtime work begins before Claude-owned visual reconciliation plus a complete retain/rework/quarantine/remove inventory. |
