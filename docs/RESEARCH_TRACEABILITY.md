...
| Datum's embedded terminal must be a fully fledged daily-driver terminal—not a console lane or homegrown partial emulator—and must run arbitrary shells, TUIs, Codex, Claude Code, Cursor-compatible CLI agents, and local agents through normal PTY semantics plus native client discovery of governed Datum CLI/MCP, pinned context, scoped authority, and portable workflows. | `docs/decisions/PRODUCT_MECHANICS_027_FULL_NATIVE_TERMINAL.md`; `docs/decisions/PRODUCT_MECHANICS_028_TERMINAL_AGENT_INTEROPERABILITY.md`; `docs/gui/DATUM_NATIVE_TERMINAL_SPEC.md`; `docs/gui/DATUM_TERMINAL_AGENT_INTEROP_SPEC.md`; Active Frontier T0–T4e | Ratified product and interoperability contracts; T0-C01 landed and the decision-028 specification barrier precedes T0-C02 | Focus, PTY, renderer, protocols, launcher/discovery, standard MCP, context/authority, workflow parity, and final verification remain bounded tracked slices. |
