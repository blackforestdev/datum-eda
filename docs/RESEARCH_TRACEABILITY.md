...
| Engineering revision must govern complete product configurations and controlled documents without equating Git commits, technical model revisions, or title-block counters with approved releases. | `research/documentation-system/PRODUCT_REVISION_ENGINE_RESEARCH.md` + `REV_C01_INTERNAL_AUTHORITY_AUDIT.md` + `PRODUCT_REVISION_ENGINE_STANDARDS_MATRIX.md` + `PRODUCT_REVISION_ENGINE_AUTHORITY_MODEL.md` + `REVISION_SEQUENCING_AND_ORIGIN_RESEARCH.md` + `REV_C03_REVISION_IDENTITY_DECISION_PACKET.md` + `docs/gui/prototypes/revision-identity-study.html` → `PRODUCT-REVISION-SPEC` / `dat-product-revision-engine-k9f` → future governed decision/specification | REV-C01 factual baseline and REV-C03 EngineeringChange plus ID-Q1 through ID-Q5 decisions are owner-approved; REV-C02 supplies the standards evidence; the V1–V7 prototype is reviewed; ID-Q6 remains pending; no implementation is authorized | Datum uses one profile-driven Revision Engine. Sequential Alphanumeric (Legacy) is the factory system preference, uses an honestly marked provisional reservation, issues through explicit Issue/Release, and exposes no ISO suitability axis. ISO 19650 uses profile-defined transitions and independently authoritative revision and suitability/status fields. Optional BuildIdentity uses the same baseline-manifest model under quiet V4a or enterprise V4b presentation selected for new Projects through future Global Preferences/profile policy. Per-CI namespaces, identity-free successor collection, and REV-C02 source-strength gates remain controlling. |
