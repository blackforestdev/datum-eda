...
| Datum's scalable Project, continuous Design Space, universal Publish Space, and documentation object model must be derived from recorded owner intent plus internal and external research rather than inherited page-based EDA assumptions. | `research/workspace-architecture/WORKSPACE_ARCHITECTURE_RESEARCH.md` → DOC-SYSTEM-SPEC → `docs/decisions/PRODUCT_MECHANICS_020_PAPER_SPACE_AND_VIEWPORTS.md` | DOC-C01 inventory, DOC-C02 authority map, and DOC-C03 object/mutation contract complete; DOC-C04 visual studies are next; no implementation authorized | The contract now specifies stable Publish objects, typed operations, projection/write fences, Revision Engine configuration/baseline integration, failures, and the narrow schematic proof. Claude-owned visual studies, owner disposition, and final ratification remain gated. Distributed collaboration is separately forced to re-enter through `dat-distributed-collaboration-architecture-lt1`. |
