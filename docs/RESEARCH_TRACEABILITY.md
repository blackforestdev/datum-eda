...
| Engineering revision must govern real adopted configurations without turning Revision into permanent chrome or fictional project data. | The full registered workspace/documentation/Revision/Preferences route → `docs/decisions/PRODUCT_MECHANICS_034_PRODUCT_REVISION_ENGINE.md` + `docs/decisions/PRODUCT_MECHANICS_038_REVISION_RECOVERY_AND_PRODUCT_BASELINE.md` + `docs/decisions/PRODUCT_MECHANICS_039_PREFERENCES_FIRST_SETTINGS_ENTRY.md` + `docs/decisions/PRODUCT_MECHANICS_040_PROJECT_WORKING_UNITS_AUTHORITY.md` + `specs/PRODUCT_REVISION_ENGINE_SPEC.md` / `dat-revision-product-recovery-dap` | REV-I10 was rejected on 2026-08-30. PM-038 restores the unmanaged baseline; PM-039 makes the conventional Global/Project Preferences doorway and Preferences-first delivery sequence controlling before Revision UI can resume; PM-040 separates Global future-Project unit defaults, Project Working Units, and Publish/document units. | Technical and human revision identity remain distinct, but an unmanaged Project has no default Revision UI or fictional records. Exact Units core precedes an early real Global Preferences slice; Global Preferences is stabilized before Project Preferences is specified and built; only then may Project Revision configuration be considered. Configuration never enters the Navigator, and broad Revision authority/public/enterprise implementation remains quarantined. |
