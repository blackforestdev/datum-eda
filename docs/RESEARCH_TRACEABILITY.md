...
| Engineering revision must govern complete product configurations and controlled documents without equating Git commits, technical model revisions, or title-block counters with approved releases. | `research/documentation-system/PRODUCT_REVISION_ENGINE_RESEARCH.md` → `PRODUCT-REVISION-SPEC` / `dat-product-revision-engine-k9f` → future governed decision/specification | Selected canonical planning program with REV-C01..C08; standalone Datum authority and applicable-industry-standard auditability are owner requirements; no implementation authorized | The program spans Design, libraries, rules, Publish, manufacturing artifacts, approvals, effectivity, Git/offline collaboration, status accounting, reproducible audits, and quiet-by-default UX. REV-C01 begins with the internal authority inventory. |
