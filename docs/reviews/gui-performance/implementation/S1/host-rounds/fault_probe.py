...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';base=Path('/tmp/pm045-readiness-x11');out=Path('/tmp/pm045-round-device-proof');out.mkdir(exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences')]:
 ...
 env.pop('WAYLAND_DISPLAY',None);env.update(DATUM_GPU_MEASUREMENTS='0',DATUM_DIAGNOSTIC_DEVICE_LOSS='once',DATUM_GUI_VERBOSE_LOG='1',DATUM_GUI_LOG=str(log),WINIT_UNIX_BACKEND='x11',XDG_CONFIG_HOME=str(case/'config'),XDG_CACHE_HOME=str(case/'cache'),TMPDIR=str(case))
