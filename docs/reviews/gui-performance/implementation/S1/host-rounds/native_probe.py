...
report={'scope':'S1 native round dispatch candidate: six native extent/final-state checks for four hosts. Native round traces verify production scheduling entry; not mixed-terminal fairness or display/performance acceptance.','revision':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'binary_sha256':sha(binary),'fixture_sha256':sha(fixture),'runs':[]}
