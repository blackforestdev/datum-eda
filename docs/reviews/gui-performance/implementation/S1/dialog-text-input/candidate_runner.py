...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-native-dialog-input-spaces');out.mkdir(exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences'),('NEW','--open-new-project')]:
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   query={'GLOBAL':'board precision','PROJECT':'board precision','NEW':'PM045 Input'}[host]
