...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-minimized-final');out.mkdir(exist_ok=True)
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   for cycle in range(2):
    ...
    if flag:subprocess.run(['xdotool','windowminimize',wid],check=True)
    time.sleep(.15)
    result['minimized_window_state']=subprocess.check_output(['xprop','-id',wid,'_NET_WM_STATE','WM_STATE'],text=True);print(result['minimized_window_state'],flush=True);assert '_NET_WM_STATE_HIDDEN' in result['minimized_window_state']
    ...
    if flag:subprocess.run(['xdotool','windowactivate','--sync',wid],check=True)
