...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-hidden-state');out.mkdir(exist_ok=True)
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   for cycle in range(2):
    time.sleep(.55)
    offset=len(complete_lines(log));subprocess.run(['xdotool','windowminimize',wid],check=True)
    time.sleep(.15)
    ...
    hidden=until(lambda:next((s for s in complete_lines(log)[offset:] if 'window event '+key+' occluded true' in s),None),p)
    ...
    offset=len(complete_lines(log));subprocess.run(['xdotool','windowactivate','--sync',wid],check=True)
    restored=until(lambda:next((s for s in complete_lines(log)[offset:] if 'window event '+key+' occluded false' in s),None),p)
