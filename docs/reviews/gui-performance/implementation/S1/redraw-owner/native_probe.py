...
report={'scope':'S1 redraw candidate: matched baseline fixture paths and six-extent native adapter check; static candidate captures; no budget, latency or overhead claim.','revision':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'binary_sha256':sha(binary),'fixture_sha256':sha(fixture),'runs':[]}
for host,flag in [('MAIN',None),('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences'),('NEW','--open-new-project')]:
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   if ok:
    windows=subprocess.check_output(['xdotool','search','--onlyvisible','--pid',str(p.pid)],text=True).split()
    expected={'MAIN':'Datum EDA','GLOBAL':'Global Preferences','PROJECT':'Project Preferences','NEW':'New Project'}[host]
    selected=[w for w in windows if expected in subprocess.check_output(['xdotool','getwindowname',w],text=True)]
    assert selected,(host,windows)
    subprocess.run(['xdotool','windowactivate','--sync',selected[0]],check=True)
    assert int(subprocess.check_output(['xdotool','getactivewindow','getwindowpid'],text=True))==p.pid
    time.sleep(.3)
    capture_env=os.environ.copy();capture_env.pop('QT_QPA_PLATFORM',None)
    subprocess.run(['spectacle','--background','--nonotify','--activewindow','--no-decoration','--no-shadow','--output',str(case/'candidate.png')],env=capture_env,check=True,timeout=15)
