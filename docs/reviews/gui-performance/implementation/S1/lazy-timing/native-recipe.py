import os,subprocess,time,json,re,hashlib,sys,ctypes as C
mode=sys.argv[1];assert mode in ("disabled","enabled")
from pathlib import Path
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-lazy-timing-'+mode);out.mkdir(exist_ok=True)
...
for host,flag in [('MAIN',None)]:
 ...
 env.pop('DATUM_TRACE_TIMING',None)
 if mode=='enabled':env['DATUM_TRACE_TIMING']='1'
...
stderr=(out/'MAIN/stderr.log').read_text()
if mode=='enabled':
 assert '[datum-timing] runtime render total=' in stderr
 assert '[datum-timing] redraw render ' in stderr
else:assert '[datum-timing]' not in stderr
print(mode,'timing output assertion passed',flush=True)
