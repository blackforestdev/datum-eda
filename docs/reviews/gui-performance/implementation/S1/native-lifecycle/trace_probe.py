...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-native-lifecycle-trace2');out.mkdir(exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences')]:
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   subprocess.run(['xdotool','windowminimize',wid],check=True);time.sleep(.3);hidden_start=len(records(log));time.sleep(.4);hidden=records(log)[hidden_start:];assert not any(r['window']==key and r['reason'] in ['submit','present'] for r in hidden)
   ...
   report['runs'].append({'host':host,'passed':True,'command':cmd,'window':key,'initial':initial,'maximized':maxed,'restored':restored,'resumed':resumed,'hidden_observation_ms':400,'hidden_submit_present':0,'maximized_state':state});(out/'result.json').write_text(json.dumps(report,indent=2)+'\n');print(host,'maximize/restore/minimize/resume pass',flush=True)
