...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';base=Path('/tmp/pm045-readiness-x11');out=Path('/tmp/pm045-counts-final-device-proof');out.mkdir(exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences')]:
 ...
 cmd=[str(binary),'--board',str(case/'board.kicad_pcb'),'--window-size','1280x800']+([flag] if flag else [])
 with (out/(host+'-stderr.log')).open('w') as stream:
  ...
  try:
   ...
   while time.monotonic()<deadline:
    data=log.read_text()
    if ('native device replacement committed' in data and '"reason":"present"' in data.split('native device replacement committed',1)[1]) or p.poll() is not None:break
    time.sleep(.1)
   ok=p.poll() is None and ('native device replacement committed' in data and '"reason":"present"' in data.split('native device replacement committed',1)[1]) and data.count('native device replacement begin')==1 and data.count('native device replacement committed')==1 and data.count('native device state preserved')==1 and any('native device replacement begin fault_code='+str(code) in data for code in [1,2,3,4])
