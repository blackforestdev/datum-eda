...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-pending-queue-close');out.mkdir(exist_ok=False)
def lines(log):
...
for host,flag,title in [('MAIN',None,'Datum EDA'),('GLOBAL','--open-global-preferences','Global Preferences — Datum'),('PROJECT','--open-project-preferences','Project Preferences — Datum'),('NEW','--open-new-project','New Project — Datum')]:
 ...
 env.update(WINIT_UNIX_BACKEND='x11',EDA_CLI_BIN=str(root/'target/release/datum-eda'),CARGO_NET_OFFLINE='true',DATUM_DIAGNOSTIC_QUEUE_COMPLETION='hold-until-retry',DATUM_GUI_VERBOSE_LOG='1',DATUM_GUI_LOG=str(log),DATUM_GPU_MEASUREMENTS='0',XDG_CONFIG_HOME=str(case/'config'),XDG_CACHE_HOME=str(case/'cache'))
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ids=until(lambda:ids if len(ids:=re.findall(r'surface identity window=WindowId\((\d+)\)',log.read_text()))==(2 if flag else 1) else None,p);key='WindowId('+ids[-1]+')'
   until(lambda:'Rendering paused for shared native queue:' in log.read_text(),p)
   before=records(log);assert any(v['frame_submissions']>0 for v in before)
   assert all(v['gpu_completed_through_receipt']==0 for v in before)
   r['target_submissions_before_close']=max((v['frame_submissions'] for v in before if v['window']==key),default=0)
   r['unpublished_receipt_before_close']=max(v['last_frame_submission_receipt'] for v in before)
   ...
   dropped=set()
   for v in records(log):
    assert v['window'] not in dropped,'lifecycle work after owner drop'
    if v['reason']=='owner_drop':dropped.add(v['window'])
   assert dropped=={'WindowId('+i+')' for i in ids}
   assert 'native Main window ownership released=true' in log.read_text()
   assert 'held completion released by Retry' not in log.read_text()
   r.update(exit=0,all_hosts_dropped=True,no_lifecycle_after_drop=True,main_window_released=True)
   print(host,r,flush=True)
