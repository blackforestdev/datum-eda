...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-native-wheel-settled');out.mkdir(exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   def settled():
    lines=complete_lines(log);key='WindowId('+wid+')';starts=[i for i,s in enumerate(lines) if 'native frame round ' in s and key in s];ends=[(i,json.loads(s.split('native redraw ledger ',1)[1])) for i,s in enumerate(lines) if 'native redraw ledger ' in s and json.loads(s.split('native redraw ledger ',1)[1])['window']==key]
    if not starts or not ends:return False
    i,r=ends[-1];return i>starts[-1] and r['damage']==r['presented_damage'] and not r['pending'] and not r['ready'] and r['rendering'] is None
   until(settled,p);time.sleep(.2)
   ...
   def step(name,button,repeat,point,changed):
    ...
    before=counts();offset=len(complete_lines(log));subprocess.run(['xdotool','click','--repeat',str(repeat),'--delay','10',str(button)],check=True);until(lambda:sum('window event WindowId('+wid+') mouse wheel' in s for s in complete_lines(log)[offset:])==2*repeat,p);time.sleep(.15);until(settled,p);time.sleep(.2);after=counts()
   ...
   subprocess.run(['xdotool','key','Alt+F4'],check=True);until(lambda:subprocess.check_output(['xdotool','getwindowfocus'],text=True).strip()==main,p);subprocess.run(['xdotool','key','Alt+F4'],check=True);p.wait(timeout=20);assert p.returncode==0
