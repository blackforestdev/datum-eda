...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-native-wheel');out.mkdir(exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   if host=='GLOBAL':
    subprocess.run(['xdotool','mousemove','--window',wid,'400','70','click','1','type','--delay','20','unit'],check=True)
   time.sleep(1)
   ...
   step('outside viewport',5,3,(30,260),False)
