...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-wayland-owner-close-candidate');out.mkdir(exist_ok=False)
def lines(log):
...
for host,flag,title in [('MAIN',None,'Datum EDA'),('GLOBAL','--open-global-preferences','Global Preferences — Datum'),('PROJECT','--open-project-preferences','Project Preferences — Datum'),('NEW','--open-new-project','New Project — Datum')]:
 ...
 env.update(WINIT_UNIX_BACKEND='wayland',EDA_CLI_BIN=str(root/'target/release/datum-eda'),CARGO_NET_OFFLINE='true',DATUM_GUI_VERBOSE_LOG='1',DATUM_GUI_LOG=str(log),DATUM_GPU_MEASUREMENTS='0',XDG_CONFIG_HOME=str(case/'config'),XDG_CACHE_HOME=str(case/'cache'))
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   after=[x for x in records(log) if x['reason']=='present'];assert len(before)==len(after),'extra idle presentation'
