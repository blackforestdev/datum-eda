...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-wayland-hidden-queue');out.mkdir(exist_ok=False)
def lines(log):
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   for entry in entries:
    ...
    if state.get('pid',{}).get('data')==p.pid:found.append((uuid,state))
   assert len(found)==1, str(found)
   uuid,initial=found[0];r['kwin_initial']=initial
