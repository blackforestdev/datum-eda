...
report={'scope':'S1 owned native frame candidate: six extent/final-state transitions on four native hosts through shared presentation. Not visual, resource-budget or display acceptance.','revision':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'binary_sha256':sha(binary),'fixture_sha256':sha(fixture),'runs':[]}
