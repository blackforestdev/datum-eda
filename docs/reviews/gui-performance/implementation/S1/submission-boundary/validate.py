...
report=[]
paths=[Path('/tmp/pm045-submit-host-proof')/h/'native.log' for h in ['GLOBAL','PROJECT','NEW']]
paths += [Path('/tmp/pm045-submit-device-proof')/(h+'.log') for h in ['GLOBAL','oom']]
for path in paths:
 ...
 for r in rows:
  ...
  assert r['last_present_receipt']<=r['frame_submission_receipts_issued'],r
  assert r['gpu_completed_through_receipt']<=r['frame_submission_receipts_issued'],r
  assert r['presented'] <= r['frame_submissions'] <= r['acquired'],r
  assert r['last_present_receipt'] <= r['last_frame_submission_receipt'] <= r['frame_submission_receipts_issued'],r
  if r['reason']=='submit':assert r['active']==1 and r['wgpu_submission'].startswith('Some(SubmissionIndex'),r
  if r['reason']=='present':assert r['active']==0 and r['presented']>0 and r['configuration_generation']>0,r
  if prev:
   assert prev['window']==r['window'],r
   for field in ['acquired','presented','discarded','acquire_attempts','configure_attempts','configuration_generation','last_present_receipt','frame_submission_receipts_issued','gpu_completed_through_receipt']:
...
Path('/tmp/pm045-submit-validation.json').write_text(json.dumps(report,indent=2)+'\n')
