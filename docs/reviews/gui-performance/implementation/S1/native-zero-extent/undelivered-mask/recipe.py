...
root=Path('/home/bfadmin/Documents/datum-eda');binary=root/'target/release/datum-gui';out=Path('/tmp/pm045-native-zero-extent');out.mkdir(exist_ok=True)
...
def notify_size(wid,geometry,width,height):
 ...
 assert x.XSendEvent(d,int(wid),0,1<<17,C.byref(e));x.XFlush(d);x.XCloseDisplay(d)
def expose(wid,width,height):
 ...
 assert x.XSendEvent(d,int(wid),0,1<<15,C.byref(e));x.XFlush(d);x.XCloseDisplay(d)
