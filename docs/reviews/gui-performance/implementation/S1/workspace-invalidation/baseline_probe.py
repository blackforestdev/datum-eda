...
root=Path('/home/bfadmin/Documents/datum-eda');binary=Path(sys.argv[1]).resolve();tag=sys.argv[2];out=Path('/tmp/pm045-workspace-proof')/tag;out.mkdir(parents=True,exist_ok=True)
...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences'),('NEW','--open-new-project')]:
 ...
 cmd=[str(binary),'--board',str(board),flag,'--window-size','1280x800','--terminal-session-profile','custom','--terminal-program','/usr/bin/python3','--terminal-arg=-u','--terminal-arg=/tmp/pm045_workspace_producer.py','--terminal-arg='+str(case)]
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   while not (case/'sent.json').exists() and p.poll() is None and time.monotonic()<deadline:time.sleep(.05)
   ...
   expected_aux=(aux>0) if tag=='baseline' or host=='NEW' else aux==0
   ok=main>0 and expected_aux
   report['runs'].append({'host':host,'command':cmd,'passed':ok,'main_rounds':main,'auxiliary_rounds':aux,'producer':json.loads((case/'sent.json').read_text()),'native_ids':ids,'log_sha256':sha(log),'output_window_sha256':sha(case/'output-window.log')})
