...
root=Path('/home/bfadmin/Documents/datum-eda'); copy=Path('/tmp/pm045-extent-negative'); out=Path('/tmp/pm045-extent-proof'); out.mkdir()
p=copy/'crates/gui-render/src/render/preferences_rows.rs'; original=p.read_bytes()
old='scroll.layout(viewport, total);'; new='scroll.layout(viewport, total.min(viewport.height));'
