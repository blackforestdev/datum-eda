...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (d/'stderr.log').open('w') as err:
  ...
  try:
   ...
   xdo('key','Escape')
   def dialog_hidden():
    q=subprocess.run(['xdotool','search','--onlyvisible','--pid',str(p.pid)],env=env,text=True,capture_output=True)
    return wid not in q.stdout.splitlines()
   wait(dialog_hidden)
   delivered=[line for line in log.read_text()[len(before):].splitlines() if 'keyboard physical=' in line and 'Escape' in line and 'state=Pressed' in line and 'synthetic=false' in line]
   assert len(delivered)==1, delivered
   result['single_escape_closes_dialog_after_close_click']=True
   result['escape_delivery']=delivered
