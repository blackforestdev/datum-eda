...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (d/'stderr.log').open('w') as err:
  ...
  try:
   ...
   n=count();click(400,70);wait(lambda:count()>n);time.sleep(.2)
   for _ in range(3):
    n=count();xdo('key','shift+Tab');wait(lambda:count()>n);time.sleep(.2)
   capture('last-name');n=count();xdo('key','Return');wait(lambda:count()>n);time.sleep(.4);capture('explanation')
   if os.environ.get('PM045_CLICK_CLOSE')=='1':
    n=count();click(938,530);wait(lambda:count()>n);time.sleep(.4);capture('closed-explanation');result['close_click_frames']=count()-n
