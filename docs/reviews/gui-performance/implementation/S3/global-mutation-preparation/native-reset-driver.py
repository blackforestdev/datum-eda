...
for host,flag in [('GLOBAL','--open-global-preferences')]:
 ...
 with (d/'stderr.log').open('w') as err:
  ...
  try:
   ...
   time.sleep(.25);capture('before-choice');n=frames();click(940,130);wait(lambda:frames()>n);time.sleep(.3);capture('choices')
   ...
   n=frames();click(940,177);wait(lambda:frames()>n)
   def reset_committed():
    h=json.loads(head.read_text());g=h['generation']['generation']
    manifest=preference_root/'generations'/f'g{g:020}'/'manifest.json'
    return m if 'datum.units.system' not in (m:=json.loads(manifest.read_text()))['state']['user'] else None
   reset=wait(reset_committed);(d/'global-reset.json').write_text(json.dumps(reset,indent=2)+'\n')
   assert json.loads(project_file.read_text())==before
   r['reset_hit_proof']='Newly positioned Reset removes User contribution; stale Metric choice would retain a User value'
