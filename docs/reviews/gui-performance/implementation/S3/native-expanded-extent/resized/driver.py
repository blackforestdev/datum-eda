...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (case/'stderr.log').open('w') as stderr:
  ...
  try:
   ...
   before=count();xdo('windowsize',wid,str(round(960*scale)),str(round(400*scale)));wait(lambda:count()>before)
   result['resize_while_expanded']='960x720 to960x400 logical pixels'
   time.sleep(.4)
