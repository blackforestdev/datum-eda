...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (case/'stderr.log').open('w') as stderr:
  ...
  try:
   ...
   before=count();xdo('mousemove','--window',wid,str(round(300*scale)),str(round(125*scale)),'click','1');wait(lambda:count()>before)
   result['expanded_explanation_trigger']='Click Measurement system label before scrolling; verify captured explanation separately'
   time.sleep(.4)
   ...
   xdo('mousemove','--window',wid,str(round(width-6*scale)),str(round(200*scale)));time.sleep(.2)
   ...
   result['focus_before_escape']=subprocess.check_output(['xdotool','getwindowfocus'],env=env,text=True).strip()
   xdo('windowactivate','--sync',wid);xdo('windowfocus','--sync',wid);time.sleep(.2)
   result['focus_after_activation']=subprocess.check_output(['xdotool','getwindowfocus'],env=env,text=True).strip()
   before=count();events=event_count('keyboard physical=Code(Escape)');xdo('key','Escape');wait(lambda:event_count('keyboard physical=Code(Escape)')>events);wait(lambda:count()>before);time.sleep(.3)
   result['collapse_frames']=count()-before
   subprocess.run(['import','-window',wid,str(case/'collapsed.png')],env=env,check=True,timeout=8)
