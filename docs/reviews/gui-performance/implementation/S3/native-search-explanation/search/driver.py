...
for host,flag in [('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences')]:
 ...
 with (case/'stderr.log').open('w') as stderr:
  ...
  try:
   ...
   before=count();xdo('mousemove','--window',wid,'350','70','click','1');time.sleep(.2)
   xdo('windowfocus','--sync',wid)
   before=count();xdo('type','--delay','100','precision');wait(lambda:count()>before);time.sleep(.5)
   subprocess.run(['import','-window',wid,str(case/'search.png')],env=env,check=True,timeout=8)
   xdo('mousemove','--window',wid,'400','300');time.sleep(.2)
   before=count();events=event_count('mouse wheel');xdo('click','5');wait(lambda:event_count('mouse wheel')>events);time.sleep(.25)
   assert count()==before,('fitting search content scrolled',before,count())
   result['fitting_search_wheel_frames']=0
   xdo('windowfocus','--sync',wid)
   before=count();xdo('key','Escape');wait(lambda:count()>before);time.sleep(.3)
   subprocess.run(['import','-window',wid,str(case/'cleared.png')],env=env,check=True,timeout=8)
   before=count();xdo('click','5');wait(lambda:count()>before);time.sleep(.25)
   result['restored_content_wheel_frames']=count()-before
