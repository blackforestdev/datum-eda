...
out=Path(tempfile.mkdtemp(prefix='pm045-close-diagnostic-'));print(out,flush=True)
...
r={'mode':mode,'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'cycles':[],'scope':'Bounded diagnostic for dat-native-final-close-observation-xbb7. No performance or shutdown qualification. At most six nine-cycle runs; stop on first unobserved close and capture existing process stacks.'}
env=os.environ.copy()
for k in list(env):
...
with (out/'stderr.log').open('w') as f:
 ...
 try:
  if mode in ('overflow','existing'):
   ...
   (out/'window-properties.txt').write_text(subprocess.check_output(['xprop','-id',str(main),'WM_PROTOCOLS','_NET_WM_PID'],text=True))
   r['close_sent_ns']=time.monotonic_ns();close_window(main)
   try:conn,_=listener.accept()
   except TimeoutError:
    r['close_timeout']=True;save()
    threads=[]
    ...
    with (out/'gdb.txt').open('w') as diagnostic:
     try:r['gdb_returncode']=subprocess.run(['gdb','-q','-nx','-batch','-ex','set pagination off','-ex','set debuginfod enabled off','-ex','thread apply all bt','-ex','detach','-p',str(pid)],stdout=diagnostic,stderr=subprocess.STDOUT,timeout=25).returncode
     except subprocess.TimeoutExpired:r['gdb_timeout']=True
    # One declared unrelated pointer event after snapshot; this is diagnosis,
    # never a retry that turns the original close into a passing trial.
    xd('mousemove','--window',main,300,350)
    listener.settimeout(3)
    try:
     awakened,_=listener.accept();awakened.settimeout(2);data=awakened.recv(4096);r['after_wake_receipt']=data.decode();awakened.sendall(b'!');awakened.close();r['after_wake_exit']=p.wait(timeout=10)
