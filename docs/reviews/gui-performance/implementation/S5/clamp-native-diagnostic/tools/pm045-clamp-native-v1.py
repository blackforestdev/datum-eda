...
out=Path(tempfile.mkdtemp(prefix='pm045-clamp-native-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  report['geometry']=geometry;report['clamp_trials']=[]
  ...
  for label,button,reverse in [('minimum',5,4),('maximum',4,5)]:
   ...
   subprocess.run(['python3','/tmp/pm045-wheel-stream.py',str(button),'1800','60',str(out/(label+'-input.json'))],check=True)
   ...
   subprocess.run(['python3','/tmp/pm045-wheel-stream.py',str(reverse),'1','60',str(out/(label+'-reversal-input.json'))],check=True)
