...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-clamp-campaign-'));print(out,flush=True)
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'W-CLAMP remaining five originally declared trials; baseline1 retained from hs2ks5vn with post-exit harness KeyError. No replacement. Both bounds 100 untimed setup detents, 5s warmup, 1800 outward detents at 60Hz over 30s, 5s still tail, one separately timed inward reversal. Descriptive evidence, not full camera-state/method qualification.','order':[['candidate',1,0],['candidate',2,0],['baseline',2,0],['baseline',3,0],['candidate',3,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-wheel-stream.py'),'native_driver_sha256':sha('/tmp/pm045-clamp-native.py'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-clamp-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
