...
out=Path(tempfile.mkdtemp(prefix='pm045-gpu-history-native-'+mode+'-'));print(out,flush=True)
binary=Path(os.environ.get('PM045_GPU_HISTORY_BINARY',str(root/'target/release/datum-gui')));trace=out/'gpu.jsonl';log=out/'native.log';log.touch()
r={'mode':mode,'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'cycles':[],'scope':'Bounded native GPU allocation-history delivery conformance; no timing/RSS/full memory/endurance acceptance.'}
env=os.environ.copy()
for k in list(env):
 if k.startswith(('DATUM_DIAGNOSTIC_','DATUM_GPU_DIAGNOSTIC_','DATUM_PRIVATE_TEXT_','DATUM_GPU_ALLOCATION_')):env.pop(k)
...
if mode!='off':env['DATUM_GPU_ALLOCATION_TRACE']=str(trace)
if mode=='overflow':env['DATUM_GPU_ALLOCATION_TRACE_CAPACITY']='1'
