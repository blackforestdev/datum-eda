...
out=Path(tempfile.mkdtemp(prefix='pm045-x11-close-v4-'));print(out,flush=True)
...
env.update(LD_AUDIT='/tmp/pm045_x11_audit_v4.so',PM045_X11_AUDIT_PATH=str(out/'x11'))
...
def diagnose():
  ...
  for entry in sorted(Path(f'/proc/{pid}/fd').iterdir()):
   row={'fd':entry.name}
   try:row.update(target=os.readlink(entry),fdinfo=Path(f'/proc/{pid}/fdinfo/{entry.name}').read_text())
   except OSError as error:row['error']=repr(error)
   fd_state['descriptors'].append(row)
  socket_state=subprocess.run(['ss','-xnp'],capture_output=True,text=True);fd_state['ss_returncode']=socket_state.returncode;fd_state['ss_stderr']=socket_state.stderr;fd_state['socket_rows']=[line for line in socket_state.stdout.splitlines() if f'pid={pid},' in line];fd_state['finished_ns']=time.monotonic_ns()
