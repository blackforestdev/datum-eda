...
static void record(const char *kind, Display *display, XEvent *event, int result) {
 ...
 int n=snprintf(line,sizeof(line),"{\"seq\":%lu,\"ns\":%llu,\"pid\":%d,\"tid\":%ld,\"kind\":\"%s\",\"display\":\"%p\",\"connection_fd\":%d,\"result\":%d,\"event_type\":%d,\"window\":%lu,\"message_type\":%lu,\"data0\":%ld,\"data1\":%ld,\"serial\":%lu,\"send_event\":%d}\n",seq,(unsigned long long)ts.tv_sec*1000000000ull+ts.tv_nsec,getpid(),syscall(SYS_gettid),kind,(void*)display,display?ConnectionNumber(display):-1,result,event?event->type:0,event?event->xany.window:0,c?c->message_type:0,c?c->data.l[0]:0,c?c->data.l[1]:0,event?event->xany.serial:0,event?event->xany.send_event:0);
