...
root=Path.cwd();base=root/'docs/reviews/gui-performance/implementation/S5/pane-native-sequence';base.mkdir(exist_ok=True)
...
(base/'tools').mkdir()
for path in ('/tmp/pm045-panes-native-v1.py','/tmp/pm045-panes-native-v2.py','/tmp/pm045_wm_close.py','/tmp/pm045-panes-package-v1.py',__file__):shutil.copy2(path,base/'tools'/Path(path).name)
