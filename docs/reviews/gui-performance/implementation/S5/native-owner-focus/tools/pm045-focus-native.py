...
out=Path(tempfile.mkdtemp(prefix='pm045-focus-native-')); print(out,flush=True)
...
cmd.append({'GLOBAL':'--open-global-preferences','PROJECT':'--open-project-preferences','NEW':'--open-new-project'}[os.environ['PM045_CONTROLS_HOST']])
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  for host in (os.environ['PM045_CONTROLS_HOST'],):
   ...
   case['owner_hint_before']=subprocess.check_output(['xprop','-id',str(wid),'WM_TRANSIENT_FOR'],text=True)
   expected=hex(main)
   assert expected in case['owner_hint_before'],case['owner_hint_before']
   if os.environ['PM045_FOCUS_CASE']=='negative':
    subprocess.run(['xprop','-id',str(wid),'-remove','WM_TRANSIENT_FOR'],check=True)
   case['owner_hint_used']=subprocess.check_output(['xprop','-id',str(wid),'WM_TRANSIENT_FOR'],text=True)
   offset=log.stat().st_size;xd('windowactivate','--sync',helper)
   until(lambda:f'window event WindowId({wid}) focused false' in log.read_bytes()[offset:].decode())
   xd('windowactivate','--sync',wid);until(lambda:int(xd('getwindowfocus'))==wid);time.sleep(.3)
   png=out/(host+'-before-close.png')
   subprocess.run(['spectacle','-b','-n','-a','-e','-S','-o',str(png)],check=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=15)
   case['static_capture']=png.name
   ref=root/f'docs/reviews/gui-performance/implementation/S5/resolver-cache-ownership/native/{host}.png'
   cmp=subprocess.run(['compare','-metric','AE',str(ref),str(png),'null:'],capture_output=True,text=True)
   case['static_AE']=cmp.stderr.strip();save();assert cmp.returncode==0,cmp.stderr
   close_window(wid)
   ...
   case['closed']=True;case['expected_focus_restored']=os.environ['PM045_FOCUS_CASE']=='positive';save();assert case['automatic_focus_restored']==case['expected_focus_restored']
  report['passed_native_cycles']=True;report['limits']=['Bounded ownership correction and missing-parent negative only, not W-CONTROLS30-cycle or full S5 qualification. Native focus and static endpoints only; no latency/resource/performance acceptance.']
