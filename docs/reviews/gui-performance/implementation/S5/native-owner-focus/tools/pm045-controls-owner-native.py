...
out=Path(tempfile.mkdtemp(prefix='pm045-controls-owner-native-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  for host in (os.environ['PM045_CONTROLS_HOST'],):
   ...
   x.XSetTransientForHint.argtypes=[ctypes.c_void_p,ctypes.c_ulong,ctypes.c_ulong]
   assert x.XSetTransientForHint(display,wid,main)
   x.XFlush(display)
   report['diagnostic_transient_owner']={'owned':wid,'owner':main,'property':subprocess.check_output(['xprop','-id',str(wid),'WM_TRANSIENT_FOR'],text=True)};save()
   ...
   def capture(name):
    png=out/(host+'-'+name+'.png');window=out/(host+'-'+name+'-window.png')
    subprocess.run(['import','-window',str(wid),str(window)],check=True)
    ...
    case.setdefault('compositor_captures',[]).append({'name':name,'path':png.name,'window_readback':window.name,'stderr':completed.stderr});save()
    return png
