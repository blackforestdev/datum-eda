...
root=Path('/home/bfadmin/Documents/datum-eda');out=Path(tempfile.mkdtemp(prefix='pm045-panes-resource-'));print(out,flush=True)
...
report={'purpose':'W-PANES native tracked-allocation and sampled ownership observation; same structural oracles','candidate':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'binary_sha256':expected,'normalized_model_sha256':modelsha(),'source_sha256':source,'driver_sha256':sha(__file__),'new_evidence':'Tracked allocation lifetime and sampled owner counters through repeated complementary pane creation/close; original structural sequence retained without claiming new configuration coverage','schedule':'five second warmup, 60 actions at 500ms start intervals: split vertical, focus next, close focused, focus next; initial/final Board plus unadmitted Schematic placeholder leaves; five second tail','oracle':'Every accepted menu action has native Enter receipt plus observed menu closure, console echo and exact pane-state transition. Pane actions are not covered by the opt-in registry dispatch emitter. Split adds complementary content with inherited active Board camera and retains existing ids/cameras/focus; focus follows ordered leaves; close removes only focused id and retains survivors; after every four actions original two ids survive. No project edits.','limits':['Verbose, action-state, sampled ownership and GPU allocation diagnostics enabled; no CPU/GPU/latency cap or formal relative inference. Tracked allocation reservations are not driver residency, complete process memory or exact API-live peaks.','Schematic placeholder exercises pane chrome only; no resolved schematic qualification. One X11/1x run; other configurations, three-trial qualification and independent replay remain open.','Endpoint compositor images are visual evidence, not physical presentation calibration.'],'actions':[]}
...
env.update(DATUM_RESOURCE_TRACE=str(out/'resources.jsonl'),DATUM_RESOURCE_TRACE_INTERVAL_MS='20',DATUM_GPU_ALLOCATION_TRACE=str(out/'gpu.jsonl'))
...
def records():
 ...
 with err.open('rb') as f:
  f.seek(read_offset);data=f.read();end=data.rfind(b'\n')+1;read_offset+=end
 cache.extend(json.loads(line.split('DATUM_ACTION_EVIDENCE ',1)[1]) for line in data[:end].decode().splitlines() if 'DATUM_ACTION_EVIDENCE ' in line)
 return cache
...
def action(index,kind,due):
 ...
 before=state();previous=max(r['sequence'] for r in records());native_offset=log.stat().st_size;started=time.monotonic_ns()
 ...
 echo={'split':'split pane vertical','switch':'focus next pane','close':'close focused pane'}[kind]
 until(lambda:state()['menu'] is None and state()['console_latest']==echo and max(r['sequence'] for r in records())>previous,.4)
 with log.open('rb') as f:f.seek(native_offset);native=f.read().decode()
 assert 'physical=Code(Enter)' in native and 'state=Pressed' in native
 receipt={'action':key,'console_echo':echo,'native_log_begin':native_offset,'state_sequence':max(r['sequence'] for r in records())}
 ...
 if kind=='split':
  ...
  assert a[next(iter(set(a)-set(b)))]==before['board_camera']
 ...
 assert all(r['content'] in ('Board','Schematic') for r in after['pane_cameras'])
 if kind=='close':assert after['focused_pane']==report['actions'][-2]['before']['focused']
...
try:
 ...
 assert cameras(state())==cameras(initial) and state()['focused_pane']==list(cameras(initial))[1]
 close_window(wid)
