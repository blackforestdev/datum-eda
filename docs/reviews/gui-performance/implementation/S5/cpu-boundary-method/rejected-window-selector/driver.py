...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  main=int(until(lambda:xd('search','--onlyvisible','--pid',p.pid,'--name','Datum')) .splitlines()[0])
  ...
  for host in ('PROJECT',):
   for i in range(3):
    ...
    wid=int(until(lambda:next((w for w in xd('search','--onlyvisible','--pid',p.pid,'--name','Project Preferences').splitlines()),None)))
