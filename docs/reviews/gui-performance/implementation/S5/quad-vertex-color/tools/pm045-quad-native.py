...
out=Path(tempfile.mkdtemp(prefix='pm045-quad-native-')); print(out,flush=True)
...
report['mode']=mode;report['trial']=trial;report['verbose_log']=os.environ['PM045_LOG_VERBOSE'];report['diagnostic_only']=True
...
report['source_state']='Quad conversion baseline b8c1fa1b versus bounded candidate; source hashes describe candidate checkout, frozen shaders and baseline bytes separately pinned'
...
report['limits']=['Three fixed alternating matched baseline/candidate diagnostic pairs; quantized XTest120Hz schedule has1280distinctinteger changes, not3600distinctpositions and not fullWPOINTER qualification. Quiet eventcounts unobserved; GPUtimestamps enabled. Not formal seven-pair inference.', 'GPU timestamps enabled with retained raw queries; observer overhead and all-client driver duty remain unqualified.', 'Exact final window pixels/focus are endpoints, not calibrated physical presentation latency.', 'Sampled overlapping resource views and conservative reservation peaks do not establish complete memory/subcap/endurance/backend/scale/independent acceptance. CPU includes GUI and cooperating descendants through owned cgroup; snapshot RSS is GUI PID only.']
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  until(ready_pixels);time.sleep(5)
  start_offset=log.stat().st_size;report['action_log_start']=len(lines());c0=cpu();t0=time.monotonic_ns();s0=len(report['cpu_observer']['samples'])-1
  subprocess.run(['python3','/tmp/pm045-pointer-stream.py',geometry['X'],geometry['Y'],str(out/'input.json')],check=True)
  c1=cpu();t1=time.monotonic_ns();s1=len(report['cpu_observer']['samples'])-1;active_end_offset=log.stat().st_size
  time.sleep(5)
  c2=cpu();t2=time.monotonic_ns();s2=len(report['cpu_observer']['samples'])-1;end_offset=log.stat().st_size
  with log.open('rb') as source:source.seek(start_offset);(out/'active-and-tail.log').write_bytes(source.read(end_offset-start_offset))
  capture=out/'final.png';subprocess.run(['import','-window',str(main),str(capture)],check=True)
  report['pointer_trial']={'started_ns':t0,'active_finished_ns':t1,'tail_finished_ns':t2,'active_cpu_ms':(c1-c0)*1000,'tail_cpu_ms':(c2-c1)*1000,'start_sample':s0,'active_end_sample':s1,'tail_end_sample':s2,'start_log_offset':start_offset,'active_end_log_offset':active_end_offset,'end_log_offset':end_offset,'final_focus':int(xd('getwindowfocus')),'gpu_timestamps':env['DATUM_GPU_MEASUREMENTS']}
  assert report['pointer_trial']['final_focus']==main;save()
