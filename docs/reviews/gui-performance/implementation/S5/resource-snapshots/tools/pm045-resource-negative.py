...
out=Path(tempfile.mkdtemp(prefix='pm045-resource-negative-'+mode+'-'));print(out,flush=True)
...
r={'mode':mode,'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'cycles':[],'scope':'Bounded native resource snapshot delivery failure controls; no timing/RSS/full memory/endurance acceptance.'}
env=os.environ.copy()
for k in list(env):
