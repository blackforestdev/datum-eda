...
out=Path(tempfile.mkdtemp(prefix='pm045-pointer-threads-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  xd('mousemove','--window',main,300,150);time.sleep(5)
  ...
  observer=subprocess.Popen(['python3','/tmp/pm045-pointer-thread-observer.py',str(p.pid),str(out)])
  ...
  observer.wait(timeout=20);assert observer.returncode==0
  time.sleep(5)
