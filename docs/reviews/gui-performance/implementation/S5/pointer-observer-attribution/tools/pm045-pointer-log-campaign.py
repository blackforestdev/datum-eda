...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-pointer-log-campaign-'));print(out,flush=True)
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'OBSERVER OVERHEAD DIAGNOSTIC ONLY: three alternatingverbose/quiet pairs samecandidate;quietmode has no received-event/workcounts,exactwarm/finalpixels andfamilyCPU. W-POINTER native120Hz30s400x240rectangle,5swarmup/idle tail;three baseline/candidatepairs andone separatecandidateGPUtimestampdiagnostic;no relativeSTATacceptance. IntegerXTestpositionsroundplannedcoordinates;duplicates/coalescingmustbereported.','order':[['verbose',1,0],['quiet',1,0],['quiet',2,0],['verbose',2,0],['verbose',3,0],['quiet',3,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-pointer-stream.py'),'native_driver_sha256':sha('/tmp/pm045-pointer-log-native.py'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-pointer-log-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
