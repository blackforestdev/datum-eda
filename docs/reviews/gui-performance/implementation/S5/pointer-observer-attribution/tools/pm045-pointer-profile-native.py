...
out=Path(tempfile.mkdtemp(prefix='pm045-pointer-profile-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 profile_cmd=['gprofng','collect','app','-a','off','-F','off','-p','hi','-y','SIGUSR2','-o',str(out/'cpu.er'),*cmd]
 report['profile_command']=profile_cmd
 monitor=subprocess.Popen(['/tmp/pm045_fd_trace',str(trace_path),*profile_cmd],cwd=root,env=env,stdout=stream,stderr=stream)
 try:
 ...
 try:
  ...
  assert Path(f'/proc/{p.pid}/exe').resolve()==binary.resolve()
