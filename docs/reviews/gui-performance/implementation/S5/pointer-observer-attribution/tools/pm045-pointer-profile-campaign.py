...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-pointer-profile-campaign-'));print(out,flush=True)
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest();binaries={'baseline':root/'target/pm045-msaa-baseline/datum-gui','candidate':root/'target/release/datum-gui','verbose':root/'target/release/datum-gui','quiet':root/'target/release/datum-gui'}
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'CLOCK PROFILE DIAGNOSTIC ONLY: installed GNU gprofng0.997ms sampling,30second active window controlledSIGUSR2,binary archivingOFF,no descendantprofiling. Not CPUqualification. Previousdrivercontext: three alternatingverbose/quiet pairs samecandidate;quietmode has no received-event/workcounts,exactwarm/finalpixels andfamilyCPU. W-POINTER native120Hz30s400x240rectangle,5swarmup/idle tail;three baseline/candidatepairs andone separatecandidateGPUtimestampdiagnostic;no relativeSTATacceptance. IntegerXTestpositionsroundplannedcoordinates;duplicates/coalescingmustbereported.','order':[['quiet',1,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-pointer-stream.py'),'native_driver_sha256':sha('/tmp/pm045-pointer-profile-native.py'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  env=os.environ.copy();env.pop('LD_AUDIT',None);env.pop('PM045_X11_AUDIT_PATH',None);env.update(PM045_LOG_VERBOSE='1' if role=='verbose' else '0',PM045_POINTER_BINARY=str(binaries[role]),PM045_POINTER_ROLE=role,PM045_POINTER_GPU=str(gpu),PM045_CANDIDATE_SHA=r['binary_sha256'][role],PM045_RESOURCE_MODE='off',PM045_TRIAL=str(i),PM045_RESOURCE_DECLARATION=str(out/'declaration.json'))
  log=out/f'{role}-{i}-gpu{gpu}.log'
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-pointer-profile-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
