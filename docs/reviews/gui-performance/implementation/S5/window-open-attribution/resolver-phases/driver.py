...
out=Path(tempfile.mkdtemp(prefix='datum-s5-resolver-profile-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  profiler=subprocess.Popen(['gdb','-q','-batch','-p',str(p.pid),'-x','/tmp/datum-s5-resolver-profile.gdb'],stdout=profile_stream,stderr=subprocess.STDOUT)
