...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  for host in ('GLOBAL','PROJECT'):
   ...
   def capture(name):
    png=out/(host+'-'+name+'.png');subprocess.run(['import','-window',str(wid),str(png)],check=True);return png
   ...
   def drive(args,offset):
    ...
    if args and args[0] in ('click','mousedown','mouseup'):
     ...
     for key in keys:
      ...
      expected.append('keyboard physical=Code('+('Key'+name.upper() if len(name)==1 else name)+') ')
   ...
   def action(name,*args):
    offset=log.stat().st_size;c0=cpu();t0=time.monotonic_ns();drive(args,offset);time.sleep(.3);c1=cpu();t1=time.monotonic_ns()
