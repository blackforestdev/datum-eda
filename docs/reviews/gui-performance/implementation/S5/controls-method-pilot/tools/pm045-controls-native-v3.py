...
out=Path(tempfile.mkdtemp(prefix='pm045-controls-native-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  xd('windowactivate','--sync',main);until(lambda:'frame present end' in log.read_text())
  ...
  for host in ('GLOBAL','PROJECT'):
   title={'GLOBAL':'Global Preferences','PROJECT':'Project Preferences'}[host]
   xd('windowactivate','--sync',main);xd('mousemove','--window',main,148,16,'click',1)
   xd('key','--delay',20,'Up','Right',*(['Down'] if host=='PROJECT' else []));time.sleep(.15);xd('key','Return')
   wid=int(until(lambda:next((w for w in xd('search','--all','--onlyvisible','--pid',p.pid,'--name','^'+title).splitlines() if int(w)!=main),None)))
   until(lambda:int(xd('getwindowfocus'))==wid);observe_pixels(wid,0,host,'initial')
   ...
   def capture(name):
    png=out/(host+'-'+name+'.png');subprocess.run(['import','-window',str(wid),str(png)],check=True)
    if name in ('last-name','expanded','search','search-cleared','cancel-before-motion','cancel-after-motion'):
     physical=out/(host+'-'+name+'-compositor.png')
     capture=subprocess.run(['spectacle','-b','-n','-a','-e','-S','-o',str(physical)],capture_output=True,text=True,timeout=15)
     assert capture.returncode==0,(capture.returncode,capture.stderr)
     case.setdefault('compositor_captures',[]).append({'name':name,'path':physical.name,'size':Image.open(physical).size,'stderr':capture.stderr});save()
    return png
   ...
   def drive(args,offset,expect_logged=True):
    ...
    def delivered():
     rows=log.read_bytes()[offset:].decode().splitlines();matched=[line for line in rows if f'window event WindowId({wid}) ' in line]
   ...
   def action(name,*args):
    ...
    png=capture(name);case['actions'].append({'name':name,'input':args,'cpu_ms':(c1-c0)*1000,'start_ns':t0,'end_ns':t1,'start_log_offset':offset,'end_log_offset':log.stat().st_size,'capture':png.name});save();return png
   ...
   def thumb():
    png=capture('thumb-scan');im=Image.open(png).convert('RGB');assert im.size==(960,720)
   ...
   focus_offset=log.stat().st_size;xd('windowfocus','--sync',helper)
   until(lambda:f'window event WindowId({wid}) focused false' in log.read_bytes()[focus_offset:].decode())
   xd('windowfocus','--sync',wid);until(lambda:int(xd('getwindowfocus'))==wid);time.sleep(.3);before=capture('cancel-before-motion')
   ...
   close_window(wid);until(lambda:int(xd('getwindowfocus'))==main)
