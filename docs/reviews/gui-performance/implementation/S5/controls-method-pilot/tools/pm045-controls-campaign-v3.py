...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'W-CONTROLS method pilot, one sequence in actual Global and Project hosts. Focus/reveal last setting name, expand/collapse, search/clear, thumb grabs25/75percent, track page, focus loss cancel. Exact method/output validation before30-cycle workload; no cap or full qualification.','order':[['candidate',1,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-wheel-edges.py'),'native_driver_sha256':sha('/tmp/pm045-controls-native.py'),'runs':[]}
