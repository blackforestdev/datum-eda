 ...
 try:
  ...
  for host in ('GLOBAL','PROJECT'):
   ...
   def action(name,*args):
    offset=log.stat().st_size;c0=cpu();t0=time.monotonic_ns();xd(*args);time.sleep(.3);c1=cpu();t1=time.monotonic_ns()
   ...
   def thumb():
    ...
    ys=[y for y in range(100,720) if im.getpixel((954,y))==(178,184,195)];assert ys and max(ys)-min(ys)+1==len(ys),ys
