...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-controls-campaign-'));print(out,flush=True)
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-controls-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
