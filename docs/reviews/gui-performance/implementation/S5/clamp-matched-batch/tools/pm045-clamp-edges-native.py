...
out=Path(tempfile.mkdtemp(prefix='pm045-clamp-edges-native-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  for label,button,reverse in [('minimum',5,4),('maximum',4,5)]:
   ...
   assert active.count('mouse wheel delta=')==1800,active.count('mouse wheel delta=')
   assert positive.count('mouse wheel delta=')==1,positive.count('mouse wheel delta=')
