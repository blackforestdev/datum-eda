...
try:
 for i in range(count):
  ...
  sent=time.monotonic_ns();assert t.XTestFakeButtonEvent(d,button,1,0);assert t.XTestFakeButtonEvent(d,button,0,0);x.XFlush(d);rows.append({'index':i,'scheduled_ns':due,'sent_ns':sent})
