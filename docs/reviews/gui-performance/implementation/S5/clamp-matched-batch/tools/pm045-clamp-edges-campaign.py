...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-clamp-edges-campaign-'));print(out,flush=True)
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'Single candidate method verification, not MET01 batch. Timed1800 alternating button press/release transitions at60Hz; one inward press in positive window, release cleanup separately logged outside timing. Original failed-rate evidence retained; no qualification or cap claim.','order':[['candidate',1,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-wheel-edges.py'),'native_driver_sha256':sha('/tmp/pm045-clamp-edges-native.py'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-clamp-edges-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
