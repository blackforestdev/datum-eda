...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-clamp-matched-campaign-'));print(out,flush=True)
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'W-CLAMP corrected native edge schedule:1800 transitions at60Hz per bound plus separately timed single inward press, cleanup release outside timing. Three quiet baseline/candidate pairs alternating order; three matched candidate verbose structural trials. Endpoint reference is corrected pilot y2pegrfz. No formal seven-pair comparative inference, whole GPU/continuous-display/resource/backend-scale/full S5 qualification.','order':[['baseline',1,0],['quiet',1,0],['verbose',1,0],['verbose',2,0],['quiet',2,0],['baseline',2,0],['baseline',3,0],['quiet',3,0],['verbose',3,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-wheel-edges.py'),'native_driver_sha256':sha('/tmp/pm045-clamp-matched-native.py'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-clamp-matched-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
