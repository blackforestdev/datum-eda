...
out=Path(tempfile.mkdtemp(prefix='pm045-read-cache-on-')); print(out,flush=True)
...
report['source_state']='Uncommitted bounded resolver correction on recorded HEAD'
paths=['crates/engine/src/substrate/mod.rs','crates/engine/src/substrate/tests.rs','crates/engine/src/substrate/project_read_cache.rs','crates/engine/src/substrate/tests/project_read_cache.rs','crates/gui-app/src/project_preferences_runtime.rs','crates/gui-app/src/project_preferences_runtime_tests.rs']
report['source_sha256']={p:hashlib.sha256((root/p).read_bytes()).hexdigest() for p in paths}
report['limits'].append('Verbose diagnostics enabled: presentation/ownership evidence, not resource acceptance.')
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  main=int(re.search(r'\d+',initial['window'])[0]);xd('windowactivate','--sync',main);time.sleep(1)
