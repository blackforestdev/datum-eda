import os, sys, time, json, subprocess, hashlib, tempfile, re, uuid, ctypes, threading
from pathlib import Path
root=Path('/home/bfadmin/Documents/datum-eda')
out=Path(tempfile.mkdtemp(prefix='pm045-gpu-window-')); print(out,flush=True)
...
report['limits']=['GPU timestamps enabled; overhead not qualified. Polling may miss entire short GPU client lifetimes.','Method probe only:3Project cycles, first cold; not required30cycles/3trials.', 'Cgroup CPU includes in-process engine and all inherited children; external services remain unadmitted.', 'Window readback shows expected static content, not physical display/latency or complete input proof.', 'No GPU client/execution, observer-overhead, endurance or independent qualification.']
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  time.sleep(.25)
  drain_end=cpu()
  report['drain']={'begin_ns':drain_start,'end_ns':time.monotonic_ns(),'cpu_ms':(drain_end-action_end)*1000,'boundary':'Fixed250ms observed tail, not proof of all pending work complete'}
  ...
  assert not report['gpu_incomplete'],report['gpu_incomplete']
