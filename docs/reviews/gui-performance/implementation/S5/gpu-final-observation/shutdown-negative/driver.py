...
out=Path(tempfile.mkdtemp(prefix='pm045-final-handoff-negative-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  for host in ('PROJECT',):
   ...
   print('Missing-ack shutdown negative; no window cycles',flush=True)
  report['passed_native_cycles']=False;report['negative_control']='Observer closes after drained receipt without acknowledging'
  ...
  assert 'final measurement acknowledgement missing' in (out/'stderr.log').read_text()
  report['expected_negative_pass']=True
