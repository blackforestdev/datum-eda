...
out=Path(tempfile.mkdtemp(prefix='pm045-controls-30-v2-native-')); print(out,flush=True)
 ...
 try:
  ...
  for host in (os.environ['PM045_CONTROLS_HOST'],):
   ...
   for cycle in range(1,31):
    ...
    case.setdefault('clip_probes',[]).append({'cycle':cycle,'x':900,'y':clip_y,'expected':'Pinned Search overlaps fully clipped first control; no choice activation, exact unchanged image','begin_log_offset':clip_offset,'end_log_offset':log.stat().st_size});save()
