...
out=Path(tempfile.mkdtemp(prefix='pm045-controls-clip-native-')); print(out,flush=True)
 ...
 try:
  ...
  for host in (os.environ['PM045_CONTROLS_HOST'],):
   ...
   def action(name,*args):
    ...
    png=capture(name);case['actions'].append({'cycle':cycle,'name':name,'input':args,'cpu_ms':(c1-c0)*1000,'start_ns':t0,'end_ns':t1,'start_log_offset':offset,'end_log_offset':log.stat().st_size,'capture':png.name});save();expected_name='collapsed' if host=='GLOBAL' and cycle>1 and name=='last-name' else name;pixels_equal(png,reference/(host+'-'+expected_name+'.png'),name);return png
   assert host=='PROJECT'
   cycle=0
   ...
   reference_image=out/(host+'-0-search-refocus.png')
   case['clip_geometry']={'content_height':688,'viewport':[210,100,750,620],'bottom_offset':68,'first_row_top':32,'first_control_y':[46,76],'probe':[900,70],'basis':'Eight86px Units rows; bottom reveal clamps to688-620=68; control y=rowtop+14 andheight30. Point lies in fully clipped control and pinnedSearch. Source and image derivation retained, not native numerical-state telemetry.'}
   time.sleep(5)
   for cycle in range(1,31):
    case['current_cycle']=cycle;save();offset=log.stat().st_size
    drive(['mousemove','--window',wid,900,70,'click',1],offset);time.sleep(.3)
    png=capture('clip-probe');pixels_equal(png,reference_image,'clip-probe')
    case.setdefault('clip_probes',[]).append({'cycle':cycle,'x':900,'y':70,'begin_log_offset':offset,'end_log_offset':log.stat().st_size})
    case['completed_cycles']=cycle;save();print(json.dumps({'host':host,'completed_clip_probes':cycle}),flush=True)
   close_window(wid)
