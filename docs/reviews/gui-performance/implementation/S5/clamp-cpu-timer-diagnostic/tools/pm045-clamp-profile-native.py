...
out=Path(tempfile.mkdtemp(prefix='pm045-clamp-profile-native-')); print(out,flush=True)
...
env.update(LD_PRELOAD='/tmp/pm045_cpu_sampler.so',PM045_CPU_SAMPLE_PATH=str(out/'cpu-samples.bin'),PM045_CPU_SAMPLE_EXE=str(binary.resolve()),PM045_CPU_SAMPLE_PERIOD_NS='4000000')
if resource_mode=='on':
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  for label,button,reverse in [('minimum',5,4),('maximum',4,5)]:
   ...
   (out/(label+'-maps.txt')).write_text(Path(f'/proc/{p.pid}/maps').read_text())
   sample_bytes=(out/'cpu-samples.bin').stat().st_size
   assert libc.syscall(234,p.pid,p.pid,signal.SIGRTMIN+9)==0
   ...
   assert libc.syscall(234,p.pid,p.pid,signal.SIGRTMIN+9)==0
   until(lambda:(out/'cpu-samples.bin').stat().st_size>sample_bytes)
