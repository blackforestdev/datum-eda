...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-clamp-profile-campaign-'));print(out,flush=True)
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];r={'purpose':'Single candidate diagnostic: main-thread CPU timer PC samples at4ms during each30s clamp interval, separate control signals targetmainthread. No profiling during positive/release/tail. Existingquietbatch is independent reference, not matched overhead qualification. Must reconcile timer CPU/sample/overrun coverage, capture maps and symbolize exact binaries.','order':[['quiet',1,0]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-wheel-edges.py'),'native_driver_sha256':sha('/tmp/pm045-clamp-profile-native.py'),'sampler_source_sha256':sha('/tmp/pm045_cpu_sampler.c'),'sampler_binary_sha256':sha('/tmp/pm045_cpu_sampler.so'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-clamp-profile-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
