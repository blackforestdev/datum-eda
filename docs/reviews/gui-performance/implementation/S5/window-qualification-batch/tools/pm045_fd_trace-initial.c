...
static void before(struct thread *t) {
    ...
    case SYS_dup2:case SYS_dup3:
        if(t->args[0]!=t->args[1])descriptor(t,"replace_before",(int)t->args[1]);break;
    case SYS_close_range:
        if(!(t->args[2]&4))descriptor_set(t,"range_before",t->args[0],t->args[1],0);break;
...
static void after(struct thread *t,int64_t result) {
    ...
    case SYS_open:case SYS_openat:case SYS_openat2:case SYS_creat:case SYS_dup:
        descriptor(t,"birth_after",(int)result);break;
    case SYS_dup2:case SYS_dup3:
        if(t->args[0]!=t->args[1])descriptor(t,"birth_after",(int)result);break;
    case SYS_fcntl:
        if(t->args[1]==F_DUPFD||t->args[1]==F_DUPFD_CLOEXEC)descriptor(t,"birth_after",(int)result);break;
    default:break;
    }
}
