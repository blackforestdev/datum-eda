...
out=Path(tempfile.mkdtemp(prefix='pm045-window-batch-')); print(out,flush=True)
...
mode=os.environ['PM045_MODE'];assert mode in ('plain','kernel')
...
report['source_state']='Clean47eefbc1; production shutdown hook4183174a plus equivalent lint correction'
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  while True:
   ...
   assert time.monotonic()<end,'kernel observer did not identify child'
   time.sleep(.01)
 ...
 p=TracedProcess()
 report['kernel_observer']=root_record
 try:
  ...
  def cycle(host,i,phase):
    ...
    report['cycles'].append({'host':host,'title':title_expected,'phase':phase,'cycle':i,'open':opened,'close':{'cpu_ms':(cpu()-c1)*1000,'wall_ms':(time.monotonic()-t1)*1000,'start_cpu_sample':close_sample,'end_cpu_sample':len(report['cpu_observer']['samples'])-1},'focus_restored':True,'rss':rss()});save()
  for host in ('GLOBAL','PROJECT','NEW'):cycle(host,0,'first-use')
  time.sleep(5)
  report['action_log_start']=len(lines())
  report['workload_cpu_start']=cpu();report['workload_first_cpu_sample']=len(report['cpu_observer']['samples'])-1
  for host in ('GLOBAL','PROJECT','NEW'):
   for i in range(1,31):cycle(host,i,'warm')
   print(mode,trial,host,'30warm cycles complete',flush=True)
  ...
  report['kernel_ledger_lines']=len(trace_path.read_text().splitlines());save()
