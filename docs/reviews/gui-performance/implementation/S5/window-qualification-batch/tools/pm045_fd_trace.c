...
static void descriptor(struct thread *t,const char *phase,int fd) {
    ...
    if(!strstr(raw,"drm-client-id:")){
        struct stat st;snprintf(path,sizeof(path),"/proc/%d/fd/%d",t->tid,fd);
        if(stat(path,&st))fail("fd classification changed during observation");
        char target[4096];ssize_t length=readlink(path,target,sizeof(target)-1);
        if(length<0)fail("fd target unavailable");
        target[length]=0;
        if(S_ISCHR(st.st_mode)&&(major(st.st_rdev)==226||!strncmp(target,"/dev/nvidia",11))){
            pid_t pid;unsigned long long start;identity(t->tid,&pid,&start);
            fprintf(output,"{\"kind\":\"opaque_gpu\",\"phase\":\"%s\",\"pid\":%d,\"start\":%llu,\"tid\":%d,\"fd\":%d,\"sequence\":%lu,\"syscall\":%llu,\"monotonic_ns\":%llu,\"major\":%u,\"minor\":%u,\"path_hex\":\"",phase,pid,start,t->tid,fd,t->sequence,(unsigned long long)t->number,(unsigned long long)now(),major(st.st_rdev),minor(st.st_rdev));
            for(ssize_t i=0;i<length;i++)fprintf(output,"%02x",(unsigned char)target[i]);
            fprintf(output,"\",\"raw_hex\":\"");for(long i=0;i<n;i++)fprintf(output,"%02x",(unsigned char)raw[i]);
            fprintf(output,"\",\"qualification\":\"unsupported; no invented counter\"}\n");flush();return;
        }
        fprintf(output,"{\"kind\":\"non_drm\",\"tid\":%d,\"fd\":%d,\"sequence\":%lu,\"phase\":\"%s\"}\n",t->tid,fd,t->sequence,phase);flush();return;
    }
...
static void remote_read(pid_t tid,uint64_t address,void *data,size_t length) {
    struct iovec local={data,length},remote={(void *)(uintptr_t)address,length};
    if(process_vm_readv(tid,&local,1,&remote,1,0)!=(ssize_t)length)fail("tracee buffer incomplete");
}
static void received(struct thread *t,uint64_t address) {
    struct msghdr message;remote_read(t->tid,address,&message,sizeof(message));
    if(!message.msg_controllen)return;
    if(message.msg_controllen>65536)fail("control buffer bound");
    unsigned char control[65536];remote_read(t->tid,(uintptr_t)message.msg_control,control,message.msg_controllen);
    message.msg_control=control;
    for(struct cmsghdr *c=CMSG_FIRSTHDR(&message);c;c=CMSG_NXTHDR(&message,c)) {
        size_t offset=(unsigned char *)c-control;
        if(c->cmsg_len<CMSG_LEN(0)||c->cmsg_len>message.msg_controllen-offset)fail("invalid control length");
        if(c->cmsg_level!=SOL_SOCKET||c->cmsg_type!=SCM_RIGHTS)continue;
        size_t length=c->cmsg_len-CMSG_LEN(0);if(length%sizeof(int))fail("invalid rights length");
        for(size_t i=0;i<length/sizeof(int);i++){
            int fd;memcpy(&fd,CMSG_DATA(c)+i*sizeof(int),sizeof(fd));descriptor(t,"receive_after",fd);
        }
    }
}
static void before(struct thread *t) {
    ...
    case SYS_exit_group:case SYS_exit:descriptor_set(t,"exit_before",0,~0u,0);break;
    case SYS_io_uring_setup:fail("io_uring descriptor operations unsupported; trial invalid");break;
    case SYS_ioctl:fail("DRM lease creation unsupported; trial invalid");break;
    case SYS_clone:
        if(t->args[0]&CLONE_UNTRACED)fail("untraced clone unsupported; trial invalid");
        break;
    case SYS_clone3: {
        uint64_t flags;remote_read(t->tid,t->args[0],&flags,sizeof(flags));
        if(flags&CLONE_UNTRACED)fail("untraced clone3 unsupported; trial invalid");
        break;
    }
    default:break;
    }
...
static void after(struct thread *t,int64_t result) {
    fprintf(output,"{\"kind\":\"result\",\"tid\":%d,\"sequence\":%lu,\"syscall\":%llu,\"result\":%lld,\"args\":[%llu,%llu,%llu],\"monotonic_ns\":%llu}\n",t->tid,t->sequence,(unsigned long long)t->number,(long long)result,(unsigned long long)t->args[0],(unsigned long long)t->args[1],(unsigned long long)t->args[2],(unsigned long long)now());flush();
    if(result<0)return;
    switch(t->number) {
    case SYS_open:case SYS_openat:case SYS_openat2:case SYS_creat:case SYS_dup:
    case SYS_open_by_handle_at:case SYS_pidfd_getfd:
    ...
    case SYS_fcntl:
        ...
        break;
    case SYS_recvmsg:received(t,t->args[1]);break;
    case SYS_recvmmsg:
        if(result>1024)fail("message vector bound");
        for(int64_t i=0;i<result;i++)received(t,t->args[1]+i*sizeof(struct mmsghdr));
        break;
    default:break;
    }
}
...
static void filter(void) {
    struct sock_filter code[]={
        ...
        TRACE(SYS_fcntl),TRACE(SYS_execve),TRACE(SYS_execveat),TRACE(SYS_exit_group),TRACE(SYS_exit),
        TRACE(SYS_recvmsg),TRACE(SYS_recvmmsg),TRACE(SYS_pidfd_getfd),TRACE(SYS_open_by_handle_at),
        TRACE(SYS_clone),TRACE(SYS_clone3),TRACE(SYS_io_uring_setup),
        /* DRM mode create-lease request type/number; reject this unadmitted path. */
        BPF_JUMP(BPF_JMP|BPF_JEQ|BPF_K,SYS_ioctl,0,4),
        BPF_STMT(BPF_LD|BPF_W|BPF_ABS,offsetof(struct seccomp_data,args[1])),
        BPF_STMT(BPF_ALU|BPF_AND|BPF_K,0xffff),
        BPF_JUMP(BPF_JMP|BPF_JEQ|BPF_K,0x64c6,0,1),
...
int main(int argc,char **argv) {
    ...
    int tracing=!getenv("DATUM_FD_TRACE_MODE")||strcmp(getenv("DATUM_FD_TRACE_MODE"),"off");
    ...
    if(!root){
        ...
        if(tracing){
            ...
            raise(SIGSTOP);filter();
        }
        execvp(argv[2],argv+2);fail("exec");
    }
    int status;
    if(!tracing){
        fprintf(output,"{\"kind\":\"root\",\"pid\":%d,\"observer_pid\":%d,\"tracing\":false,\"monotonic_ns\":%llu}\n",root,getpid(),(unsigned long long)now());flush();
        while(waitpid(root,&status,0)<0){if(errno!=EINTR)fail("untraced wait");}
        int code=WIFEXITED(status)?WEXITSTATUS(status):128+WTERMSIG(status);
        fprintf(output,"{\"kind\":\"observer_end\",\"root_exit\":%d,\"tracing\":false}\n",code);flush();fclose(output);return code;
    }
    if(waitpid(root,&status,0)!=root||!WIFSTOPPED(status))fail("initial stop");
    ...
    while(1){
        ...
        if(signal==SIGSTOP){
            pid_t pid;unsigned long long start;identity(tid,&pid,&start);
            fprintf(output,"{\"kind\":\"identity\",\"tid\":%d,\"pid\":%d,\"start\":%llu}\n",tid,pid,start);flush();
            if(pid==tid)descriptor_set(t,"inherited",0,~0u,0);
        }
