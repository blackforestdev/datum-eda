...
out=Path(tempfile.mkdtemp(prefix='pm045-byte-cache-profile-')); print(out,flush=True)
...
report={'candidate':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'command':cmd,'backend':'x11 on Xwayland','cycles':[],'limits':['GDB diagnostic phase attribution, not qualification metrics. Diagnostics off: process CPU and native window/focus only; engine, GPU, live allocations and full timing not qualified.','No endurance or independent replay acceptance.']}
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  profile_stream=(out/'profile.log').open('w')
  profiler=subprocess.Popen(['gdb','-q','-batch','-p',str(p.pid),'-x','/tmp/pm045-byte-cache-profile.gdb'],stdout=profile_stream,stderr=subprocess.STDOUT)
  until(lambda:'PROFILE_READY' in (out/'profile.log').read_text())
  for host in ('PROJECT',):
   for i in range(5):
   ...
   print(host,'5 profiled cycles complete',flush=True)
  ...
  if 'profiler' in locals():
   profiler.wait(timeout=10);profile_stream.close()
