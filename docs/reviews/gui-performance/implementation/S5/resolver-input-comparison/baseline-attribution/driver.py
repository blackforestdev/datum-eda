...
out=Path(tempfile.mkdtemp(prefix='pm045-read-cache-profile-')); print(out,flush=True)
...
with (out/'stderr.log').open('w') as stream:
 ...
 try:
  ...
  profiler=subprocess.Popen(['gdb','-q','-batch','-p',str(p.pid),'-x','/tmp/pm045-read-cache-profile.gdb'],stdout=profile_stream,stderr=subprocess.STDOUT)
