...
root=Path.cwd();out=Path(tempfile.mkdtemp(prefix='pm045-stroke-campaign-'));print(out,flush=True)
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest();binaries={'baseline':root/'target/pm045-stroke-baseline/datum-gui','candidate':root/'target/release/datum-gui'}
...
source=json.loads(Path('/tmp/pm045-resource-campaign-jksuuoyg/declaration.json').read_text())['source_sha256'];source['crates/gui-render/src/render/gpu_strokes.rs']=sha(root/'crates/gui-render/src/render/gpu_strokes.rs');r={'purpose':'STROKE CORRECTION DIAGNOSTIC: three alternating matched baseline/candidate pairs,allquiet andGPUtimestampsON. Exactwarm/finalpixels/focus andfamilyCPU. No received-event/workcounts. Quantizedinput andtimestampoverhead unqualified;notfullWPOINTER/CPUqualification orformalrelativeSTAT. W-POINTER native120Hz30s400x240rectangle,5swarmup/idle tail;three baseline/candidatepairs andone separatecandidateGPUtimestampdiagnostic;no relativeSTATacceptance. IntegerXTestpositionsroundplannedcoordinates;duplicates/coalescingmustbereported.','order':[['baseline',1,1],['candidate',1,1],['candidate',2,1],['baseline',2,1],['baseline',3,1],['candidate',3,1]],'source_sha256':source,'binary_sha256':{k:sha(v) for k,v in binaries.items()},'input_driver_sha256':sha('/tmp/pm045-pointer-stream.py'),'native_driver_sha256':sha('/tmp/pm045-stroke-native.py'),'runs':[]}
...
try:
 ...
 for role,i,gpu in r['order']:
  ...
  with log.open('w') as f:p=subprocess.run(['python3','/tmp/pm045-stroke-native.py'],env=env,stdout=f,stderr=subprocess.STDOUT)
