...
out=pathlib.Path('/tmp/pm045-readiness');out.mkdir(exist_ok=True)
binary=root/'target/release/datum-gui'
fixture=pathlib.Path('/tmp/datum-gui-candidate-boundary-v3/DOA2526.kicad_pcb')
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
report={'scope':'Bounded S0 baseline: actual production host startup and existing six-extent native input/final-state regression; not a budget, latency or overhead trial.','revision':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'binary_sha256':sha(binary),'fixture_sha256':sha(fixture),'runs':[]}
for host,flag in [('MAIN',None),('GLOBAL','--open-global-preferences'),('PROJECT','--open-project-preferences'),('NEW','--open-new-project')]:
 ...
 env.update(DATUM_GPU_MEASUREMENTS='0',DATUM_GUI_VERBOSE_LOG='1',DATUM_TRACE_TIMING='1',DATUM_GUI_LOG=str(case/'native.log'),WINIT_UNIX_BACKEND='wayland',XDG_CONFIG_HOME=str(case/'config'),XDG_CACHE_HOME=str(case/'cache'),TMPDIR=str(case))
 ...
 with (case/'stderr.log').open('w') as stream:
  ...
  try:
   ...
   ok=p.poll() is None and len(observed)==6 and '1280x800 configured 1280x800 presented=true' in observed[-1] and len(identity)==(1 if host=='MAIN' else 2) and all('window_backend=wayland' in r for r in identity)
