...
def capture(window,path):
 pass # Stack-sampling diagnostic only; no visual proof.
def pixels(path):
...
def action(w,name,n):
 if name in ('height','width'):
  ...
  if n%2==0:
   ...
   for _ in range(50):
    found=cmd('xdotool','search','--onlyvisible','--pid',str(gui_pid)).split()
...
def sample(pid,window,name,rate=60):
 ...
 if name!='idle':os.kill(pid,12) # gprofng SIGUSR2 collection toggle
 ...
 sample['prepared_builds']=(diag.count('prepared scene build begin') if env.get('DATUM_GUI_VERBOSE_LOG') else None);sample['retained_builds']=(diag.count('retained scene build begin') if env.get('DATUM_GUI_VERBOSE_LOG') else None);sample['surface_recoveries']=diag.count('surface acquire recovered');sample['surface_timeouts']=diag.count('surface acquire timeout')
 ...
 probe={}
 for phase,wall,process,thread in re.findall(r'cpu_probe phase=(\w+) wall_ns=(\d+) process_ns=(\d+) thread_ns=(\d+)',diag):
  entry=probe.setdefault(phase,{'calls':0,'wall_ns':0,'process_ns':0,'thread_ns':0})
  entry['calls']+=1
  for k,v in [('wall_ns',wall),('process_ns',process),('thread_ns',thread)]:entry[k]+=int(v)
 sample['cpu_probe']=probe
...
with logpath.open('w') as log:
 p=subprocess.Popen(['gprofng','collect','app','-F','off','-a','off','-p','5','-y','USR2','-o',str(out/'profile.er'),str(binary),'--board',str(board),'--window-size','1280x800',*flags],cwd=root,env=env,stdout=log,stderr=log)
 try:
  deadline=time.monotonic()+60
  while time.monotonic()<deadline:
   diag_path=out/'datum-gui-last.log'
   startup=diag_path.read_text() if diag_path.exists() else ''
   found=re.search(r'pid=(\d+)',startup)
   if found:gui_pid=int(found[1]);break
   if p.poll() is not None:raise RuntimeError('profiler launcher exited')
   time.sleep(.1)
  else:raise TimeoutError('profiled application startup')
  windows=[]
  for _ in range(200):
   if p.poll() is not None:raise RuntimeError('GUI exited: '+logpath.read_text()[-2000:])
   found=subprocess.run(['xdotool','search','--onlyvisible','--pid',str(gui_pid)],env=env,capture_output=True,text=True).stdout.split();windows=[(int(w),cmd('xdotool','getwindowname',w)) for w in found]
  ...
  observe(target)
  time.sleep(5)
  sample(gui_pid,target,'idle',rate=10)
  ...
  for phase in phases:
   ...
   if phase=='new_cycle':capture_target=next(int(v) for v in cmd('xdotool','search','--onlyvisible','--pid',str(gui_pid)).split() if int(v)!=main)
   ...
   time.sleep(5)
   for trial in range(a.trials):sample(gui_pid,target,phase,60 if phase in ('pan','height','width') else 20 if phase=='scroll' else 2)
  ...
  for _ in range(int(os.environ.get('DATUM_RESIZE_IDLE_REPEATS','3'))):sample(gui_pid,target,'idle',rate=10)

 finally:
  if p.poll() is None:
   if 'gui_pid' in locals():os.kill(gui_pid,15)
   p.wait(timeout=15)
  x.XCloseDisplay(d)
