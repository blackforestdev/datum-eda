...
with logpath.open('w') as log:
 ...
 try:
  ...
  # Separate correctness-only probe. Read PTY size; never send shell commands.
  import fcntl,struct,termios
  click(target,40,760);time.sleep(2)
  terminal_rows=[]
  for width,height in [(1280,1040),(1520,800),(1280,800)]:
   x.XResizeWindow(d,target,width,height);x.XFlush(d);time.sleep(1)
   diag=(out/'datum-gui-last.log').read_text()
   found=re.findall(r'terminal resize begin (\d+)x(\d+)',diag)
   assert found,'Terminal tab did not expose resize diagnostics'
   columns,rows=map(int,found[-1]);observed=[]
   for info in pathlib.Path(f'/proc/{p.pid}/fdinfo').iterdir():
    match=re.search(r'^tty-index:\s*(\d+)',info.read_text(),re.M)
    if not match:continue
    fd=os.open('/dev/pts/'+match[1],os.O_RDONLY|os.O_NOCTTY)
    try:pr,pc,pw,ph=struct.unpack('HHHH',fcntl.ioctl(fd,termios.TIOCGWINSZ,bytes(8)))
    finally:os.close(fd)
    observed.append({'columns':pc,'rows':pr,'pixel_width':pw,'pixel_height':ph})
   assert observed and any(v['columns']==columns and v['rows']==rows for v in observed),(columns,rows,observed)
   terminal_rows.append({'window':[width,height],'layout_columns':columns,'layout_rows':rows,'pty':observed})
  capture(target,out/'terminal-final.png')
  (out/'terminal-proof.json').write_text(json.dumps({'passed':True,'samples':terminal_rows,'scope':'Visible terminal layout diagnostics compared with read-only kernel PTY ioctl after both physical axes and restored size; no shell commands.'},indent=2)+'\n')
