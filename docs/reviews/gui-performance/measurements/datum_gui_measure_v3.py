...
def sample(pid,window,name,rate=60):
 # Final screenshots are output evidence, not compositor feedback.
 ...
 sample={'phase':name,'duration_s':elapsed,'input_events_sent':sent,'requested_hz':rate,'cpu_seconds':cpu,'cpu_percent_one_core':cpu/elapsed*100,'rss_peak_kib':peak,'gpu_engine_active_percent':{k:max(0,v-start_gpu[k])/(elapsed*1e9)*100 for k,v in gpu_end.items() if k in start_gpu},'world_bundle_rebuilds':log.count('world bundle rebuilt') if a.trace else None,'main_submissions':len(frames) if a.trace else None,'dialog_submissions':len(dialogs) if a.trace else None,'prepared_miss_frames':sum('prepared_was_cached=false' in v for v in frames) if a.trace else None,'retained_miss_frames':sum('retained_was_cached=false' in v for v in frames) if a.trace else None,'main_renderer_ms':[int(re.search(r'renderer=(\d+)ms',v)[1]) for v in frames],'dialog_renderer_us':dialogs,'raw_counters':timeline}
