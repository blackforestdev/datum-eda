...
Item {
 ...
 function pairValid() {
  let windows = Workspace.stackingOrder.filter(w => w.pid === 343447);
 ...
 Component.onCompleted: {
  ...
  let matches = Workspace.stackingOrder.filter(w => w.pid === 343447 && !w.caption.includes("Preferences") && !w.caption.includes("New Project"));
 ...
 Timer { id: timer; interval: 20; repeat: true
  onTriggered: {
   if (!owned || owned.pid !== 343447 || !pairValid()) { stop(); return; }
   if (count === 240) {
