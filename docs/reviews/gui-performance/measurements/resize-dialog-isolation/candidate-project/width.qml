...
Item {
 ...
 function pairValid() {
  let windows = Workspace.stackingOrder.filter(w => w.pid === 344032);
  return windows.length === 2 && windows.filter(w => w.caption.includes("Project Preferences") && !w.minimized).length === 1;
 }
 Component.onCompleted: {
  if (!pairValid()) { console.error("DATUM requested dialog absent"); return; }
  let matches = Workspace.stackingOrder.filter(w => w.pid === 344032 && !w.caption.includes("Preferences") && !w.caption.includes("New Project"));
 ...
 Timer { id: timer; interval: 20; repeat: true
  onTriggered: {
   if (!owned || owned.pid !== 344032 || !pairValid()) { stop(); return; }
   if (count === 240) {
   ...
   owned.frameGeometry = Qt.rect(original.x, original.y,
      original.width + ("width" === "width" ? delta : 0),
      original.height + ("width" === "height" ? delta : 0));
   count++;
  }
 }
}
