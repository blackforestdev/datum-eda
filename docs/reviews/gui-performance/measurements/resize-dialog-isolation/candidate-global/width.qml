...
Item {
 ...
 function pairValid() {
  let windows = Workspace.stackingOrder.filter(w => w.pid === 343704);
  return windows.length === 2 && windows.filter(w => w.caption.includes("Global Preferences") && !w.minimized).length === 1;
 }
 Component.onCompleted: {
  if (!pairValid()) { console.error("DATUM requested dialog absent"); return; }
  let matches = Workspace.stackingOrder.filter(w => w.pid === 343704 && !w.caption.includes("Preferences") && !w.caption.includes("New Project"));
 ...
 Timer { id: timer; interval: 20; repeat: true
  onTriggered: {
   if (!owned || owned.pid !== 343704 || !pairValid()) { stop(); return; }
   if (count === 240) {
