...
def sample(pid,window,name,rate=60):
 ...
 if name=='clamped_zoom':
  ...
  time.sleep(1)
  capture(window,out/f'before-{len(samples)}.png')
 ...
 if name=='clamped_zoom':
  capture(window,out/f'after-{len(samples)}.png')
  ...
  time.sleep(.5)
  reverse=out/f'reverse-{len(samples)}.png';capture(window,reverse)
  ...
  if before_hash!=after_hash or not changed or (a.trace and rendered==0):
   (out/'failed-sample.json').write_text(json.dumps(sample,indent=2)+'\n')
   raise RuntimeError('reverse control failed: target/input not verified')
