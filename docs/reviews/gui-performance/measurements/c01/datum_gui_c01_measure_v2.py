...
def action(w,name,n):
 if name=='pan':move(w,370+(n%60),370+(n%30))
...
with logpath.open('w') as log:
 ...
 try:
  ...
  if owned:
   ...
   if a.mode=='global':click(owned,90,106)
   move(owned,420,400);button(5);button(5)
  time.sleep(5)
  sample(p.pid,target,'idle',rate=10)
  phases=a.phases.split(',') if a.mode=='main' else ['scroll']
