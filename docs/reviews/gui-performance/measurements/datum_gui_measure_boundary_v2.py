...
def sample(pid,window,name,rate=60):
 ...
 sample={'phase':name,'duration_s':elapsed,'input_events_sent':sent,'requested_hz':rate,'cpu_seconds':cpu,'cpu_percent_one_core':cpu/elapsed*100,'rss_peak_kib':peak,'gpu_engine_active_percent':{k:max(0,v-start_gpu[k])/(elapsed*1e9)*100 for k,v in gpu_end.items() if k in start_gpu},'world_bundle_rebuilds':log.count('world bundle rebuilt') if a.trace else None,'main_submissions':len(frames) if a.trace else None,'dialog_submissions':len(dialogs) if a.trace else None,'prepared_miss_frames':sum('prepared_was_cached=false' in v for v in frames) if a.trace else None,'retained_miss_frames':sum('retained_was_cached=false' in v for v in frames) if a.trace else None,'control_reverse':None,'main_renderer_ms':[int(re.search(r'renderer=(\d+)ms',v)[1]) for v in frames],'dialog_renderer_us':dialogs,'raw_counters':timeline}

 if name=='clamped_zoom':
  subprocess.run(['import','-window',str(window),str(out/f'after-{len(samples)}.png')],env=env,check=True,timeout=10)
  control_offset=logpath.stat().st_size
  for _ in range(3):
   xt.XTestFakeButtonEvent(d,5,1,0);xt.XTestFakeButtonEvent(d,5,0,0);x.XFlush(d);time.sleep(.1)
  time.sleep(.5)
  reverse=out/f'reverse-{len(samples)}.png';subprocess.run(['import','-window',str(window),str(reverse)],env=env,check=True,timeout=10)
  before_hash=hashlib.sha256(pixels(out/f'before-{len(samples)}.png')).hexdigest();after_hash=hashlib.sha256(pixels(out/f'after-{len(samples)}.png')).hexdigest();reverse_hash=hashlib.sha256(pixels(reverse)).hexdigest();changed=reverse_hash!=after_hash
  rendered=len(re.findall(r'\[datum-timing\] runtime render ',logpath.read_bytes()[control_offset:].decode(errors='replace')))
  sample['control_reverse']={'events_sent':3,'pixels_changed':changed,'clamp_pixels_unchanged':before_hash==after_hash,'before_rgba_sha256':before_hash,'after_rgba_sha256':after_hash,'reverse_rgba_sha256':reverse_hash,'main_submissions':rendered if a.trace else None}
  if before_hash!=after_hash or not changed or (a.trace and rendered==0):raise RuntimeError('reverse control failed: target/input not verified')
