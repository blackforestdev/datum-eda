...
with (out/'gui.log').open('w') as log:
 p=subprocess.Popen(['gprofng','collect','app','-F','off','-a','off','-p','5','-y','USR2','-o',str(out/'profile.er'),str(binary),'--board',str(board),'--window-size','1280x800'],cwd=root,env=env,stdout=log,stderr=log)
 try:
  ...
  gui_pid=int(re.search(r'pid=(\d+)',diag.read_text())[1])
  time.sleep(3)
  for axis in ['idle','height','width','idle']:
   if axis!='idle':
    plugin=f'datum-resize-accounting-{gui_pid}-{axis}'
    qml=out/(axis+'.qml');qml.write_text(script(gui_pid,axis))
   ...
   if axis!='idle':os.kill(gui_pid,signal.SIGUSR2)
   t0=time.monotonic();c0=cpu(gui_pid);g0=gpu(gui_pid)
   ...
   while time.monotonic()-t0<10:
    ...
    timeline.append({'elapsed_s':time.monotonic()-t0,'cpu_ticks':cpu(gui_pid),'gpu':gpu(gui_pid)})
    time.sleep(.1)
   elapsed=time.monotonic()-t0;c1=cpu(gui_pid);g1=gpu(gui_pid)
   if axis!='idle':os.kill(gui_pid,signal.SIGUSR2)
  ...
  if p.poll() is None:
   if 'gui_pid' in locals():os.kill(gui_pid,signal.SIGTERM)
   else:p.terminate()
   p.wait(timeout=30)
