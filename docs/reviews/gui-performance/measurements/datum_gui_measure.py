...
def cmd(*args):return subprocess.check_output(args,env=env,text=True).strip()
def ticks(pid):
...
def sample(pid,window,name,rate=60):
 # Screenshot before/after is visual/output evidence, not presentation feedback.
 cmd('xdotool','windowactivate','--sync',str(window));time.sleep(.4)
 x.XWarpPointer(d,0,window,0,0,0,0,600,400);x.XFlush(d);time.sleep(.4)
 ...
 while time.monotonic()-start<a.seconds:
  if name=='pointer':x.XWarpPointer(d,0,window,0,0,0,0,500+(sent%100)*2,350+(sent%50))
  elif name in ('zoom','scroll'):
   button=4 if (sent//10)%2==0 else 5;xt.XTestFakeButtonEvent(d,button,1,0);xt.XTestFakeButtonEvent(d,button,0,0)
  x.XFlush(d)
  if name!='idle':sent+=1
 ...
 elapsed=time.monotonic()-start;cpu=(ticks(pid)-start_ticks)/hz;gpu_end=drm(pid);log=logpath.read_text()[start_offset:]
...
with logpath.open('w') as log:
 ...
 try:
  ...
  for phase,window,rate in [('pointer',main,60),('zoom',main,10)]:
  ...
  if owned and a.mode in ('global','project'):
   ...
   if a.mode=='global':
    cmd('xdotool','windowactivate','--sync',str(owned));x.XWarpPointer(d,0,owned,0,0,0,0,90,275);x.XFlush(d);xt.XTestFakeButtonEvent(d,1,1,0);xt.XTestFakeButtonEvent(d,1,0,0);x.XFlush(d);time.sleep(1)
