...
pub(super) fn visible_rows(
    ...
    bounds: RectPx,
    scale: f32,
) -> anyhow::Result<Vec<(usize, RectPx)>> {
    Ok({
...
mod tests {
        ...
        for scale in [1.0, 1.25, 1.5, 2.0] {
            for width in [180.0, 330.0, 518.0] {
                ...
                for end in 1..=rows.len() {
                    let visible = visible_rows(&rows[..end], bounds, scale).unwrap();
    ...
    fn wrapping_counts_newlines_and_long_words_and_bounds_oversized_records() {
        assert!(
            measured_text_run_height_px("first\nsecond", 200.0, 11.5, TextFace::Mono).unwrap()
                > measured_text_run_height_px("first", 200.0, 11.5, TextFace::Mono).unwrap()
        );
        assert!(
            measured_text_run_height_px(&"X".repeat(100), 100.0, 11.5, TextFace::Mono).unwrap()
                > measured_text_run_height_px("X", 100.0, 11.5, TextFace::Mono).unwrap()
        );
        ...
        let visible = visible_rows(&rows, bounds, 1.0).unwrap();
