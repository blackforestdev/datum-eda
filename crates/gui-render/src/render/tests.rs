...
mod tests {
    ...
    // P2.2f: the companion schematic pass draws a subtle SQUARE grid underlay. This
    // is a structural check that `push_schematic_grid` produces grid geometry in
    // the schematic `#sgrid` whisper colours (major + minor at the fine tier) and
    // that every emitted quad is one of those two colours — the grid must never
    ...
    fn schematic_grid_emits_square_underlay_geometry() {
        ...
        // The grid is square: emitted as WORLD-space lines, it produces both
        ...
        assert!(
            ...
            "a square grid must emit both vertical and horizontal world lines"
        );
    }
}
