...
#[test]
fn project_compare_forward_annotation_artifact_reports_applicable_drifted_and_stale() {
    let root = unique_project_root("datum-eda-cli-project-forward-annotation-compare-artifact");
    create_native_project(&root, Some("Forward Annotation Compare Demo".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Forward Annotation Compare Demo Board",
                "stackup": { "layers": [] },
