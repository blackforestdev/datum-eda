...
use eda_engine::board::{
    ...
    RoutePathCandidateAuthoredCopperGraphPolicyReport, RoutePathCandidateStatus,
};
use uuid::Uuid;
...
pub(crate) fn render_native_project_route_path_candidate_authored_copper_graph_text(
    report: &RoutePathCandidateAuthoredCopperGraphPolicyReport,
) -> String {
    let mut lines = vec![
...
fn render_status(report: &RoutePathCandidateAuthoredCopperGraphPolicyReport) -> &'static str {
    match report.status {
