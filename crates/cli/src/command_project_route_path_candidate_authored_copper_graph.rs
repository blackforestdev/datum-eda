...
use eda_engine::board::{RoutePathCandidateAuthoredCopperGraphReport, RoutePathCandidateStatus};
use uuid::Uuid;
...
) -> Result<RoutePathCandidateAuthoredCopperGraphReport> {
    let project = load_native_project(root)?;
    let board = build_native_project_board(&project)?;
    board
        .route_path_candidate_authored_copper_graph(net_uuid, from_anchor_pad_uuid, to_anchor_pad_uuid)
        .map_err(|err| anyhow!(err))
}

pub(crate) fn render_native_project_route_path_candidate_authored_copper_graph_text(
...
fn render_status(report: &RoutePathCandidateAuthoredCopperGraphReport) -> &'static str {
    match report.status {
