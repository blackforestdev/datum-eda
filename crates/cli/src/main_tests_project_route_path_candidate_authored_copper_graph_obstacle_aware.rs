...
fn route_path_candidate_authored_copper_graph_obstacle_aware_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
...
fn project_query_route_path_candidate_authored_copper_graph_obstacle_aware_reports_blocked_existing_path()
 {
    ...
    create_native_project(
        &root,
        Some("Route Path Candidate Authored Copper Graph Obstacle Aware Demo".to_string()),
    )
    .expect("initial scaffold should succeed");

    let target_net_uuid = Uuid::from_u128(0x3650);
    let class_uuid = Uuid::from_u128(0x3651);
    let package_a_uuid = Uuid::from_u128(0x3652);
    let package_b_uuid = Uuid::from_u128(0x3653);
    let anchor_a_uuid = Uuid::from_u128(0x3654);
    let anchor_b_uuid = Uuid::from_u128(0x3655);
    let track_uuid = Uuid::from_u128(0x3656);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::from_u128(0x3657),
                "name": "Route Path Candidate Authored Copper Graph Obstacle Aware Demo Board",
                "stackup": {
                    "layers": [
