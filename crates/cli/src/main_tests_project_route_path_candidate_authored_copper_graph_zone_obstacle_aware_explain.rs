...
fn route_path_candidate_authored_copper_graph_zone_obstacle_aware_explain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
    ...
    Cli::try_parse_from([
        ...
        "query",
        root.to_str().unwrap(),
        "route-path-candidate-authored-copper-graph-zone-obstacle-aware-explain",
        "--net",
        &net_uuid.to_string(),
        "--from-anchor",
...
fn project_query_route_path_candidate_authored_copper_graph_zone_obstacle_aware_explain_reports_blocked_path()
 {
    ...
    create_native_project(
        &root,
        Some(
            "Route Path Candidate Authored Copper Graph Zone Obstacle Aware Explain Demo"
                .to_string(),
        ),
    )
    ...
    let zone_uuid = Uuid::from_u128(0x3956);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                ...
                "name": "Route Path Candidate Authored Copper Graph Zone Obstacle Aware Explain Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(
        route_path_candidate_authored_copper_graph_zone_obstacle_aware_explain_query_cli(
            &root,
            target_net_uuid,
            anchor_a_uuid,
            anchor_b_uuid,
    ...
    assert!(text_output.contains(
        "contract: m5_route_path_candidate_authored_copper_graph_zone_obstacle_aware_explain_v1"
    ));
    ...
    assert!(text_output.contains("blocked_zone_connections: 1"));
