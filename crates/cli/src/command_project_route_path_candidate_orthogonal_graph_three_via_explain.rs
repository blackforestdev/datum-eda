...
use eda_engine::board::{
    ...
    RoutePathCandidateOrthogonalGraphThreeViaExplainReport, RoutePathCandidateStatus,
};
use uuid::Uuid;

use super::super::{build_native_project_board, load_native_project};

pub(crate) fn query_native_project_route_path_candidate_orthogonal_graph_three_via_explain(
    root: &Path,
    net_uuid: Uuid,
    from_anchor_pad_uuid: Uuid,
    to_anchor_pad_uuid: Uuid,
) -> Result<RoutePathCandidateOrthogonalGraphThreeViaExplainReport> {
    let project = load_native_project(root)?;
    let board = build_native_project_board(&project)?;
    board
        .route_path_candidate_orthogonal_graph_three_via_explain(
            net_uuid,
...
pub(crate) fn render_native_project_route_path_candidate_orthogonal_graph_three_via_explain_text(
    report: &RoutePathCandidateOrthogonalGraphThreeViaExplainReport,
) -> String {
    let mut lines = vec![
    ...
    if let Some(selected) = &report.selected_triple {
...
fn render_kind(kind: &RoutePathCandidateOrthogonalGraphThreeViaExplainKind) -> &'static str {
    match kind {
