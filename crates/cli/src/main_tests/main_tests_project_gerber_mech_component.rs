...
#[test]
fn project_mechanical_layer_component_circles_export_validate_and_compare() {
    let root = unique_project_root("datum-eda-cli-project-gerber-mech-component-circle");
    create_native_project(&root, Some("Gerber Mech Component Circle Demo".to_string()))
        .expect("initial scaffold should succeed");
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Gerber Mech Component Circle Demo Board",
                "stackup": {
                    "layers": [
                        {"id": 41, "name": "Mechanical 1", "layer_type": "Mechanical", "thickness_nm": 0}
                    ]
    ...
    let gerber_path = root.join("mech-component-circle.gbr");
    let export_cli = Cli::try_parse_from([
        "eda",
        "--format",
        "json",
    ...
    assert_eq!(compare_report["matched_count"], 24);
    assert_eq!(compare_report["missing_count"], 0);
    assert_eq!(compare_report["extra_count"], 0);
