...
#[test]
fn project_validate_gerber_paste_layer_reports_match_and_mismatch() {
    let root = unique_project_root("datum-eda-cli-project-gerber-paste-validate");
    create_native_project(&root, Some("Gerber Paste Validate Demo".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Gerber Paste Validate Demo Board",
                "stackup": {
                    "layers": [
    ...
    let _ = execute(export_cli).expect("gerber paste export should succeed");
