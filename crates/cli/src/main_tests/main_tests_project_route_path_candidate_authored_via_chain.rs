...
fn route_path_candidate_authored_via_chain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
...
fn project_query_route_path_candidate_authored_via_chain_reports_deterministic_authored_via_chain_path()
 {
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                ...
                "name": "Route Path Candidate Authored Via Chain Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(route_path_candidate_authored_via_chain_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert!(text_output.contains("contract: m5_route_path_candidate_authored_via_chain_v1"));
    assert!(text_output.contains("status: deterministic_path_found"));
    assert!(text_output.contains("path_via_chain_uuids:"));
