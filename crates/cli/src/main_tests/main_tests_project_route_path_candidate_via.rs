...
fn route_path_candidate_via_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
...
#[test]
fn project_query_route_path_candidate_via_reports_deterministic_authored_via_path() {
    let root = unique_project_root("datum-eda-cli-project-route-path-candidate-via");
    create_native_project(&root, Some("Route Path Candidate Via Demo".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                ...
                "name": "Route Path Candidate Via Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(route_path_candidate_via_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert_eq!(report["contract"], "m5_route_path_candidate_via_v1");
    ...
    assert_eq!(report["path"]["via_uuid"], via_uuid.to_string());
    assert_eq!(report["path"]["segments"].as_array().unwrap().len(), 2);

    let repeated = execute(route_path_candidate_via_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert!(text_output.contains("contract: m5_route_path_candidate_via_v1"));
    assert!(text_output.contains("status: deterministic_path_found"));
    assert!(text_output.contains("path_via_uuid:"));
    assert!(text_output.contains("path_segments: 2"));
