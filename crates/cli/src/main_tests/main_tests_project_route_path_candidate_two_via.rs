...
fn route_path_candidate_two_via_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
...
#[test]
fn project_query_route_path_candidate_two_via_reports_deterministic_authored_two_via_path() {
    let root = unique_project_root("datum-eda-cli-project-route-path-candidate-two-via");
    create_native_project(&root, Some("Route Path Candidate Two Via Demo".to_string()))
        .expect("initial scaffold should succeed");

    let target_net_uuid = Uuid::from_u128(0xe10);
    let class_uuid = Uuid::from_u128(0xe11);
    let package_a_uuid = Uuid::from_u128(0xe12);
    let package_b_uuid = Uuid::from_u128(0xe13);
    let anchor_a_uuid = Uuid::from_u128(0xe14);
    let anchor_b_uuid = Uuid::from_u128(0xe15);
    let via_a_uuid = Uuid::from_u128(0xe16);
    let via_b_uuid = Uuid::from_u128(0xe17);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::from_u128(0xe18),
                "name": "Route Path Candidate Two Via Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(route_path_candidate_two_via_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert_eq!(report["contract"], "m5_route_path_candidate_two_via_v1");
    ...
    assert_eq!(report["path"]["intermediate_layer"], 3);
    assert_eq!(report["path"]["segments"].as_array().unwrap().len(), 3);

    let repeated = execute(route_path_candidate_two_via_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert!(text_output.contains("contract: m5_route_path_candidate_two_via_v1"));
    ...
    assert!(text_output.contains("path_segments: 3"));
