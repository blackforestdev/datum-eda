...
#[test]
fn project_apply_forward_annotation_artifact_applies_self_sufficient_value_and_remove_actions() {
    ...
    assert_eq!(apply["applied_actions"], 2);
    assert_eq!(apply["skipped_deferred_actions"], 0);
    assert_eq!(apply["skipped_rejected_actions"], 0);
    ...
    assert_eq!(apply["applied"][1]["proposal_action"], "remove_component");
    assert_eq!(apply["applied"][1]["component_report"]["reference"], "U1");
...
#[test]
fn project_apply_forward_annotation_artifact_rejects_legacy_executable_without_proposal() {
    let root =
        unique_project_root("datum-eda-cli-project-forward-annotation-apply-artifact-legacy");
    create_native_project(
        &root,
        Some("Forward Annotation Artifact Apply Legacy Demo".to_string()),
    )
    .expect("initial scaffold should succeed");
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Forward Annotation Artifact Apply Legacy Demo Board",
                "stackup": { "layers": [] },
                "outline": { "vertices": [], "closed": true },
                "packages": {
                    r1_component_uuid.to_string(): serde_json::to_value(PlacedPackage {
    ...
    let mut artifact_json: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(&filtered_artifact_path).expect("artifact should read"),
    )
    ...
    std::fs::write(
        ...
        format!(
            "{}\n",
            to_json_deterministic(&artifact_json).expect("artifact should serialize")
        ),
    )
    ...
    .expect("CLI should parse");
    let err = execute(apply_cli).expect_err("legacy artifact apply should fail");
    let msg = format!("{err:#}");
    assert!(
        msg.contains("forward-annotation artifact apply requires an embedded substrate proposal")
    );
