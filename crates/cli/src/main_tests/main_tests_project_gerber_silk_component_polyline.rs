...
#[test]
fn project_gerber_silkscreen_layer_supports_component_polylines() {
    let root = unique_project_root("datum-eda-cli-project-gerber-silk-component-polyline");
    create_native_project(&root, Some("Gerber Silk Polyline Demo".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Gerber Silk Polyline Demo Board",
                "stackup": {
                    "layers": [
