...
use eda_engine::board::{RoutePathCandidateFiveViaReport, RoutePathCandidateStatus};
use uuid::Uuid;

use super::super::{build_native_project_board, load_native_project};

pub(crate) fn query_native_project_route_path_candidate_five_via(
    root: &Path,
    net_uuid: Uuid,
    from_anchor_pad_uuid: Uuid,
    to_anchor_pad_uuid: Uuid,
) -> Result<RoutePathCandidateFiveViaReport> {
...
pub(crate) fn render_native_project_route_path_candidate_five_via_text(
...
fn render_status(report: &RoutePathCandidateFiveViaReport) -> &'static str {
    match report.status {
