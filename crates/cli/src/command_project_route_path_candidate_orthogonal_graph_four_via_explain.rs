...
use eda_engine::board::{
    ...
    RoutePathCandidateOrthogonalGraphFourViaExplainReport, RoutePathCandidateStatus,
};
use uuid::Uuid;

use super::super::{build_native_project_board, load_native_project};

pub(crate) fn query_native_project_route_path_candidate_orthogonal_graph_four_via_explain(
    root: &Path,
    net_uuid: Uuid,
    from_anchor_pad_uuid: Uuid,
    to_anchor_pad_uuid: Uuid,
) -> Result<RoutePathCandidateOrthogonalGraphFourViaExplainReport> {
    let project = load_native_project(root)?;
    let board = build_native_project_board(&project)?;
    board
        .route_path_candidate_orthogonal_graph_four_via_explain(
            net_uuid,
...
pub(crate) fn render_native_project_route_path_candidate_orthogonal_graph_four_via_explain_text(
    report: &RoutePathCandidateOrthogonalGraphFourViaExplainReport,
) -> String {
    let mut lines = vec![
    ...
    if let Some(selected) = &report.selected_quadruple {
...
fn render_kind(kind: &RoutePathCandidateOrthogonalGraphFourViaExplainKind) -> &'static str {
    match kind {
