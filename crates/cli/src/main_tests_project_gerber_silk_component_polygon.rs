...
#[test]
fn project_gerber_silkscreen_layer_supports_component_polygons() {
    let root = unique_project_root("datum-eda-cli-project-gerber-silk-component-polygon");
    create_native_project(&root, Some("Gerber Silk Polygon Demo".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Gerber Silk Polygon Demo Board",
                "stackup": {
                    "layers": [
    ...
    assert_eq!(export_report["component_arc_count"], 0);
    assert_eq!(export_report["component_circle_count"], 0);
    ...
    assert!(gerber.contains("X2500000Y5000000D01*"));
    assert!(gerber.contains("X3000000Y4000000D01*"));
