...
use eda_engine::board::{
    RoutePathCandidateStatus, RoutePathCandidateThreeViaExplainKind,
    RoutePathCandidateThreeViaExplainReport,
};
use uuid::Uuid;

use super::super::{build_native_project_board, load_native_project};

pub(crate) fn query_native_project_route_path_candidate_three_via_explain(
    root: &Path,
    net_uuid: Uuid,
    from_anchor_pad_uuid: Uuid,
    to_anchor_pad_uuid: Uuid,
) -> Result<RoutePathCandidateThreeViaExplainReport> {
    let project = load_native_project(root)?;
    let board = build_native_project_board(&project)?;
    board
        .route_path_candidate_three_via_explain(net_uuid, from_anchor_pad_uuid, to_anchor_pad_uuid)
        .map_err(|err| anyhow!(err))
}

pub(crate) fn render_native_project_route_path_candidate_three_via_explain_text(
    report: &RoutePathCandidateThreeViaExplainReport,
) -> String {
    let mut lines = vec![
    ...
    if let Some(triple) = &report.selected_triple {
        lines.push(format!("selected_via_a_uuid: {}", triple.via_a_uuid));
        lines.push(format!("selected_via_b_uuid: {}", triple.via_b_uuid));
        lines.push(format!("selected_via_c_uuid: {}", triple.via_c_uuid));
        lines.push(format!(
            "selected_first_intermediate_layer: {}",
            triple.first_intermediate_layer
        ));
        lines.push(format!(
            "selected_second_intermediate_layer: {}",
            triple.second_intermediate_layer
        ));
        lines.push(format!("selection_reason: {}", triple.selection_reason));
    } else {
    ...
    lines.push(format!(
        "blocked_matching_triple_count: {}",
        report.blocked_matching_triples.len()
    ));
    lines.join("\n")
}

fn render_status(report: &RoutePathCandidateThreeViaExplainReport) -> &'static str {
    match report.status {
...
fn render_kind(report: &RoutePathCandidateThreeViaExplainReport) -> &'static str {
    match report.explanation_kind {
        ...
        RoutePathCandidateThreeViaExplainKind::AllMatchingViaTriplesBlocked => {
            "all_matching_via_triples_blocked"
        }
    }
}
