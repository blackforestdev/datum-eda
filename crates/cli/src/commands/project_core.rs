...
pub(crate) fn build_native_project_schematic(project: &LoadedNativeProject) -> Result<Schematic> {
...
pub(crate) fn summarize_native_schematic_checks(
...
pub(crate) fn summarize_native_board_checks(
    diagnostics: &[ConnectivityDiagnosticInfo],
) -> CheckSummary {
    let mut by_code: BTreeMap<String, usize> = BTreeMap::new();
    let mut errors = 0usize;
