// commands/project/query.rs — `project query`: the resolver-backed native
// project query surface (ProjectQueryArgs::run) and its route path-candidate
// query helpers (absorbed from the dissolved command_exec_project_query.rs
// and command_exec_project_query_route_graph.rs).

use crate::args::NativeRoutePathCandidateAuthoredCopperGraphPolicy;
use crate::*;
use anyhow::anyhow;

impl ProjectQueryArgs {
    pub(crate) fn run(self, format: &OutputFormat) -> Result<(String, i32)> {
        let Self { path, what } = self;
        match what {
            ...
            NativeProjectQueryCommands::Erc => {
                let report = query_native_project_check_run_with_profile(&path, Some("erc"))?;
                Ok((render_output(format, &report), 0))
            }
            NativeProjectQueryCommands::Drc => {
                let report = query_native_project_check_run_with_profile(&path, Some("drc"))?;
                Ok((render_output(format, &report), 0))
            }
...
fn execute_native_project_route_path_candidate_authored_copper_graph_query(
    format: &OutputFormat,
    path: &Path,
    net: Uuid,
    from_anchor: Uuid,
...
fn execute_native_project_route_path_candidate_authored_copper_plus_one_gap_query(
    format: &OutputFormat,
    path: &Path,
    net: Uuid,
    from_anchor: Uuid,
...
fn execute_native_project_route_path_candidate_authored_copper_plus_one_gap_explain_query(
    format: &OutputFormat,
    path: &Path,
    net: Uuid,
    from_anchor: Uuid,
...
fn execute_native_project_route_path_candidate_authored_copper_graph_explain_query(
    format: &OutputFormat,
    path: &Path,
    net: Uuid,
    from_anchor: Uuid,
