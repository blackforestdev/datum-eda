...
pub(crate) fn apply_native_project_forward_annotation_action(
    ...
    if substrate::can_apply_with_embedded_proposal(std::slice::from_ref(&action)) {
        return substrate::apply_forward_annotation_proposal(
            root,
            substrate::build_forward_annotation_proposal(
                root,
...
pub(crate) fn apply_native_project_forward_annotation_reviewed(
    ...
    let proposal = query_native_project_forward_annotation_proposal(root)?;
    let review = review_state::load_forward_annotation_review(root)?;
    ...
    let applied = if applied.is_empty() {
        ...
        let proposal = substrate::build_forward_annotation_proposal(root, &applied, &[])?
            .ok_or_else(|| {
                anyhow::anyhow!("forward-annotation reviewed substrate proposal could not be built")
            })?;
        substrate::apply_forward_annotation_proposal(root, proposal, &applied)?
    };
...
pub(crate) fn record_native_project_forward_annotation_review(
    ...
    let mut review = review_state::load_forward_annotation_review(root)?;
    review.insert(
    ...
    review_state::write_forward_annotation_review(root, &review)?;
