...
pub(crate) fn export_native_project_forward_annotation_proposal(
    ...
    let artifact = ForwardAnnotationProposalArtifact {
        ...
        proposal: substrate::build_forward_annotation_proposal(root, &actions, &reviews)?,
        actions,
        reviews,
    };
...
pub(crate) fn filter_forward_annotation_proposal_artifact(
    ...
    let reviews = loaded
        ...
        .collect::<Vec<_>>();
    let embedded_proposal = substrate::build_forward_annotation_proposal(root, &actions, &reviews)?;
...
pub(crate) fn apply_forward_annotation_proposal_artifact(
    ...
    if !executable_actions.is_empty()
        && (loaded.artifact.proposal.is_none()
            || !substrate::can_apply_with_embedded_proposal(&executable_actions))
    {
        bail!(
    ...
    let applied = if executable_actions.is_empty() {
        ...
        substrate::apply_forward_annotation_proposal(root, proposal, &executable_actions)?
    };
