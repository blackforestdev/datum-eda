use super::*;
use crate::query_native_project_drc_with_rules;
...
pub(crate) fn query_native_project_check_with_inputs(
    ...
    let diagnostics = if include_relationships {
        schematic_diagnostics(&schematic)
    } else {
        Default::default()
    };
    let erc = if include_erc {
        run_prechecks(&schematic)
    } else {
        Default::default()
    };
