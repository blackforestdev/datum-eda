...
use eda_engine::board::{
    ...
    RoutePathCandidateAuthoredCopperPlusOneGapExplainReport, RoutePathCandidateStatus,
};
use uuid::Uuid;

use super::super::{build_native_project_board, load_native_project};

pub(crate) fn query_native_project_route_path_candidate_authored_copper_plus_one_gap_explain(
    root: &Path,
    net_uuid: Uuid,
    from_anchor_pad_uuid: Uuid,
    to_anchor_pad_uuid: Uuid,
) -> Result<RoutePathCandidateAuthoredCopperPlusOneGapExplainReport> {
...
pub(crate) fn render_native_project_route_path_candidate_authored_copper_plus_one_gap_explain_text(
    ...
    if let Some(path) = &report.selected_path {
        lines.push(format!("selected_path_steps: {}", path.path.steps.len()));
...
fn render_status(report: &RoutePathCandidateAuthoredCopperPlusOneGapExplainReport) -> &'static str {
    match report.status {
