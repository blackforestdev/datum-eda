...
fn route_path_candidate_three_via_explain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
    ...
    Cli::try_parse_from([
        ...
        "query",
        root.to_str().unwrap(),
        "route-path-candidate-three-via-explain",
        "--net",
        &net_uuid.to_string(),
        "--from-anchor",
...
#[test]
fn project_query_route_path_candidate_three_via_explain_reports_selected_triple_reasoning() {
    let root = unique_project_root("datum-eda-cli-project-route-path-candidate-three-via-explain");
    create_native_project(
        &root,
        Some("Route Path Candidate Three Via Explain Demo".to_string()),
    )
    .expect("initial scaffold should succeed");

    let target_net_uuid = Uuid::from_u128(0x1030);
    let class_uuid = Uuid::from_u128(0x1031);
    let package_a_uuid = Uuid::from_u128(0x1032);
    let package_b_uuid = Uuid::from_u128(0x1033);
    let anchor_a_uuid = Uuid::from_u128(0x1034);
    let anchor_b_uuid = Uuid::from_u128(0x1035);
    let via_a_uuid = Uuid::from_u128(0x1036);
    let via_b_uuid = Uuid::from_u128(0x1037);
    let via_c_uuid = Uuid::from_u128(0x1038);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::from_u128(0x1039),
                "name": "Route Path Candidate Three Via Explain Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(route_path_candidate_three_via_explain_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert_eq!(report["contract"], "m5_route_path_candidate_three_via_explain_v1");
    ...
    let repeated = execute(route_path_candidate_three_via_explain_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert!(text_output.contains("contract: m5_route_path_candidate_three_via_explain_v1"));
