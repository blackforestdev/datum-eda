...
fn project_board_component_materializes_supported_pool_graphics_into_persisted_board_state() {
    ...
    .expect("board should parse");
    assert_eq!(
        board["component_silkscreen"][&component_uuid].as_array().unwrap().len(),
        1
    );
    assert_eq!(
    ...
    std::fs::write(
        pool_root.join("packages").join(format!("{}.json", package_uuid)),
        "{}\n",
    )
...
fn project_board_component_materialization_prefers_lower_priority_pool_package_match() {
    ...
    create_native_project(&root, Some("Board Component Pool Priority Demo".to_string()))
        .expect("initial scaffold should succeed");
    configure_native_project_for_pool_materialization(
        &root,
        serde_json::json!([
    ...
    write_pool_package(
        ...
        &Package {
            ...
            pads: HashMap::new(),
            courtyard: Polygon { vertices: vec![], closed: true },
            silkscreen: vec![Primitive::Line {
                from: Point { x: 0, y: 0 },
                to: Point { x: 1_000_000, y: 0 },
                width: 100_000,
    ...
    write_pool_package(
        ...
        &Package {
            ...
            silkscreen: vec![Primitive::Circle {
                center: Point { x: 300_000, y: 400_000 },
                radius: 200_000,
                width: 80_000,
            }],
            models_3d: Vec::new(),
...
fn project_board_component_materialization_supports_absolute_pool_paths() {
    ...
    create_native_project(&root, Some("Board Component Pool Absolute Demo".to_string()))
        .expect("initial scaffold should succeed");
...
#[test]
fn project_board_component_materialization_keeps_silkscreen_validate_and_compare_pool_independent() {
