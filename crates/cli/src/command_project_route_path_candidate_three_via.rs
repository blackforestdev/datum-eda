...
use eda_engine::board::{RoutePathCandidateStatus, RoutePathCandidateThreeViaReport};
use uuid::Uuid;

use super::super::{build_native_project_board, load_native_project};

pub(crate) fn query_native_project_route_path_candidate_three_via(
    root: &Path,
    net_uuid: Uuid,
    from_anchor_pad_uuid: Uuid,
    to_anchor_pad_uuid: Uuid,
) -> Result<RoutePathCandidateThreeViaReport> {
...
pub(crate) fn render_native_project_route_path_candidate_three_via_text(
...
fn render_route_path_candidate_three_via_status(
    report: &RoutePathCandidateThreeViaReport,
) -> &'static str {
    match report.status {
