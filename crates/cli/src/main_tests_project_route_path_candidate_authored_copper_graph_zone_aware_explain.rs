...
fn route_path_candidate_authored_copper_graph_zone_aware_explain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
    ...
    Cli::try_parse_from([
        ...
        "query",
        root.to_str().unwrap(),
        "route-path-candidate-authored-copper-graph-zone-aware-explain",
        "--net",
        &net_uuid.to_string(),
        "--from-anchor",
...
fn project_query_route_path_candidate_authored_copper_graph_zone_aware_explain_reports_selected_path()
{
    ...
    create_native_project(
        &root,
        Some("Route Path Candidate Authored Copper Graph Zone Aware Explain Demo".to_string()),
    )
    .expect("initial scaffold should succeed");

    let target_net_uuid = Uuid::from_u128(0x3550);
    let class_uuid = Uuid::from_u128(0x3551);
    let package_a_uuid = Uuid::from_u128(0x3552);
    let package_b_uuid = Uuid::from_u128(0x3553);
    let anchor_a_uuid = Uuid::from_u128(0x3554);
    let anchor_b_uuid = Uuid::from_u128(0x3555);
    let zone_uuid = Uuid::from_u128(0x3556);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::from_u128(0x3557),
                "name": "Route Path Candidate Authored Copper Graph Zone Aware Explain Demo Board",
                "stackup": {
                    "layers": [
