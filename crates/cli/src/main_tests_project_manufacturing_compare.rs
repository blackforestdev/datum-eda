...
fn write_manufacturing_compare_board(root: &Path) {
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Manufacturing Compare Demo Board",
                "stackup": {
                    "layers": [
...
#[test]
fn project_compare_manufacturing_set_passes_for_clean_export() {
    let root = unique_project_root("datum-eda-cli-project-manufacturing-compare-pass");
    create_native_project(&root, Some("Manufacturing Compare Demo".to_string()))
        .expect("initial scaffold should succeed");
    write_manufacturing_compare_board(&root);
    ...
    assert_eq!(report["action"], "compare_manufacturing_set");
...
#[test]
fn project_compare_manufacturing_set_reports_missing_mismatched_and_extra_files() {
    let root = unique_project_root("datum-eda-cli-project-manufacturing-compare-drift");
    create_native_project(&root, Some("Manufacturing Compare Demo".to_string()))
        .expect("initial scaffold should succeed");
    write_manufacturing_compare_board(&root);
    ...
    std::fs::remove_file(output_dir.join("release-a-drill.drl")).expect("drill should remove");
    std::fs::write(
        output_dir.join("release-a-l3-top-silk-silk.gbr"),
        "G04 drifted silk*\nM02*\n",
    )
    .expect("gerber should rewrite");
...
#[test]
fn project_compare_manufacturing_set_uses_resolver_materialized_board_state() {
    let root = unique_project_root("datum-eda-cli-project-manufacturing-compare-resolved");
    create_native_project(
        &root,
        Some("Manufacturing Compare Resolved Demo".to_string()),
    )
    .expect("initial scaffold should succeed");
    let board_json = root.join("board/board.json");
    ...
    .expect("comparison should run from resolver state");
    let report: serde_json::Value = serde_json::from_str(&output).expect("report JSON");
    assert_eq!(exit_code, 0);
    assert_eq!(report["action"], "compare_manufacturing_set");
    ...
    assert_eq!(
        report["prefix"],
        "manufacturing-compare-resolved-demo-board"
    );
