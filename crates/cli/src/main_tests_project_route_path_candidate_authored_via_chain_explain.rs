...
fn route_path_candidate_authored_via_chain_explain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
    ...
    Cli::try_parse_from([
        ...
        "query",
        root.to_str().unwrap(),
        "route-path-candidate-authored-via-chain-explain",
        "--net",
        &net_uuid.to_string(),
        "--from-anchor",
...
#[test]
fn project_query_route_path_candidate_authored_via_chain_explain_reports_selected_chain() {
    let root = unique_project_root(
        "datum-eda-cli-project-route-path-candidate-authored-via-chain-explain",
    );
    create_native_project(
        &root,
        Some("Route Path Candidate Authored Via Chain Explain Demo".to_string()),
    )
    .expect("initial scaffold should succeed");

    let target_net_uuid = Uuid::from_u128(0x3150);
    let class_uuid = Uuid::from_u128(0x3151);
    let package_a_uuid = Uuid::from_u128(0x3152);
    let package_b_uuid = Uuid::from_u128(0x3153);
    let anchor_a_uuid = Uuid::from_u128(0x3154);
    let anchor_b_uuid = Uuid::from_u128(0x3155);
    let via_a_uuid = Uuid::from_u128(0x3156);
    let via_b_uuid = Uuid::from_u128(0x3157);
    let via_c_uuid = Uuid::from_u128(0x3158);
    let via_d_uuid = Uuid::from_u128(0x3159);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::from_u128(0x315a),
                "name": "Route Path Candidate Authored Via Chain Explain Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(route_path_candidate_authored_via_chain_explain_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert_eq!(
        report["contract"],
        "m5_route_path_candidate_authored_via_chain_explain_v1"
    );
