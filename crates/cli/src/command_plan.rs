...
// Phase 5: the `plan` sub-enum router, absorbed from the dissolved
// command_exec_plan.rs (PlanCommands variants carry inline fields, not args
// structs, so this stays a family fn rather than per-variant run() impls).
pub(crate) fn execute_plan_command(
    format: &OutputFormat,
    action: PlanCommands,
) -> Result<(String, i32)> {
    match action {
