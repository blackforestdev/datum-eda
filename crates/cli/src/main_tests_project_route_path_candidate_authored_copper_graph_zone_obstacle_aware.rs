...
fn route_path_candidate_authored_copper_graph_zone_obstacle_aware_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
...
#[test]
fn project_query_route_path_candidate_authored_copper_graph_zone_obstacle_aware_reports_zone_path()
{
    let root = unique_project_root(
        "datum-eda-cli-project-route-path-candidate-authored-copper-graph-zone-obstacle-aware",
    );
    create_native_project(
        &root,
        Some("Route Path Candidate Authored Copper Graph Zone Obstacle Aware Demo".to_string()),
    )
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                ...
                "name": "Route Path Candidate Authored Copper Graph Zone Obstacle Aware Demo Board",
                "stackup": {
                    "layers": [
    ...
    assert!(text_output.contains("blocked_zone_connections: 0"));
