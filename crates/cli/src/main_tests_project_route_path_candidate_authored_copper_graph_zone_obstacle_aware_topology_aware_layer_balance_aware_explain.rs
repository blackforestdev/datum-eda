...
fn layer_balance_aware_explain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
...
fn project_query_route_path_candidate_authored_copper_graph_zone_obstacle_aware_topology_aware_layer_balance_aware_explain_is_deterministic() {
    ...
    create_native_project(&root, Some("Layer Balance Aware Explain".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Layer Balance Aware Explain Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(layer_balance_aware_explain_query_cli(
        &root,
        net_uuid,
        from_anchor_uuid,
        to_anchor_uuid,
