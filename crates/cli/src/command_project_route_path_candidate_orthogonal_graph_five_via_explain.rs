...
use eda_engine::board::{
    ...
    RoutePathCandidateOrthogonalGraphFiveViaExplainReport, RoutePathCandidateStatus,
};
use uuid::Uuid;
...
pub(crate) fn render_native_project_route_path_candidate_orthogonal_graph_five_via_explain_text(
    report: &RoutePathCandidateOrthogonalGraphFiveViaExplainReport,
) -> String {
    let mut lines = vec![
    ...
    if let Some(selected) = &report.selected_quintuple {
...
fn render_kind(kind: &RoutePathCandidateOrthogonalGraphFiveViaExplainKind) -> &'static str {
    match kind {
