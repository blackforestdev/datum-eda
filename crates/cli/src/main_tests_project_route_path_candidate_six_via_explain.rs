...
fn route_path_candidate_six_via_explain_query_cli(
    root: &Path,
    net_uuid: Uuid,
    from_anchor: Uuid,
    to_anchor: Uuid,
    ...
    Cli::try_parse_from([
        ...
        "query",
        root.to_str().unwrap(),
        "route-path-candidate-six-via-explain",
        "--net",
        &net_uuid.to_string(),
        "--from-anchor",
...
#[test]
fn project_query_route_path_candidate_six_via_explain_reports_selected_sextuple_reasoning() {
    let root = unique_project_root("datum-eda-cli-project-route-path-candidate-six-via-explain");
    create_native_project(
        &root,
        Some("Route Path Candidate Six Via Explain Demo".to_string()),
    )
    .expect("initial scaffold should succeed");

    let target_net_uuid = Uuid::from_u128(0x1080);
    let class_uuid = Uuid::from_u128(0x1081);
    let package_a_uuid = Uuid::from_u128(0x1082);
    let package_b_uuid = Uuid::from_u128(0x1083);
    let anchor_a_uuid = Uuid::from_u128(0x1084);
    let anchor_b_uuid = Uuid::from_u128(0x1085);
    let via_a_uuid = Uuid::from_u128(0x1086);
    let via_b_uuid = Uuid::from_u128(0x1087);
    let via_c_uuid = Uuid::from_u128(0x1088);
    let via_d_uuid = Uuid::from_u128(0x1089);
    let via_e_uuid = Uuid::from_u128(0x108a);
    let via_f_uuid = Uuid::from_u128(0x108b);
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::from_u128(0x108c),
                "name": "Route Path Candidate Six Via Explain Demo Board",
                "stackup": {
                    "layers": [
    ...
    let output = execute(route_path_candidate_six_via_explain_query_cli(
        &root,
        target_net_uuid,
        anchor_a_uuid,
        anchor_b_uuid,
    ...
    assert!(text_output.contains("contract: m5_route_path_candidate_six_via_explain_v1"));
    ...
    assert!(text_output.contains("selected_via_f_uuid:"));
    assert!(text_output.contains("blocked_matching_sextuple_count: 0"));
