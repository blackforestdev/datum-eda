...
#[test]
fn project_gerber_silkscreen_layer_supports_component_circles() {
    let root = unique_project_root("datum-eda-cli-project-gerber-silk-component-circle");
    create_native_project(&root, Some("Gerber Silk Circle Demo".to_string()))
    ...
    std::fs::write(
        ...
        format!(
            ...
            to_json_deterministic(&serde_json::json!({
                "schema_version": 1,
                "uuid": Uuid::new_v4(),
                "name": "Gerber Silk Circle Demo Board",
                "stackup": {
                    "layers": [
    ...
    assert!(gerber.contains("X4000000Y4000000D02*"));
    assert!(gerber.contains("X3965926Y4258819D01*"));
