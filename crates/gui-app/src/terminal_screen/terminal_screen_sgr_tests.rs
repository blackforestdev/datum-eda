...
fn sgr_foreground_color_is_retained_as_terminal_spans() {
    ...
    assert_eq!(state.grid_lines(), vec!["red plain"]);
    assert_eq!(state.grid_styled_lines()[0].text, "red plain");
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 1);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 0);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 3);
    assert_eq!(
        state.grid_styled_lines()[0].spans[0].fg.as_deref(),
        Some("red")
    );
    assert!(!state.grid_styled_lines()[0].spans[0].bold);
}
...
fn sgr_bold_and_foreground_share_one_span_until_reset() {
    ...
    assert_eq!(state.grid_lines(), vec!["ERR ok"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 1);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 0);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 3);
    assert_eq!(
...
fn sgr_reset_prevents_style_from_leaking_to_later_cells() {
    ...
    assert_eq!(state.grid_lines(), vec!["pre ok neutral bold done"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 2);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 4);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 6);
    assert_eq!(
        state.grid_styled_lines()[0].spans[0].fg.as_deref(),
        Some("green")
    );
    assert!(!state.grid_styled_lines()[0].spans[0].bold);
    assert_eq!(state.grid_styled_lines()[0].spans[1].start, 15);
    assert_eq!(state.grid_styled_lines()[0].spans[1].end, 19);
    assert_eq!(state.grid_styled_lines()[0].spans[1].fg, None);
    assert!(state.grid_styled_lines()[0].spans[1].bold);
}
...
fn sgr_background_color_is_retained_as_terminal_span_metadata() {
    ...
    assert_eq!(state.grid_lines(), vec!["blue-bg plain"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 1);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 0);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 7);
    assert_eq!(
        state.grid_styled_lines()[0].spans[0].bg.as_deref(),
        Some("blue")
    );
    assert_eq!(state.grid_styled_lines()[0].spans[0].fg, None);
}
...
fn sgr_extended_ansi256_colors_are_retained_as_terminal_span_metadata() {
    ...
    assert_eq!(state.grid_lines(), vec!["fgbg"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 2);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 0);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 2);
    assert_eq!(
        state.grid_styled_lines()[0].spans[0].fg.as_deref(),
        Some("ansi256:196")
    );
    assert_eq!(state.grid_styled_lines()[0].spans[1].start, 2);
    assert_eq!(state.grid_styled_lines()[0].spans[1].end, 4);
    assert_eq!(
        state.grid_styled_lines()[0].spans[1].fg.as_deref(),
        Some("ansi256:196")
    );
    assert_eq!(
        state.grid_styled_lines()[0].spans[1].bg.as_deref(),
        Some("ansi256:22")
    );
}
...
fn sgr_extended_truecolor_values_are_retained_as_terminal_span_metadata() {
    ...
    assert_eq!(state.grid_lines(), vec!["rgb"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 1);
    assert_eq!(
        state.grid_styled_lines()[0].spans[0].fg.as_deref(),
        Some("rgb:12:34:56")
...
fn malformed_extended_sgr_color_does_not_clear_existing_style() {
    ...
    assert_eq!(state.grid_lines(), vec!["red still-red"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 1);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 0);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 13);
    assert_eq!(
        state.grid_styled_lines()[0].spans[0].fg.as_deref(),
        Some("red")
    );
...
fn sgr_inverse_state_is_retained_until_reset() {
    ...
    assert_eq!(state.grid_lines(), vec!["pre inv fg"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 2);
    assert_eq!(state.grid_styled_lines()[0].spans[0].start, 4);
    assert_eq!(state.grid_styled_lines()[0].spans[0].end, 7);
    assert_eq!(
    ...
    assert!(state.grid_styled_lines()[0].spans[0].inverse);
    assert_eq!(state.grid_styled_lines()[0].spans[1].start, 7);
    assert_eq!(state.grid_styled_lines()[0].spans[1].end, 10);
    assert_eq!(
        state.grid_styled_lines()[0].spans[1].fg.as_deref(),
        Some("red")
    );
    ...
    assert!(!state.grid_styled_lines()[0].spans[1].inverse);
}
...
fn sgr_underline_italic_and_strikethrough_are_retained_until_reset() {
    ...
    assert_eq!(state.grid_lines(), vec!["decor no-italic no-under plain"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 3);

    let decorated = &state.grid_styled_lines()[0].spans[0];
    ...
    let no_italic = &state.grid_styled_lines()[0].spans[1];
    ...
    let no_under = &state.grid_styled_lines()[0].spans[2];
...
fn sgr_dim_and_conceal_are_retained_until_matching_reset() {
    ...
    assert_eq!(state.grid_lines(), vec!["secret plain-dim-reset visible"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 2);

    let secret = &state.grid_styled_lines()[0].spans[0];
    ...
    let concealed = &state.grid_styled_lines()[0].spans[1];
...
fn sgr_blink_is_retained_until_reset() {
    ...
    assert_eq!(state.grid_lines(), vec!["blink plain rapid"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 2);

    let blink = &state.grid_styled_lines()[0].spans[0];
    assert_eq!(blink.start, 0);
    assert_eq!(blink.end, 5);
    assert!(blink.blink);

    let rapid = &state.grid_styled_lines()[0].spans[1];
...
fn sgr_overline_is_retained_until_reset() {
    ...
    assert_eq!(state.grid_lines(), vec!["over plain"]);
    assert_eq!(state.grid_styled_lines()[0].spans.len(), 1);

    let over = &state.grid_styled_lines()[0].spans[0];
