...
fn native_dialog_scroll_release_cannot_click_through_to_controls() {
    ...
    for index in 0..3 {
        ...
        let window = Arc::new(
            events
                .create_window(
                    Window::default_attributes()
                        .with_visible(false)
                        .with_inner_size(winit::dpi::PhysicalSize::new(960, 480)),
                )
                .unwrap(),
        );
        let mut owned =
            GlobalPreferencesWindowSurface::new(runtime, window.clone(), Some(1.0)).unwrap();
        ...
        let thumb = owned
            .scroll
            .thumb()
            .expect("real dialog must overflow test extent");
        let rect = owned
            .presented_hits
            .regions()
            .iter()
        ...
        let focused = |app: &App| {
            ...
            match index {
                0 => ui.global_preferences.open_choice_key.as_deref() == Some("datum.units.system"),
                1 => {
                    ui.project_preferences.open_choice_key.as_deref() == Some("datum.units.system")
                }
        ...
        assert!(
            app.handle_owned_dialog_pointer(host(index), &motion(thumb.x + 1.0, thumb.y + 4.0))
        );
        assert!(app.handle_owned_dialog_pointer(host(index), &primary(ElementState::Pressed)));
        ...
        eprintln!(
            "host={index} scrollbar release suppressed={} fresh control click=true",
