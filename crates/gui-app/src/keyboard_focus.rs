...
pub(crate) fn handle_keyboard_input(app: &mut App, event: &KeyEvent) -> bool {
    ...
    let terminal_chrome_owns_escape = app.runtime.as_ref().is_some_and(|runtime| {
        let terminal = &runtime.workspace().ui.terminal;
        terminal.search.active
            || terminal.search.escape_release_pending
