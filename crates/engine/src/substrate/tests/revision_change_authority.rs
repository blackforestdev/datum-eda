...
fn managed_snapshot(
    ...
    let records = vec![
        AuthorityRecord::ConfigurationBaseline(body(
            baseline_id,
            project_id,
            vec![AuthorityRef::ConfigurationItem(item_id)],
            ConfigurationBaselineData {
                human_number: None,
                baseline_type: "working-predecessor".to_string(),
                scope: vec![AuthorityRef::ConfigurationItem(item_id)],
                members: vec![BaselineMember {
                    stable_authority_ref: AuthorityRef::ConfigurationItem(item_id),
                    exact_technical_revision: "0".to_string(),
                    semantic_digest: AlgorithmQualifiedDigest(format!("sha256:{}", "0".repeat(64))),
                    target_ref: None,
                    display_name: None,
                    role: "design".to_string(),
                    inclusion_reason: "REV-I04 predecessor fixture".to_string(),
                }],
                source_model_revision: "fixture-model".to_string(),
                accepted_transaction_tip: "fixture-tip".to_string(),
                governing_changes: Vec::new(),
                departures: Vec::new(),
                profile_refs: Vec::new(),
                establishment_attestations: Vec::new(),
                established_by: AuthorityRef::ConfigurationItem(item_id),
                established_at: 1,
