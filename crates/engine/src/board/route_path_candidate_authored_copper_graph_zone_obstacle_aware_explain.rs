...
use crate::board::{
    Board, RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareReport, RoutePathCandidateError,
...
pub enum RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainKind {
    DeterministicPathFound,
    NoExistingAuthoredCopperPath,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainStep {
    pub kind:
        super::route_path_candidate_authored_copper_graph_zone_obstacle_aware::RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareStepKindView,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainSelectedPath {
    pub steps: Vec<RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainStep>,
    pub selection_reason: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainSummary {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainReport {
    pub contract: String,
    pub persisted_native_board_state_only: bool,
    pub status: RoutePathCandidateStatus,
    pub explanation_kind: RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainKind,
    pub net_uuid: Uuid,
    pub net_name: String,
    ...
    pub summary: RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainSummary,
    pub selected_path:
        Option<RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainSelectedPath>,
}

impl Board {
    pub fn route_path_candidate_authored_copper_graph_zone_obstacle_aware_explain(
        &self,
        net_uuid: Uuid,
        from_anchor_pad_uuid: Uuid,
        to_anchor_pad_uuid: Uuid,
    ) -> Result<RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainReport, RoutePathCandidateError>
    {
        let path_candidate = self.route_path_candidate_authored_copper_graph_zone_obstacle_aware(
            net_uuid,
        ...
        let selected_path = path_candidate.path.as_ref().map(|path| {
            RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainSelectedPath {
                steps: path
                    .steps
                    .iter()
                    .map(|step| RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainStep {
                ...
                selection_reason: format!(
                    "selected because it is the first minimum-step existing authored-copper path under the deterministic sorted graph traversal rule after excluding persisted target-net track/via/zone edges blocked by current authored obstacle checks, with {} step(s)",
                    path.steps.len()
                ),
            }
        });

        Ok(RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainReport {
            contract:
            ...
            summary: RoutePathCandidateAuthoredCopperGraphZoneObstacleAwareExplainSummary {
                ...
                blocked_zone_connection_count: path_candidate.summary.blocked_zone_connection_count,
