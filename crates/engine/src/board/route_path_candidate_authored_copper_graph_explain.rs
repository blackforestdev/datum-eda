...
use crate::board::{
    Board, RoutePathCandidateAuthoredCopperGraphReport, RoutePathCandidateError,
...
pub enum RoutePathCandidateAuthoredCopperGraphExplainKind {
    DeterministicPathFound,
    NoExistingAuthoredCopperPath,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphExplainStep {
    pub kind: super::route_path_candidate_authored_copper_graph::RoutePathCandidateAuthoredCopperGraphStepKindView,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphExplainSelectedPath {
    pub steps: Vec<RoutePathCandidateAuthoredCopperGraphExplainStep>,
    pub selection_reason: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphExplainSummary {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateAuthoredCopperGraphExplainReport {
    pub contract: String,
    pub persisted_native_board_state_only: bool,
    pub status: RoutePathCandidateStatus,
    pub explanation_kind: RoutePathCandidateAuthoredCopperGraphExplainKind,
    pub net_uuid: Uuid,
    pub net_name: String,
    ...
    pub summary: RoutePathCandidateAuthoredCopperGraphExplainSummary,
    pub selected_path: Option<RoutePathCandidateAuthoredCopperGraphExplainSelectedPath>,
}

impl Board {
    pub fn route_path_candidate_authored_copper_graph_explain(
        &self,
        net_uuid: Uuid,
        from_anchor_pad_uuid: Uuid,
        to_anchor_pad_uuid: Uuid,
    ) -> Result<RoutePathCandidateAuthoredCopperGraphExplainReport, RoutePathCandidateError> {
        let path_candidate = self.route_path_candidate_authored_copper_graph(
            net_uuid,
        ...
        let selected_path = path_candidate.path.as_ref().map(|path| {
            RoutePathCandidateAuthoredCopperGraphExplainSelectedPath {
                steps: path
                    .steps
                    .iter()
                    .map(|step| RoutePathCandidateAuthoredCopperGraphExplainStep {
                ...
                selection_reason: format!(
                    "selected because it is the first minimum-step existing authored-copper path under the deterministic sorted graph traversal rule with {} step(s)",
                    path.steps.len()
                ),
            }
        });

        Ok(RoutePathCandidateAuthoredCopperGraphExplainReport {
            contract: "m5_route_path_candidate_authored_copper_graph_explain_v1".to_string(),
            ...
            summary: RoutePathCandidateAuthoredCopperGraphExplainSummary {
