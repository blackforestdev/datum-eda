...
pub const ROUTE_PATH_CANDIDATE_FIVE_VIA_SELECTION_RULE: &str = "select the first unblocked matching authored via quintuple in ascending (via_a_uuid, via_b_uuid, via_c_uuid, via_d_uuid, via_e_uuid) order whose layer sequence connects the requested anchor layers through four intermediate copper layers";

#[derive(Debug, Clone)]
pub(super) struct RoutePathCandidateFiveViaMatch {
    pub via_a: Via,
    pub via_b: Via,
    pub via_c: Via,
    pub via_d: Via,
            ...
            for (via_c_index, via_c) in candidate_vias.iter().enumerate() {
                ...
                for (via_d_index, via_d) in candidate_vias.iter().enumerate() {
                    ...
                    for (via_e_index, via_e) in candidate_vias.iter().enumerate() {
                        ...
                        if let Some(entry) = matching_five_via_quintuple(
                            board,
                            net_uuid,
                            from_anchor,
                            to_anchor,
                            ...
                            matching_quintuples.push(entry);
                        }
                    }
                }
            }
...
fn matching_five_via_quintuple(
    ...
    Some(RoutePathCandidateFiveViaMatch {
        via_a: via_a.clone(),
        via_b: via_b.clone(),
        via_c: via_c.clone(),
        via_d: via_d.clone(),
        ...
        target_segment: analyze_route_segment(
            ...
            to_anchor.position,
            &format!(
                "via-e-to-target segment via {} on layer {}",
                via_e.uuid, to_anchor.layer
            ),
        ),
    })
}

fn intermediate_layers_for_quintuple(
    via_a: &Via,
    via_b: &Via,
    via_c: &Via,
    via_d: &Via,
    ...
    from_layer: LayerId,
    to_layer: LayerId,
) -> Option<(LayerId, LayerId, LayerId, LayerId)> {
    ...
    let via_e_other_layer = other_boundary_layer(via_e, to_layer)?;
    if via_e_other_layer == fourth_intermediate_layer {
        Some((
            first_intermediate_layer,
            second_intermediate_layer,
            third_intermediate_layer,
...
pub(super) fn selected_matching_five_via(
    matches: &[RoutePathCandidateFiveViaMatch],
) -> Option<&RoutePathCandidateFiveViaMatch> {
...
pub(super) fn five_via_path_points(
    entry: &RoutePathCandidateFiveViaMatch,
    from_anchor: &RoutePreflightAnchor,
    to_anchor: &RoutePreflightAnchor,
) -> [[Point; 2]; 6] {
    [
        ...
        [entry.via_e.position, to_anchor.position],
    ]
}
