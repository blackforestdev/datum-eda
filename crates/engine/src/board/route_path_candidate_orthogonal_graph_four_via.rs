...
const ROUTE_PATH_CANDIDATE_ORTHOGONAL_GRAPH_FOUR_VIA_SELECTION_RULE: &str = "select the first matching authored via quadruple in ascending (via_a_uuid, via_b_uuid, via_c_uuid, via_d_uuid) order whose source-layer, first-intermediate-layer, second-intermediate-layer, third-intermediate-layer, and target-layer persisted-coordinate orthogonal graph searches all yield deterministic paths; each side reuses the same deterministic graph-search rule as the same-layer orthogonal graph candidate";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateOrthogonalGraphFourViaSegment {
    pub layer: LayerId,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateOrthogonalGraphFourViaPath {
    pub via_a_uuid: Uuid,
    ...
    pub segments: Vec<RoutePathCandidateOrthogonalGraphFourViaSegment>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateOrthogonalGraphFourViaSummary {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RoutePathCandidateOrthogonalGraphFourViaReport {
    pub contract: String,
    ...
    pub summary: RoutePathCandidateOrthogonalGraphFourViaSummary,
    pub path: Option<RoutePathCandidateOrthogonalGraphFourViaPath>,
}

impl Board {
    pub fn route_path_candidate_orthogonal_graph_four_via(
        &self,
        net_uuid: Uuid,
        from_anchor_pad_uuid: Uuid,
        to_anchor_pad_uuid: Uuid,
    ) -> Result<RoutePathCandidateOrthogonalGraphFourViaReport, RoutePathCandidateError> {
        ...
        let path = quadruple_searches.iter().find_map(
            |(
                ...
                Some(RoutePathCandidateOrthogonalGraphFourViaPath {
        ...
        Ok(RoutePathCandidateOrthogonalGraphFourViaReport {
            contract: "m5_route_path_candidate_orthogonal_graph_four_via_v1".to_string(),
            persisted_native_board_state_only: true,
            selection_rule:
                ROUTE_PATH_CANDIDATE_ORTHOGONAL_GRAPH_FOUR_VIA_SELECTION_RULE.to_string(),
            component_selection_rules: vec![
                ROUTE_PATH_CANDIDATE_FOUR_VIA_SELECTION_RULE.to_string(),
            ...
            summary: RoutePathCandidateOrthogonalGraphFourViaSummary {
                candidate_copper_layer_count: preflight.candidate_copper_layers.len(),
                candidate_via_count: candidate_vias.len(),
                candidate_via_quadruple_count: permutation_count(candidate_vias.len(), 4),
