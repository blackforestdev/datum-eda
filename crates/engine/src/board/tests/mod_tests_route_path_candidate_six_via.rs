...
fn route_path_candidate_six_via_reports_deterministic_path_using_authored_via_sextuple() {
    ...
    assert_eq!(report.path.as_ref().map(|path| path.fifth_intermediate_layer), Some(11));
...
fn route_path_candidate_six_via_selects_next_matching_sextuple_when_earlier_sextuple_is_blocked() {
    let (mut board, net_uuid, _, anchor_top_uuid, anchor_bottom_uuid, first_via_a_uuid, via_b_uuid, via_c_uuid, via_d_uuid, via_e_uuid, via_f_uuid) =
        demo_board();
