...
fn route_path_candidate_orthogonal_graph_three_via_explain_reports_selected_triple() {
    ...
    assert_eq!(
        report.selected_triple.as_ref().map(|path| path.via_a_uuid),
        Some(via_a_uuid)
    );
    assert_eq!(
        report.selected_triple.as_ref().map(|path| path.via_b_uuid),
        Some(via_b_uuid)
    );
    assert_eq!(
        report.selected_triple.as_ref().map(|path| path.via_c_uuid),
        Some(via_c_uuid)
    );
}
