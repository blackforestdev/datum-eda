...
fn parse_pad_non_copper_layers_anywhere(
    block: &str,
    layer_table: &HashMap<String, i32>,
...
fn parse_pad_expansion_setup(contents: &str) -> PadExpansionSetup {
    PadExpansionSetup {
...
fn parse_block_mm_value_anywhere(block: &str, key: &str) -> Option<i64> {
...
fn parse_block_ratio_ppm_anywhere(block: &str, key: &str) -> Option<i32> {
    let needle = format!("({key} ");
    block.lines().find_map(|line| {
        let trimmed = line.trim_start();
        if !trimmed.starts_with(&needle) {
...
fn parse_footprint_mm_value_before_pads(block: &str, key: &str) -> Option<i64> {
    let needle = format!("({key} ");
    for line in block.lines() {
        let trimmed = line.trim_start();
...
fn parse_footprint_ratio_ppm_before_pads(block: &str, key: &str) -> Option<i32> {
    let needle = format!("({key} ");
    for line in block.lines() {
        let trimmed = line.trim_start();
        if trimmed.starts_with("(pad ") {
