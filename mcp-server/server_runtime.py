...
class EngineDaemonClient:
    ...
    def create_pool_package(self, path: str, pool: str, package: str, name: str, pad: str | None = None, padstack: str | None = None, pad_name: str = "1", x_nm: int = 0, y_nm: int = 0, layer: int = 1) -> JsonRpcResponse:
        args = ["project", "create-pool-package", path, "--pool", pool, "--package", package, "--name", name]
        if pad is not None: args.extend(["--pad", pad])
        if padstack is not None: args.extend(["--padstack", padstack])
        if pad is not None or padstack is not None: args.extend(["--pad-name", pad_name, "--x-nm", str(x_nm), "--y-nm", str(y_nm), "--layer", str(layer)])
        return self._run_cli_json(self.build_request("create_pool_package", {"path": path, "pool": pool, "package": package, "name": name, "pad": pad, "padstack": padstack, "pad_name": pad_name, "x_nm": x_nm, "y_nm": y_nm, "layer": layer}), args)
    def create_pool_footprint(self, path: str, pool: str, footprint: str, package: str, name: str) -> JsonRpcResponse: return self._run_cli_json(self.build_request("create_pool_footprint", {"path": path, "pool": pool, "footprint": footprint, "package": package, "name": name}), ["project", "create-pool-footprint", path, "--pool", pool, "--footprint", footprint, "--package", package, "--name", name])
    def set_pool_footprint_pad(self, path: str, pool: str, footprint: str, pad: str, padstack: str, pad_name: str, x_nm: int, y_nm: int, layer: int) -> JsonRpcResponse: return self._run_cli_json(self.build_request("set_pool_footprint_pad", {"path": path, "pool": pool, "footprint": footprint, "pad": pad, "padstack": padstack, "pad_name": pad_name, "x_nm": x_nm, "y_nm": y_nm, "layer": layer}), ["project", "set-pool-footprint-pad", path, "--pool", pool, "--footprint", footprint, "--pad", pad, "--padstack", padstack, "--pad-name", pad_name, "--x-nm", str(x_nm), "--y-nm", str(y_nm), "--layer", str(layer)])
    ...
    def align_board_components(self, path: str, components: list[str], mode: str) -> JsonRpcResponse: args = ["project", "align-board-components", path]; [args.extend(["--component", component]) for component in components]; args.extend(["--mode", mode]); return self._run_cli_json(self.build_request("align_board_components", {"path": path, "components": components, "mode": mode}), args)
...
from server_runtime_daemon_client_specs import _REQUIRED, DAEMON_CLIENT_METHOD_SPECS  # noqa: F401
...
from server_runtime_library import install_library_methods; from server_runtime_preferences import install_preferences_methods; from server_runtime_project_genesis import install_project_genesis_method; from server_runtime_project_query import install_project_query_methods; from server_runtime_proposals import install_proposal_authoring_methods; from server_runtime_schematic_drawing import install_schematic_drawing_methods; from server_runtime_schematic_sheet import install_schematic_sheet_methods; from server_runtime_schematic_symbol import install_schematic_symbol_methods; from server_runtime_units import install_units_methods; install_library_methods(EngineDaemonClient); install_preferences_methods(EngineDaemonClient); install_project_genesis_method(EngineDaemonClient); install_project_query_methods(EngineDaemonClient); install_proposal_authoring_methods(EngineDaemonClient, _append_optional); install_schematic_drawing_methods(EngineDaemonClient, _append_optional); install_schematic_sheet_methods(EngineDaemonClient, _append_optional); install_schematic_symbol_methods(EngineDaemonClient, _append_optional); install_units_methods(EngineDaemonClient, _append_optional)
