# Broader workflow delivery implementation and adoption

Frontier: WORKFLOW-DELIVERY-IMPLEMENTATION.
Issue: `dat-wdq-rollout-implementation-ffy`.
Authority: WDQ-R03 recorded owner approval in WORKFLOW_DELIVERY_ROLLOUT_PLAN.md.
Route: `workflow-delivery-rollout`.
...
<!-- REQ:WORKFLOW-DELIVERY-IMPLEMENTATION:WDQ-I01 -->
## WDQ-I01 — implement coverage and transition refusals

Build the approved closed schema-2 coverage/source-scope validator and its
production selector/staged-CLI integration. Preserve schema-1 compatibility.
Test missing/duplicate/new classifications, omitted contracts, historical
reopening, specification execution, scope escape, stale/missing claims and
unauthorized source changes. Evidence must include real candidate/index paths,
not only mocked helper calls. Keep source modules within their budgets.

Use the approved repository-local isolated candidate workflow for changes to
pinned gate code. Retain exact candidate commits and reviewable patches in Git;
do not weaken live hooks or make another Documents bundle. Candidate code is
not installed enforcement. No product lane may be claimed to satisfy this step.

<!-- REQ:WORKFLOW-DELIVERY-IMPLEMENTATION:WDQ-I02 -->
## WDQ-I02 — implement separate review and enrollment preparation

Implement versioned five-phase mappings, independent replay before review-step
completion and unchanged final owner-receipt/defect requirements. Prepare exact
contract/enrollment candidates and the rollout infrastructure contract. Preserve
the three product cohorts' actual C01 owners and required readiness inputs;
missing handlers, fixtures or mechanisms remain explicit refusals. Do not fill
...
Present the independently verified exact amendment/candidate/base/environment
and adoption-enrollment packet, plus coordinated Preferences migration or its
bounded explicit exception. Obtain owner ratification and exact owner-controlled
activation. Do not interpret R03 as candidate promotion. Verify the actual main
checkout, installed runner/hooks, local trust and blocking checks afterwards;
failed/partial installation stays incomplete. Preserve active external support
