...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Owner explicitly authorized amended S0-S5 execution. Preserve recovered candidate c8bac0b6; S0 measurement conformance and bounded integration review active. Resize investigation remains separately pinned and nonblocking; no runtime acceptance. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
