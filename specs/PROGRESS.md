...
| Item | Status | Notes |
|------|--------|-------|
| Registry crate + generated catalog + drift gate + clap round-trip test | [x] | Landed; registry declares 152 verbs; thirteen prefixes are registry-owned in the MCP catalog with their hand-written dicts deleted (`datum.artifact` 11, `datum.check` 10, `datum.component_instance` 3, `datum.context` 4, `datum.journal` 4, `datum.manufacturing` 6, `datum.output_job` 5, `datum.pool` 3, `datum.project` 3, `datum.proposal` 44, `datum.query` 48, `datum.replacement` 5, `datum.session` 4 — 150 tools), plus 2 further terminal verbs (library) registry-declared; registered tool set unchanged (519) |
| All registered verbs registry-owned; hand-written MCP catalogs deleted | [~] | 13 of the public `datum.*` prefixes migrated via `MIGRATED_PREFIXES` (150 of 332 public tools generated); remaining families (library/pcb/route/schematic) still hand-written; taxonomy/count gates keep their invariants during migration |
