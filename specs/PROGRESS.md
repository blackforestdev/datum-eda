...
- **Complete and production-accept Global Preferences** (`GLOBAL-PREFERENCES-COMPLETION`; `dat-global-preferences-completion-f84`).
   Complete Global Preferences by activating only consumer-ready descriptors, preserving incomplete identities as reserved candidates, and proving truthful controls, timing, accessibility, recovery, and exclusions before later parity and production-acceptance gates. *state `ready`; authorization `execution`; **CANONICAL NEXT**.*
...
- **Repair the owner-directed workflow revision path** (`WORKFLOW-OWNER-REVISION`; `dat-workflow-owner-revision-71j`).
   Admit, claim and finish the owner-directed revision repair with usable preparation, durable commits, independent proof and exact promotion. *state `in_progress`; authorization `planning`.*
   *Dependencies:* none. *Unblocks:* none. *Governing:* `specs/WORKFLOW_OWNER_REVISION_REPAIR_PLAN.md`, `research/process-quality/WORKFLOW_OWNER_REVISION_ADMISSION.md`, `docs/reviews/workflow-owner-revision/admission-owner-20260916.json`, `specs/WORKFLOW_OWNER_REVISION_CONTRACT.md`.
<!-- ACTIVE FRONTIER:END -->
