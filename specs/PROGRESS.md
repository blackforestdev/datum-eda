...
- **Enforce pinned context and scoped agent authority** (`TERMINAL-T4C-CONTEXT-AUTHORITY`; `dat-terminal-emulator-epic-jzv.10`).
   Separate live and immutable pinned context, fence mutations by revision, and grant short-lived inspect/propose/apply-approved authority with complete audit provenance. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
