...
- **Build the Global Preferences foundation and first GUI slice** (`GLOBAL-PREFERENCES-ENGINE`; `dat-global-preferences-engine-build-vge`).
   ...
   *Dependencies:* `dat-global-preferences-engine-qcv`, `dat-shared-units-engine-build-915`. *Unblocks:* dat-shared-units-surface-parity-s9x, dat-global-preferences-completion-f84. *Governing:* `docs/decisions/PRODUCT_MECHANICS_039_PREFERENCES_FIRST_SETTINGS_ENTRY.md`, `docs/decisions/PRODUCT_MECHANICS_037_GLOBAL_PREFERENCES_ENGINE.md`, `specs/GLOBAL_PREFERENCES_V1_DESCRIPTOR_CATALOG.md`, `research/preferences-system/GP_C04_STORAGE_MIGRATION_EXCHANGE_RECOVERY_CONTRACT.md`, `research/preferences-system/GP_C05_INTERACTION_AND_VISUAL_CONTRACT.md`, `specs/GLOBAL_PREFERENCES_ENGINE_IMPLEMENTATION_PLAN.md`, `docs/decisions/PRODUCT_MECHANICS_029_DEPENDENCY_AUTHORITY.md`, `docs/gui/prototypes/preferences-window.html`.
- **Complete shared Units surface parity through Preferences** (`SHARED-UNITS-SURFACE-PARITY`; `dat-shared-units-surface-parity-s9x`).
   Reconcile the shared Units requirement around eight typed Global defaults, Project Working Units, per-quantity precision, lossless editing, exact scientific notation, decimal-degree V1 behavior, deterministic migration, and separate Publish/document authority; obtain approved Global and Project targets before staged execution and owner acceptance. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
...
- **Complete and production-accept Global Preferences** (`GLOBAL-PREFERENCES-COMPLETION`; `dat-global-preferences-completion-f84`).
   Expand the accepted Global Preferences vertical slice through the complete active catalog, managed/recovery behavior, accessibility, API and seed parity, real Projects, and production acceptance while Revision descriptors remain absent. *state `blocked`; authorization `owner_decision`.*
