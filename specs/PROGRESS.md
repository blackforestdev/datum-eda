...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Owner-approved bounded S0 readiness complete. S1 shared default scheduling/lifecycle implementation awaits readiness disposition under the approved S1 exit amendment; unfinished SH/LF/affected-HP qualification remains explicitly required at S5. GEFN behavior preserved; resize investigation nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
