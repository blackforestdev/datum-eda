...
- **Complete shared Units surface parity through Preferences** (`SHARED-UNITS-SURFACE-PARITY`; `dat-shared-units-surface-parity-s9x`).
   After the first real Global Preferences wgpu slice exists, prove the exact Units core across the six ratified unit descriptors, GUI, CLI, MCP, import/export, and real Projects without inventing a temporary settings surface. *state `specified`; authorization `owner_decision`; **CANONICAL NEXT**.*
