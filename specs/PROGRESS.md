...
- **Restore foreign-shell screen authority and visible-output proof** (`TERMINAL-T0-SHELL-TRUTH`; `dat-pan-trace-terminal-pollution-0j0`).
   ...
   *Dependencies:* `dat-full-native-terminal-contract-e2d`, `dat-terminal-emulator-epic-jzv.7`. *Unblocks:* dat-terminal-emulator-epic-jzv.2, dat-terminal-emulator-epic-jzv.3. *Governing:* `docs/decisions/PRODUCT_MECHANICS_027_FULL_NATIVE_TERMINAL.md`, `docs/decisions/PRODUCT_MECHANICS_029_DEPENDENCY_AUTHORITY.md`, `docs/gui/DATUM_NATIVE_TERMINAL_SPEC.md`.
...
- **Build the Datum-owned TerminalCore** (`TERMINAL-T1-CORE`; `dat-terminal-emulator-epic-jzv.2`).
   Implement Datum-owned VT parsing, state, cells, modes, replies, damage, history, reflow, selection, search, and protocols without third-party terminal code. *state `planned`; authorization `owner_decision`; **CANONICAL NEXT**.*
