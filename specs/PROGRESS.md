...
- **Build the Global Preferences foundation and first GUI slice** (`GLOBAL-PREFERENCES-ENGINE`; `dat-global-preferences-engine-build-vge`).
   Implement the typed resolver and durable repository foundation, then deliver the first functional Global Preferences wgpu slice through the conventional application-menu entry using real registered and persisted values. *state `blocked`; authorization `owner_decision`.*
...
- **Complete shared Units surface parity through Preferences** (`SHARED-UNITS-SURFACE-PARITY`; `dat-shared-units-surface-parity-s9x`).
   After the first real Global Preferences wgpu slice exists, prove the exact Units core across the six ratified unit descriptors, GUI, CLI, MCP, import/export, and real Projects without inventing a temporary settings surface. *state `blocked`; authorization `owner_decision`.*
   *Dependencies:* `dat-global-preferences-engine-build-vge`. *Unblocks:* dat-global-preferences-completion-f84. *Governing:* `docs/decisions/PRODUCT_MECHANICS_039_PREFERENCES_FIRST_SETTINGS_ENTRY.md`, `docs/decisions/PRODUCT_MECHANICS_037_GLOBAL_PREFERENCES_ENGINE.md`, `research/preferences-system/GP_SHARED_UNITS_ENGINE_REQUIREMENT.md`, `specs/GLOBAL_PREFERENCES_ENGINE_IMPLEMENTATION_PLAN.md`, `docs/gui/prototypes/preferences-window.html`, `docs/decisions/PRODUCT_MECHANICS_029_DEPENDENCY_AUTHORITY.md`.
...
- **Recover the Revision product baseline and contain rejected REV-I10** (`REVISION-PRODUCT-RECOVERY`; `dat-revision-product-recovery-dap`).
   Apply the owner-approved manual-first product baseline, supersede permanent unmanaged Revision presentation, quarantine unvalidated implementation breadth, remove fictional runtime records, and require a small comprehensible owner review before any wider Revision execution resumes. *state `specified`; authorization `owner_decision`; **CANONICAL NEXT**.*
