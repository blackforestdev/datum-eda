...
|------|------------------|---------------------------------|
| Canonical pin electrical type | [~] `pool::PinDirection` is now an alias of `LibraryPinElectricalType`; schematic ERC still consumes `schematic::PinElectricalType`; pool-symbol placement maps library pin electrical types into schematic pins for the current placement slice. | One library-owned canonical taxonomy drives schematic materialization and ERC without a parallel schematic authority enum. |
...
| `LibraryGraph` ownership | [~] `eda_engine::pool::LibraryGraph` now owns the first dependency diagnostic seam for pool-family references, and `project validate` projects those engine diagnostics instead of carrying a separate CLI reference validator. Resolver validation, duplicate/shadowing policy, validation tiers, constructors, MCP bridges, and materialization rules are still not a single graph-owned contract. | One engine-owned `LibraryGraph` owns dependency resolution, duplicate/shadowing policy, validation tiers, and materialization rules for CLI/MCP/GUI. |
...
- Canonical MCP `datum.check.explain_violation` is now public and dispatches to
  ...
  guard now locks 308 public tools, 517 registered tools, and 209 hidden
...
| Daemon JSON-RPC dispatch | [x] | 35 methods in `dispatch.rs`, with private writer bypasses retired and coverage in daemon tests |
