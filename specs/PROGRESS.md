...
- **Complete shared Units surface parity through Preferences** (`SHARED-UNITS-SURFACE-PARITY`; `dat-shared-units-surface-parity-s9x`).
   Reconcile the shared Units requirement around eight typed Global defaults, Project Working Units, per-quantity precision, lossless editing, exact scientific notation, decimal-degree V1 behavior, deterministic migration, and separate Publish/document authority; obtain approved Global and Project targets before staged execution and owner acceptance. *state `in_progress`; authorization `planning`; **CANONICAL NEXT**.*
