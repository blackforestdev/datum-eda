...
| Gerber export | [~] | `project export-gerber-outline <dir> --out <path>` now writes a narrow RS-274X Gerber file for the persisted native board outline via `eda_engine::export`, `project validate-gerber-outline <dir> --gerber <path>` now re-renders the expected native outline Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-copper-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for persisted board pads, tracks, vias, and zones on one selected copper layer with deterministic circular/rectangular aperture assignment, flashed pad/via emission, and region emission for zones, `project validate-gerber-copper-layer <dir> --layer <id> --gerber <path>` now re-renders the expected copper-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-soldermask-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for explicit circular/rectangular pad openings on one selected soldermask layer derived from the nearest copper layer in stackup order, `project validate-gerber-soldermask-layer <dir> --layer <id> --gerber <path>` now re-renders that expected soldermask-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-silkscreen-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for authored board text plus persisted explicit component-silkscreen text and line strokes on one selected silkscreen layer, using explicit stored text height/stroke-width plus the current fixed stroke-font renderer for board text and component text, and explicit stored stroke widths for component lines, `project validate-gerber-silkscreen-layer <dir> --layer <id> --gerber <path>` now re-renders that expected silkscreen-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-paste-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for explicit circular/rectangular pad openings on one selected paste layer derived from the nearest copper layer in stackup order, `project validate-gerber-paste-layer <dir> --layer <id> --gerber <path>` now re-renders that expected paste-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-mechanical-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for native board keepout polygons on one selected mechanical layer with closed keepouts emitted as filled regions, `project validate-gerber-mechanical-layer <dir> --layer <id> --gerber <path>` now re-renders that expected mechanical-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, and `project plan-gerber-export <dir> [--prefix <text>]` still reports the broader deterministic planned artifact set from the persisted native board outline and stackup; full layer-set writing, broader pad geometry beyond the current flashed circle/rectangle subset, package silkscreen arc/shape expansion beyond the current explicit component-text-and-line subset, package silkscreen/mask/paste expansion or stencil policy beyond the current narrow slice, broader mechanical documentation geometry, and gerbv validation remain open |
