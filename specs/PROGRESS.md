...
- **Ratify terminal agent discovery and interoperability** (`TERMINAL-AGENT-INTEROP-CONTRACT`; `dat-terminal-emulator-epic-jzv.7`).
   Replace prompt-memory discovery with client-native adapters, standard MCP, pinned context, scoped authority, portable workflows, and production interoperability proof before terminal execution continues. *state `in_progress`; authorization `planning`; **CANONICAL NEXT**.*
