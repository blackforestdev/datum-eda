...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   S0-S4 implementation exits complete under approved sequencing and PM047. S5 selected for full in-scope qualification: preserve every S1 obligation,50 S2 and20 S3 qualification rows, native/backend/scale/resource/endurance and distinct-reviewer requirements. Console static golden failure resolved in 95287f56; full drift still fails separately tracked menu CSV and bootstrap guards. No numerical-cap, full performance or product acceptance; GEFN and nonblocking resize exclusions preserved. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
