...
- **Build the shared Units exact core** (`SHARED-UNITS-ENGINE`; `dat-shared-units-engine-build-915`).
   Implement the engine-owned exact integer-nanometer and typed parse/format core before the first real Global Preferences slice; surface parity and Units production acceptance occur later through that real Preferences surface. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
