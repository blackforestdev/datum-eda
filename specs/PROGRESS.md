...
| Criterion | Status | Notes |
|-----------|--------|-------|
| Native format | [~] | Current M4 CLI slice supports deterministic scaffold creation, scaffold inspection, and the first native read/query surface: `project new <dir> [--name]` writes versioned native `project.json`, `schematic/schematic.json`, `board/board.json`, and `rules/rules.json`; `project inspect <dir>` validates the resolved file layout; `project query <dir> summary`, `project query <dir> design-rules`, `project query <dir> symbols`, `project query <dir> symbol-fields --symbol <uuid>`, `project query <dir> symbol-semantics --symbol <uuid>`, `project query <dir> symbol-pins --symbol <uuid>`, `project query <dir> texts`, `project query <dir> drawings`, `project query <dir> nets`, `project query <dir> diagnostics`, `project query <dir> erc`, `project query <dir> check`, `project query <dir> board-components`, `project query <dir> board-pads`, `project query <dir> board-tracks`, `project query <dir> board-vias`, `project query <dir> board-zones`, `project query <dir> board-diagnostics`, `project query <dir> board-unrouted`, `project query <dir> board-check`, `project query <dir> board-texts`, `project query <dir> board-keepouts`, `project query <dir> board-dimensions`, `project query <dir> board-outline`, `project query <dir> board-stackup`, `project query <dir> board-nets`, and `project query <dir> board-net-classes` read the native scaffold directly and report aggregated native schematic/board/rule/check state; focused CLI tests prove idempotent rewrite, inspect-path correctness, referenced-sheet summary counts, native rules payload reporting, native symbol/text/drawing persistence-query behavior, native connectivity query behavior via an in-memory `Schematic` bridge over the authored sheet files, native ERC precheck behavior on that same bridge, native combined schematic check reporting in the existing `CheckReport::Schematic` shape, and native board-component full first-family lifecycle/query plus engine-backed native board diagnostics/unrouted/check parity and board pad edit/net-assignment and board track/via/zone/text/keepout/dimension/outline/stackup/net/net-class persistence-query behavior through `board/board.json` |
...
**M4 overall**: [~] Native-project persistence now has deterministic scaffold creation, minimal native inspect/query/check surfaces including native connectivity nets/diagnostics/ERC/check and engine-backed board diagnostics/unrouted/check parity, focused authored schematic coverage for symbols, text objects, drawing primitives, labels, wires, junctions, hierarchical ports, buses/bus entries, and no-connect markers, and the first native board-authored low-risk families plus initial component placement, pad lifecycle, and copper primitives; most schematic ops, most board authoring ops, ECO, and manufacturing exports remain open
