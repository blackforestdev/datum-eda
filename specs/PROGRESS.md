...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Owner authorized amended S0-S5 execution; recovered candidate c8bac0b6 and GEFN QA behavior preserved. S0 GPU pass-measurement unit verified; overhead, admission/accounting and matched baseline remain open. Resize investigation is separately pinned and nonblocking; no runtime acceptance. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
