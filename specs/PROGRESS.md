...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Owner approved component-scoped measurement sequencing: bounded S0 readiness, affected proof with S1-S4, full in-scope qualification at S5. Recovered candidate c8bac0b6 and GEFN behavior preserved; GPU measurement unit verified. S0 readiness remains open; no runtime acceptance. Resize investigation stays nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
