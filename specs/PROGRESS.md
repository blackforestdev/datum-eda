...
- **Build agent launcher and discovery adapters** (`TERMINAL-T4A-AGENT-LAUNCH`; `dat-terminal-emulator-epic-jzv.8`).
   Implement native discovery adapters and datum-eda agent list/doctor/launch with protected per-session configuration for Codex, Claude Code, Cursor-compatible CLI, and generic local agents. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
