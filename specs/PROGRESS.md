...
- **Make canonical task completion plans deterministic** (`STATE-TASK-DETAILS`; `dat-task-completion-alignment-4jp`).
   Add validated structured completion plans and one deterministic task-details query so agents cannot omit controlling requirements or infer post-completion ordering. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
