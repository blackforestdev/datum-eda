...
| Item | Status | Notes |
|------|--------|-------|
| Registry crate + generated catalog + drift gate + clap round-trip test | [x] | Landed; registry declares 117 verbs; ten prefixes are registry-owned in the MCP catalog with their hand-written dicts deleted (`datum.artifact` 11, `datum.check` 10, `datum.component_instance` 3, `datum.context` 4, `datum.journal` 4, `datum.manufacturing` 6, `datum.output_job` 5, `datum.project` 3, `datum.query` 45, `datum.session` 4 — 95 tools), plus 22 further terminal verbs (library/proposal) registry-declared; registered tool set unchanged (519) |
| All registered verbs registry-owned; hand-written MCP catalogs deleted | [~] | 10 of the public `datum.*` prefixes migrated via `MIGRATED_PREFIXES` (95 of 332 public tools generated); remaining families (pcb/schematic/library/proposal/…) still hand-written; taxonomy/count gates keep their invariants during migration |
