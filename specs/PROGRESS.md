...
- **Specify the performance-first shared GUI foundation** (`GUI-PERFORMANCE-SPEC`; `dat-gui-performance-spec-u9n`).
   Owner reopened shared rendering-engine scope after unacceptable resize CPU and blinking. GPS-C02 must define the common surface lifecycle across main and every dialog; GPS-C03 adds temporal/absolute-resource proof and GPS-C04 requires global adoption. Prior bounded corrections remain partial evidence, not resize acceptance. *state `planned`; authorization `planning`; **CANONICAL NEXT**.*
   *Dependencies:* none. *Unblocks:* dat-gui-performance-implementation-vkq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `research/gui-performance/SHARED_GUI_PERFORMANCE_AUDIT.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`, `docs/reviews/gui-performance/zoom-noop-slice.md`, `docs/reviews/gui-performance/resize-slice.md`, `docs/reviews/gui-performance/rendering-reopening.json`.
