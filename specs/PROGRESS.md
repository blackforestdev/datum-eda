...
semantic unit, symbol, entity, padstack, package, package-pad, and
package-courtyard producers. They dispatch to
...
`pin_pad_maps[*].part` envelope. Mapping keys must resolve to pins on the
part entity's gate units, and mapped values must resolve to pads on the part
package.
