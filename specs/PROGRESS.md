...
- **Restore foreign-shell screen authority and visible-output proof** (`TERMINAL-T0-SHELL-TRUTH`; `dat-pan-trace-terminal-pollution-0j0`).
   Make PTY-derived state the sole terminal-cell authority and prove real typed shell output through the production display path. *state `ready`; authorization `execution`; **CANONICAL NEXT**.*
