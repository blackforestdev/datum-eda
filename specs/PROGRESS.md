...
- **Specify the performance-first shared GUI foundation** (`GUI-PERFORMANCE-SPEC`; `dat-gui-performance-spec-u9n`).
   Owner-directed GPS-C02 through GPS-C06 specification development is active. GPS-C03 uses explicitly limited small-project qualification; larger-project and temporal/display-latency acceptance remain excluded. Resize recovery stays unresolved and pending after the specification decision. *state `in_progress`; authorization `planning`; **CANONICAL NEXT**.*
...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Shared-engine implementation reservation includes R39-R42/E10 surface lifecycle, resize resource/temporal correctness and adoption by every native host; isolated CPU improvements do not satisfy it. Wider execution still requires approved specification/numbered decision and the existing owner execution boundary; bounded corrections retain their exact separate scope. *state `blocked`; authorization `none`.*
   *Dependencies:* `dat-gui-performance-spec-u9n`. *Unblocks:* dat-uvt-s5a-build-1wv, dat-gui-p2-cross-probe-27z, dat-gui-p2-full-inspector-0ye, dat-marking-menu-shell-g0w, dat-gui-write-path-qiu, dat-project-preferences-build-tlq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`.
...
| `specs/GUI_PERFORMANCE_ACCEPTANCE_MATRIX.md` | GPS-C03 draft: owner limits initial qualification to pinned small-project scope; larger-project and temporal/display-latency acceptance excluded; remaining in-scope methods/counts/budgets open | Complete justified numerical matrix and validated feasible methods before C03 closure |
