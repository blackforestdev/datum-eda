...
- **Complete shared Units surface parity through Preferences** (`SHARED-UNITS-SURFACE-PARITY`; `dat-shared-units-surface-parity-s9x`).
   Reconcile the twelve-clause shared Units requirement and seven proof families into one exact cross-surface implementation contract and approved Preferences target before any UNIT-I03 runtime work or production acceptance. *state `in_progress`; authorization `planning`; **CANONICAL NEXT**.*
