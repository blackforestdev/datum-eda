...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Owner-approved bounded S0 readiness complete: exact fresh release baseline and four Xwayland host extent checks; fresh Wayland smoke remains unqualified. S1 shared redraw ownership migration in progress. GEFN behavior preserved; full component accounting and S5 qualification remain open; resize investigation nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
