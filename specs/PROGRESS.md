...
- **Harden the Datum-owned PTY transport** (`TERMINAL-T1-PTY`; `dat-terminal-emulator-epic-jzv.1`).
   Implement and prove the bounded Datum-owned Linux PTY/session transport through DTC-P03..P06 with no third-party terminal dependency. *state `specified`; authorization `owner_decision`; **CANONICAL NEXT**.*
