...
| Gerber export | [~] | `project export-gerber-outline <dir> --out <path>` now writes a narrow RS-274X Gerber file for the persisted native board outline via `eda_engine::export`, `project validate-gerber-outline <dir> --gerber <path>` now re-renders the expected native outline Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-copper-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for persisted board pads, tracks, vias, and zones on one selected copper layer with deterministic circular aperture assignment, flashed pad/via emission, and region emission for zones, `project validate-gerber-copper-layer <dir> --layer <id> --gerber <path>` now re-renders the expected copper-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, and `project plan-gerber-export <dir> [--prefix <text>]` still reports the broader deterministic planned artifact set from the persisted native board outline and stackup; full layer-set writing, non-circular pad geometry, pads/masks/silkscreen beyond the current flashed-circle subset, and gerbv validation remain open |
...
| Gerber comparison | [~] | `project compare-gerber-export-plan <dir> --output-dir <path> [--prefix <text>]` now compares the deterministic planned Gerber artifact set against a directory and reports matched, missing, and extra files, `project compare-gerber-outline <dir> --gerber <path>` now parses the currently emitted outline RS-274X subset and compares it semantically against the native board outline geometry, and `project compare-gerber-copper-layer <dir> --layer <id> --gerber <path>` now parses the currently emitted copper-layer RS-274X subset and compares circular flashed pads, tracks, vias, and zones semantically against the native layer geometry with deterministic matched/missing/extra reporting; broader RS-274X feature coverage, non-circular pad geometry, full layer-set comparison, and viewer-level validation remain open |

**M4 overall**: [~] Native-project persistence now has deterministic scaffold creation, minimal native inspect/query/check surfaces including native connectivity nets/diagnostics/ERC/check and engine-backed board diagnostics/unrouted/check parity, focused authored schematic coverage for symbols, text objects, drawing primitives, labels, wires, junctions, hierarchical ports, buses/bus entries, and no-connect markers, the first native board-authored low-risk families plus initial component placement, pad lifecycle with explicit circular copper diameter, and copper primitives, forward-annotation audit/proposal/export/live-selection-export/artifact-selection/inspect/compare/filter/plan/review-import/review-replace plus keyed single-action apply, a first filtered-artifact apply slice, and a first review-aware batch apply slice, and a materially broader native manufacturing slice: deterministic BOM/PnP export plus semantic compare against live board inventory, deterministic CSV via-drill export plus narrow real Excellon write/inspect/validate/compare and explicit drill hole-class reporting, narrow real RS-274X board-outline and copper-layer (circular flashed pads, tracks, vias, and zones) writers through `eda_engine::export`, deterministic Gerber artifact planning from board outline plus stackup, deterministic Gerber plan-vs-directory comparison, and semantic Gerber comparison for the currently emitted outline and single-layer copper subset; most schematic ops, most board authoring ops, broader ECO apply/reject flow, broader layer-set/geometry-level manufacturing output, and full Gerber feature coverage remain open
