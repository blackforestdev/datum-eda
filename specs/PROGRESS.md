...
- **Build the Global Preferences foundation and first GUI slice** (`GLOBAL-PREFERENCES-ENGINE`; `dat-global-preferences-engine-build-vge`).
   Implement the typed resolver and durable repository foundation, then deliver the first functional Global Preferences wgpu slice through the conventional application-menu entry using real registered and persisted values. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
