...
| Board operations | [~] | Native board-authored low-risk families are landed directly on `board/board.json`: board texts (`project place-board-text <dir> --text <text> --x-nm <i64> --y-nm <i64> [--rotation-deg <i32>] --layer <i32>`, `project edit-board-text <dir> --text <uuid> [--value <text>] [--x-nm <i64> --y-nm <i64>] [--rotation-deg <i32>] [--layer <i32>]`, `project delete-board-text <dir> --text <uuid>`), board keepouts (`project place-board-keepout <dir> --kind <text> --layer <i32>... --vertex <x:y>...`, `project edit-board-keepout <dir> --keepout <uuid> [--kind <text>] [--layer <i32>...] [--vertex <x:y>...]`, `project delete-board-keepout <dir> --keepout <uuid>`), board dimensions (`project place-board-dimension <dir> --from-x-nm <i64> --from-y-nm <i64> --to-x-nm <i64> --to-y-nm <i64> [--text <text>]`, `project edit-board-dimension <dir> --dimension <uuid> [--from-x-nm <i64> --from-y-nm <i64>] [--to-x-nm <i64> --to-y-nm <i64>] [--text <text>] [--clear-text]`, `project delete-board-dimension <dir> --dimension <uuid>`), direct board outline replacement (`project set-board-outline <dir> --vertex <x:y>...`), and full stackup replacement (`project set-board-stackup <dir> --layer <id:name:type:thickness_nm>...`); `project query <dir> board-texts`, `project query <dir> board-keepouts`, `project query <dir> board-dimensions`, `project query <dir> board-outline`, and `project query <dir> board-stackup` verify persisted board state directly from `board/board.json`; routed copper, packages/components, zones, and other board authoring remain open |
...
**M4 overall**: [~] Native-project persistence now has deterministic scaffold creation, minimal native inspect/query/check surfaces including native connectivity nets/diagnostics/ERC/check, focused authored schematic coverage for symbols, text objects, drawing primitives, labels, wires, junctions, hierarchical ports, buses/bus entries, and no-connect markers, and the first native board-authored low-risk families via board texts, keepouts, dimensions, direct outline replacement, and stackup replacement; most schematic ops, most board authoring ops, ECO, and manufacturing exports remain open
