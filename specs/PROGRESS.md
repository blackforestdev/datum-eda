...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   S0 and amended S1/S2 bounded readiness complete. Owner approved exact ccf42618 S3 bounded-exit amendment: 20 unfinished S3 qualification rows remain required at S5, with original acceptance rules and partial statuses. S3 remains active pending evidence-based readiness review; missing default adoption and unexplained implementation regressions still block it. Fifty S2 predicates retain S4 implementation/S5 qualification. GEFN preserved; resize nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
...
| `specs/GUI_PERFORMANCE_IMPLEMENTATION_CONTRACT.md` | S0 and amended S1/S2 bounded readiness complete; S3 active. Exact ccf42618 bounded-exit amendment approved and applied; 20 unfinished qualification rows required at S5 | Evidence-based S3 readiness, then S4 implementation and S5 qualification; no partial row marked passed |
