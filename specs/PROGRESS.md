...
- **Specify the performance-first shared GUI foundation** (`GUI-PERFORMANCE-SPEC`; `dat-gui-performance-spec-u9n`).
   Owner demands resize defect remain active until resolved. Reopened GPS-C01R covers native flicker/resource diagnosis, corrected harness, shared lifecycle corrections and verified low-resource temporal UX; prior CPU improvement is partial evidence. GPS-C02 and unrelated work do not displace this defect. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
   *Dependencies:* none. *Unblocks:* dat-gui-performance-implementation-vkq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `research/gui-performance/SHARED_GUI_PERFORMANCE_AUDIT.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`, `docs/reviews/gui-performance/zoom-noop-slice.md`, `docs/reviews/gui-performance/resize-slice.md`, `docs/reviews/gui-performance/rendering-reopening.json`, `docs/reviews/gui-performance/resize-recovery.md`.
