...
| Gerber export | [~] | `project export-gerber-outline <dir> --out <path>` now writes a narrow RS-274X Gerber file for the persisted native board outline via `eda_engine::export`, `project validate-gerber-outline <dir> --gerber <path>` now re-renders the expected native outline Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-copper-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for persisted board tracks on one selected copper layer with deterministic aperture assignment by track width, and `project plan-gerber-export <dir> [--prefix <text>]` still reports the broader deterministic planned artifact set from the persisted native board outline and stackup; full layer-set writing, pads/zones/masks/silkscreen, and gerbv validation remain open |
| Drill export | [~] | `project export-drill <dir> --out <path>` now writes a deterministic CSV from the persisted native via inventory in `board/board.json`; Excellon formatting, slot/hole classes, tool tables, and richer fabrication contract details remain open |
| BOM export | [~] | `project export-bom <dir> --out <path>` now writes a deterministic CSV from the persisted native board-component inventory in `board/board.json`; pool-derived manufacturer fields, schematic-only items, variants, and richer purchasing metadata remain open |
| PnP export | [~] | `project export-pnp <dir> --out <path>` now writes a deterministic CSV from the persisted native board-component inventory in `board/board.json`; richer feeder/origin/variant/manufacturer contract details remain open |
