...
- **Complete and production-accept Global Preferences** (`GLOBAL-PREFERENCES-COMPLETION`; `dat-global-preferences-completion-f84`).
   Expand the accepted Global Preferences vertical slice through the complete active catalog, managed/recovery behavior, accessibility, API and seed parity, real Projects, and production acceptance while Revision descriptors remain absent. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
