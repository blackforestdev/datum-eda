...
| `docs/IPC_FOOTPRINT_SYSTEM.md` | pool/import footprint tests + engine IPC generator tests + CLI/proposal IPC generation tests | First engine-owned IPC-7351B two-terminal chip generator, structured basis, journaled CLI generation, proposal generation, and LibraryGraph IPC basis diagnostics landed; MCP surfaces, SOIC/SOT families, check-run findings, governed mask/paste-in-export, deviations, and import audit remain unbuilt/ungated |
...
2. **IPC-7351B footprint generator / structured basis** (010, IPC doc):
   first engine two-terminal chip generator, CLI/proposal authoring surfaces,
   and LibraryGraph validation diagnostics landed; MCP surfaces, check/export
   consumption, deviations, and import audit remain open.
...
| Honest remainder | [~] | MCP direct tools for a few proposal-gated families lack proposal twins (`component_instance` bind/set/delete unreachable under `assistant` provenance) — follow-up surface work. Public verb-registry migration complete (17 public `datum.*` prefixes generated; 332/332 public tools). |
...
| Item | Status | Notes |
|------|--------|-------|
| Registry crate + generated catalog + drift gate + clap round-trip test | [x] | Landed; registry declares all 332 public verbs; seventeen public prefixes are registry-owned in the MCP catalog with their hand-written dicts deleted or filtered (`datum.artifact` 11, `datum.check` 10, `datum.component_instance` 3, `datum.context` 4, `datum.journal` 4, `datum.library` 54, `datum.manufacturing` 6, `datum.output_job` 5, `datum.pcb` 45, `datum.pool` 3, `datum.project` 3, `datum.proposal` 44, `datum.query` 48, `datum.replacement` 5, `datum.route` 21, `datum.schematic` 62, `datum.session` 4 — 332 tools); public tool set unchanged (332) |
| All public `datum.*` verbs registry-owned; hand-written MCP public catalog entries deleted | [x] | 17 public `datum.*` prefixes migrated via `MIGRATED_PREFIXES` (332 of 332 public tools generated); hidden compatibility tools remain fenced outside the generated public loader; taxonomy/count gates keep their invariants |
