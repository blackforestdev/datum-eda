...
| ScopedReplacementPlanManifest | [~] | CLI current slice supports versioned scoped replacement manifest export plus manifest-backed apply with recorded board/library provenance and drift checks via `plan export-scoped-replacement-manifest` and `--apply-scoped-replacement-manifest` |
