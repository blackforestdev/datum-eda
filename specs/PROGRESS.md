...
- **Build and production-accept the shared Units engine** (`SHARED-UNITS-ENGINE`; `dat-shared-units-engine-build-915`).
   Implement one cross-cutting engine-owned exact Units service for integer-nanometer storage truth and shared typed format/parse behavior before Global Preferences execution, with every slice separately authorized. *state `blocked`; authorization `owner_decision`.*
...
- **Enforce Claude ownership of GUI visual-truth files** (`VISUAL-TRUTH-FILE-LANE`; `dat-visual-truth-file-lane-enforcement-jfl`).
   ...
   *Dependencies:* none. *Unblocks:* dat-global-preferences-engine-qcv. *Governing:* `research/process-quality/VISUAL_TRUTH_FILE_LANE_ENFORCEMENT_PACKET.md`, `docs/PROJECT_STATE_POLICY.md`.
<!-- ACTIVE FRONTIER:END -->
