...
- **Specify the performance-first shared GUI foundation** (`GUI-PERFORMANCE-SPEC`; `dat-gui-performance-spec-u9n`).
   GPS-C01-C06 specification milestones complete; owner ratified exact packet and PM045 for specification only. GPS-C01R remains pending and unresolved, unclaimed with no new execution authorization. Wider implementation remains separately blocked/gated; no runtime acceptance. *state `ready`; authorization `none`; **CANONICAL NEXT**.*
   *Dependencies:* none. *Unblocks:* dat-gui-performance-implementation-vkq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `research/gui-performance/SHARED_GUI_PERFORMANCE_AUDIT.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`, `docs/reviews/gui-performance/zoom-noop-slice.md`, `docs/reviews/gui-performance/resize-slice.md`, `docs/reviews/gui-performance/rendering-reopening.json`, `docs/reviews/gui-performance/resize-recovery.md`, `specs/GUI_PERFORMANCE_ACCEPTANCE_MATRIX.md`, `docs/reviews/gui-performance/initial-qualification-owner-direction.json`, `specs/GUI_PERFORMANCE_IMPLEMENTATION_CONTRACT.md`, `docs/decisions/PRODUCT_MECHANICS_045_SHARED_GUI_PERFORMANCE.md`, `docs/reviews/gui-performance/final-specification-review.json`, `docs/reviews/gui-performance/owner-packet.json`, `docs/reviews/gui-performance/final-owner-direction.json`.
...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Blocked requirement-linked S0-S5 implementation reservation prepared in prepared-implementation-completion.json. Authorization none, no claim/selection. On specification blocker closure install its first owner execution decision with specified/owner_decision state, unclaimed and unselected. E10 temporal prerequisite and independent runtime replay preserved. *state `blocked`; authorization `none`.*
   *Dependencies:* `dat-gui-performance-spec-u9n`. *Unblocks:* dat-uvt-s5a-build-1wv, dat-gui-p2-cross-probe-27z, dat-gui-p2-full-inspector-0ye, dat-marking-menu-shell-g0w, dat-gui-write-path-qiu, dat-project-preferences-build-tlq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`, `specs/GUI_PERFORMANCE_IMPLEMENTATION_CONTRACT.md`, `docs/reviews/gui-performance/prepared-implementation-completion.json`, `docs/reviews/gui-performance/implementation-map.json`.
...
| `specs/GUI_PERFORMANCE_IMPLEMENTATION_CONTRACT.md` | C04/C05 independently reviewed: 225 individual predicates, 14 consumers, 67 source clauses and prepared S0-S5 reservation | Complete adoption/retirement and regression-sensitive proof contract behind owner execution gate |
...
| Implementation | Wider implementation reservation remains blocked; owner-authorized no-op zoom correction verified within GPS-C01; GPS-C01R rapid-resize geometry reuse shows about 13% lower CPU in both axes with mixed GPU duty, preserved pixels and PTY dimensions; historical partial proof only; GPS-C01R retains pending global surface/resource correction and temporal UX qualification; broader QA/acceptance open | Individual requirement-to-consumer-to-test/evidence mapping, regression/rollback gates and independent native replay behind a separate owner execution gate |
