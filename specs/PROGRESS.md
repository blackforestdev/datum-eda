...
- **Build the Datum-owned TerminalCore** (`TERMINAL-T1-CORE`; `dat-terminal-emulator-epic-jzv.2`).
   Implement Datum-owned VT parsing, state, cells, modes, replies, damage, history, reflow, selection, search, and protocols without third-party terminal code. *state `ready`; authorization `execution`; **CANONICAL NEXT**.*
