...
| Criterion | Status | Notes |
|-----------|--------|-------|
| Native format | [~] | Current M4 CLI slice supports deterministic scaffold creation, scaffold inspection, and the first native read/query surface: `project new <dir> [--name]` writes versioned native `project.json`, `schematic/schematic.json`, `board/board.json`, and `rules/rules.json`; `project inspect <dir>` validates the resolved file layout; `project query <dir> summary`, `project query <dir> design-rules`, `project query <dir> forward-annotation-audit`, `project query <dir> forward-annotation-proposal`, `project query <dir> forward-annotation-review`, `project query <dir> symbols`, `project query <dir> symbol-fields --symbol <uuid>`, `project query <dir> symbol-semantics --symbol <uuid>`, `project query <dir> symbol-pins --symbol <uuid>`, `project query <dir> texts`, `project query <dir> drawings`, `project query <dir> nets`, `project query <dir> diagnostics`, `project query <dir> erc`, `project query <dir> check`, `project query <dir> board-components`, `project query <dir> board-pads`, `project query <dir> board-tracks`, `project query <dir> board-vias`, `project query <dir> board-zones`, `project query <dir> board-diagnostics`, `project query <dir> board-unrouted`, `project query <dir> board-check`, `project query <dir> board-texts`, `project query <dir> board-keepouts`, `project query <dir> board-dimensions`, `project query <dir> board-outline`, `project query <dir> board-stackup`, `project query <dir> board-nets`, and `project query <dir> board-net-classes` read the native scaffold directly and report aggregated native schematic/board/rule/check state; focused CLI tests prove idempotent rewrite, inspect-path correctness, referenced-sheet summary counts, native rules payload reporting, native forward-annotation audit/proposal/review reporting, native symbol/text/drawing persistence-query behavior, native connectivity query behavior via an in-memory `Schematic` bridge over the authored sheet files, native ERC precheck behavior on that same bridge, native combined schematic check reporting in the existing `CheckReport::Schematic` shape, native board-component full first-family lifecycle/query plus engine-backed native board diagnostics/unrouted/check parity and board pad edit/net-assignment and board track/via/zone/text/keepout/dimension/outline/stackup/net/net-class persistence-query behavior through `board/board.json`, and forward-annotation artifact export/inspect/compare/filter behavior through versioned native proposal packages |
| Schematic operations | [~] | Nine native authored schematic object families now have focused coverage: symbols (`project place-symbol <dir> --sheet <uuid> --reference <text> --value <text> [--lib-id <text>] --x-nm <i64> --y-nm <i64> [--rotation-deg <i32>] [--mirrored]`, `project move-symbol <dir> --symbol <uuid> --x-nm <i64> --y-nm <i64>`, `project rotate-symbol <dir> --symbol <uuid> --rotation-deg <i32>`, `project mirror-symbol <dir> --symbol <uuid>`, `project delete-symbol <dir> --symbol <uuid>`, `project set-symbol-reference <dir> --symbol <uuid> --reference <text>`, `project set-symbol-value <dir> --symbol <uuid> --value <text>`, `project set-symbol-lib-id <dir> --symbol <uuid> --lib-id <text>`, `project clear-symbol-lib-id <dir> --symbol <uuid>`, `project set-symbol-entity <dir> --symbol <uuid> --entity <uuid>`, `project clear-symbol-entity <dir> --symbol <uuid>`, `project set-symbol-part <dir> --symbol <uuid> --part <uuid>`, `project clear-symbol-part <dir> --symbol <uuid>`, `project set-symbol-unit <dir> --symbol <uuid> --unit <text>`, `project clear-symbol-unit <dir> --symbol <uuid>`, `project set-symbol-gate <dir> --symbol <uuid> --gate <uuid>`, `project clear-symbol-gate <dir> --symbol <uuid>`, `project set-symbol-display-mode <dir> --symbol <uuid> --mode <library-default|show-hidden-pins|hide-optional-pins>`, `project set-symbol-hidden-power-behavior <dir> --symbol <uuid> --behavior <source-defined-implicit|explicit-power-object|preserved-as-imported-metadata>`, `project set-pin-override <dir> --symbol <uuid> --pin <uuid> --visible <true|false> [--x-nm <i64> --y-nm <i64>]`, `project clear-pin-override <dir> --symbol <uuid> --pin <uuid>`, `project add-symbol-field <dir> --symbol <uuid> --key <text> --value <text> [--hidden] [--x-nm <i64> --y-nm <i64>]`, `project edit-symbol-field <dir> --field <uuid> [--key <text>] [--value <text>] [--visible <true|false>] [--x-nm <i64> --y-nm <i64>]`, `project delete-symbol-field <dir> --field <uuid>`), text objects (`project place-text <dir> --sheet <uuid> --text <text> --x-nm <i64> --y-nm <i64> [--rotation-deg <i32>]`, `project edit-text <dir> --text <uuid> [--value <text>] [--x-nm <i64> --y-nm <i64>] [--rotation-deg <i32>]`, `project delete-text <dir> --text <uuid>`), drawing primitives (`project place-drawing-line <dir> --sheet <uuid> --from-x-nm <i64> --from-y-nm <i64> --to-x-nm <i64> --to-y-nm <i64>`, `project place-drawing-rect <dir> --sheet <uuid> --min-x-nm <i64> --min-y-nm <i64> --max-x-nm <i64> --max-y-nm <i64>`, `project place-drawing-circle <dir> --sheet <uuid> --center-x-nm <i64> --center-y-nm <i64> --radius-nm <i64>`, `project place-drawing-arc <dir> --sheet <uuid> --center-x-nm <i64> --center-y-nm <i64> --radius-nm <i64> --start-angle-mdeg <i64> --end-angle-mdeg <i64>`, `project edit-drawing-line <dir> --drawing <uuid> [--from-x-nm <i64> --from-y-nm <i64>] [--to-x-nm <i64> --to-y-nm <i64>]`, `project edit-drawing-rect <dir> --drawing <uuid> ...`, `project edit-drawing-circle <dir> --drawing <uuid> ...`, `project edit-drawing-arc <dir> --drawing <uuid> ...`, `project delete-drawing <dir> --drawing <uuid>`), labels (`project place-label <dir> --sheet <uuid> --name <text> [--kind local|global|hierarchical|power] --x-nm <i64> --y-nm <i64>`, `project rename-label <dir> --label <uuid> --name <text>`, `project delete-label <dir> --label <uuid>`), wires (`project draw-wire <dir> --sheet <uuid> --from-x-nm <i64> --from-y-nm <i64> --to-x-nm <i64> --to-y-nm <i64>`, `project delete-wire <dir> --wire <uuid>`), junctions (`project place-junction <dir> --sheet <uuid> --x-nm <i64> --y-nm <i64>`, `project delete-junction <dir> --junction <uuid>`), hierarchical ports (`project place-port <dir> --sheet <uuid> --name <text> --direction <input|output|bidirectional|passive> --x-nm <i64> --y-nm <i64>`, `project edit-port <dir> --port <uuid> [--name <text>] [--direction ...] [--x-nm <i64>] [--y-nm <i64>]`, `project delete-port <dir> --port <uuid>`), buses (`project create-bus <dir> --sheet <uuid> --name <text> --member <text>...`, `project edit-bus-members <dir> --bus <uuid> --member <text>...`, `project place-bus-entry <dir> --sheet <uuid> --bus <uuid> [--wire <uuid>] --x-nm <i64> --y-nm <i64>`, `project delete-bus-entry <dir> --bus-entry <uuid>`), and no-connect markers (`project place-noconnect <dir> --sheet <uuid> --symbol <uuid> --pin <uuid> --x-nm <i64> --y-nm <i64>`, `project delete-noconnect <dir> --noconnect <uuid>`); `project query <dir> symbols`, `project query <dir> symbol-fields --symbol <uuid>`, `project query <dir> symbol-semantics --symbol <uuid>`, `project query <dir> symbol-pins --symbol <uuid>`, `project query <dir> texts`, `project query <dir> drawings`, `project query <dir> labels`, `project query <dir> wires`, `project query <dir> junctions`, `project query <dir> ports`, `project query <dir> buses`, `project query <dir> bus-entries`, `project query <dir> noconnects`, `project query <dir> nets`, `project query <dir> diagnostics`, and `project query <dir> summary` verify the persisted results through the native read surface; `set-symbol-entity` clears any existing `part_uuid`, and `set-symbol-part` clears any existing `entity_uuid` so unresolved and resolved identity do not coexist ambiguously |
| Board operations | [~] | Native board-authored low-risk families are landed directly on `board/board.json`: first package placement/transform/reassignment/locking/delete lifecycle (`project place-board-component <dir> --part <uuid> --package <uuid> --reference <text> --value <text> --x-nm <i64> --y-nm <i64> --layer <i32>`, `project move-board-component <dir> --component <uuid> --x-nm <i64> --y-nm <i64>`, `project set-board-component-part <dir> --component <uuid> --part <uuid>`, `project set-board-component-package <dir> --component <uuid> --package <uuid>`, `project rotate-board-component <dir> --component <uuid> --rotation-deg <i32>`, `project set-board-component-locked <dir> --component <uuid>`, `project clear-board-component-locked <dir> --component <uuid>`, `project delete-board-component <dir> --component <uuid>`), first package-linked pad slice (`project query <dir> board-pads`, `project place-board-pad <dir> --package <uuid> --name <text> --x-nm <i64> --y-nm <i64> --layer <i32> [--shape <circle|rect>] [--diameter-nm <i64>] [--width-nm <i64>] [--height-nm <i64>] [--net <uuid>]`, `project edit-board-pad <dir> --pad <uuid> [--x-nm <i64> --y-nm <i64>] [--layer <i32>] [--shape <circle|rect>] [--diameter-nm <i64>] [--width-nm <i64>] [--height-nm <i64>]`, `project set-board-pad-net <dir> --pad <uuid> --net <uuid>`, `project clear-board-pad-net <dir> --pad <uuid>`, `project delete-board-pad <dir> --pad <uuid>`), first routed-copper slice (`project draw-board-track <dir> --net <uuid> --from-x-nm <i64> --from-y-nm <i64> --to-x-nm <i64> --to-y-nm <i64> --width-nm <i64> --layer <i32>`, `project delete-board-track <dir> --track <uuid>`, `project place-board-via <dir> --net <uuid> --x-nm <i64> --y-nm <i64> --drill-nm <i64> --diameter-nm <i64> --from-layer <i32> --to-layer <i32>`, `project delete-board-via <dir> --via <uuid>`, `project place-board-zone <dir> --net <uuid> --vertex <x:y>... --layer <i32> --priority <u32> --thermal-relief <bool> --thermal-gap-nm <i64> --thermal-spoke-width-nm <i64>`, `project delete-board-zone <dir> --zone <uuid>`), board texts (`project place-board-text <dir> --text <text> --x-nm <i64> --y-nm <i64> [--rotation-deg <i32>] [--height-nm <i64>] [--stroke-width-nm <i64>] --layer <i32>`, `project edit-board-text <dir> --text <uuid> [--value <text>] [--x-nm <i64> --y-nm <i64>] [--rotation-deg <i32>] [--height-nm <i64>] [--stroke-width-nm <i64>] [--layer <i32>]`, `project delete-board-text <dir> --text <uuid>`), board keepouts (`project place-board-keepout <dir> --kind <text> --layer <i32>... --vertex <x:y>...`, `project edit-board-keepout <dir> --keepout <uuid> [--kind <text>] [--layer <i32>...] [--vertex <x:y>...]`, `project delete-board-keepout <dir> --keepout <uuid>`), board dimensions (`project place-board-dimension <dir> --from-x-nm <i64> --from-y-nm <i64> --to-x-nm <i64> --to-y-nm <i64> [--text <text>]`, `project edit-board-dimension <dir> --dimension <uuid> [--from-x-nm <i64> --from-y-nm <i64>] [--to-x-nm <i64> --to-y-nm <i64>] [--text <text>] [--clear-text]`, `project delete-board-dimension <dir> --dimension <uuid>`), direct board outline replacement (`project set-board-outline <dir> --vertex <x:y>...`), full stackup replacement (`project set-board-stackup <dir> --layer <id:name:type:thickness_nm>...`), board net lifecycle (`project place-board-net <dir> --name <text> --class <uuid>`, `project edit-board-net <dir> --net <uuid> [--name <text>] [--class <uuid>]`, `project delete-board-net <dir> --net <uuid>`), and board net-class lifecycle (`project place-board-net-class <dir> --name <text> --clearance-nm <i64> --track-width-nm <i64> --via-drill-nm <i64> --via-diameter-nm <i64> [--diffpair-width-nm <i64>] [--diffpair-gap-nm <i64>]`, `project edit-board-net-class <dir> --net-class <uuid> ...`, `project delete-board-net-class <dir> --net-class <uuid>`); `project query <dir> board-components`, `project query <dir> board-pads`, `project query <dir> board-tracks`, `project query <dir> board-vias`, `project query <dir> board-zones`, `project query <dir> board-diagnostics`, `project query <dir> board-unrouted`, `project query <dir> board-check`, `project query <dir> board-texts`, `project query <dir> board-keepouts`, `project query <dir> board-dimensions`, `project query <dir> board-outline`, `project query <dir> board-stackup`, `project query <dir> board-nets`, and `project query <dir> board-net-classes` verify persisted board state directly from `board/board.json`, with `board-diagnostics`, `board-unrouted`, and `board-check` reusing the engine board model rather than a native-only checker; broader board authoring remains open |
| Schematic query parity | [~] | Native CLI read/query currently covers symbols, symbol fields, symbol semantic selection state, symbol pins with per-pin override state, texts, drawing primitives, labels, hierarchy ports, wires, junctions, buses, bus entries, no-connects, summary aggregation, design-rules inspection, connectivity nets/diagnostics, ERC precheck findings, and the combined schematic `check` report surface; full engine/daemon/MCP/CLI parity for all required schematic topology and checking queries remains open |
| Forward annotation | [~] | Native forward annotation now has audit, proposal, exported artifact packaging plus inspection/load/filter/apply-planning support, artifact-versus-live comparison, artifact review import/replace, selected-subset export from both live proposal state and existing artifacts, persisted review state, keyed single-action apply, a first artifact-driven apply slice, and a first batch apply slice: `project query <dir> forward-annotation-audit` compares schematic symbols against board components by reference and reports matched pairs, unresolved symbols, missing board components, orphaned board components, value mismatches, and part mismatches; `project query <dir> forward-annotation-proposal` deterministically classifies those findings into add/update/remove component ECO actions with stable `action_id` values and grouped action lists; `project export-forward-annotation-proposal <dir> --out <path>` now writes a versioned artifact containing the current proposal plus persisted review decisions so the review package can move as a stable file contract, `project export-forward-annotation-proposal-selection <dir> --action-id <sha256:...>... --out <path>` now writes that same artifact kind with only the selected live proposal actions and matching review records, `project select-forward-annotation-proposal-artifact --artifact <path> --action-id <sha256:...>... --out <path>` now narrows an existing artifact down to the selected action subset and matching review records, `project inspect-forward-annotation-proposal-artifact <path>` now loads and reports that artifact’s version/project/action/review counts, `project compare-forward-annotation-proposal-artifact <dir> --artifact <path>` now classifies exported actions against the current live proposal as `applicable`, `drifted`, or `stale`, `project filter-forward-annotation-proposal-artifact <dir> --artifact <path> --out <path>` now writes a filtered versioned artifact containing only the actions still `applicable` against the current live project plus their retained review decisions, `project plan-forward-annotation-proposal-artifact-apply <dir> --artifact <path>` now reports which artifact actions are self-sufficient to apply versus which still require explicit caller input, `project apply-forward-annotation-proposal-artifact <dir> --artifact <path>` now applies a filtered same-project artifact only when every retained action is still `applicable` and `self_sufficient` while still honoring any retained `deferred` or `rejected` review decisions, `project import-forward-annotation-artifact-review <dir> --artifact <path>` now merges artifact review decisions back into live project state only for actions that still exist in the current live proposal, and `project replace-forward-annotation-artifact-review <dir> --artifact <path>` now replaces the live review map with that validated artifact subset so stale or superseded live review decisions can be cleared intentionally; `project defer-forward-annotation-action <dir> --action-id <sha256:...>`, `project reject-forward-annotation-action <dir> --action-id <sha256:...>`, `project clear-forward-annotation-action-review <dir> --action-id <sha256:...>`, and `project query <dir> forward-annotation-review` now persist, clear, and report review decisions by stable key against the current proposal set; `project apply-forward-annotation-action <dir> --action-id <sha256:...>` now applies `remove_component`, `update_component` for `value_mismatch`, `add_component` when the caller supplies explicit `--package <uuid> --x-nm <i64> --y-nm <i64> --layer <i32>` input, and `update_component` for `part_mismatch` when the caller supplies an explicit `--package <uuid>` target, with `--part <uuid>` required only when the proposal does not carry a resolved schematic part; `project apply-forward-annotation-reviewed <dir>` now batch-applies the currently self-sufficient proposal actions while honoring persisted `deferred`/`rejected` review decisions and skipping actions that still require explicit input |
| Gerber export | [~] | `project export-gerber-outline <dir> --out <path>` now writes a narrow RS-274X Gerber file for the persisted native board outline via `eda_engine::export`, `project validate-gerber-outline <dir> --gerber <path>` now re-renders the expected native outline Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-copper-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for persisted board pads, tracks, vias, and zones on one selected copper layer with deterministic circular/rectangular aperture assignment, flashed pad/via emission, and region emission for zones, `project validate-gerber-copper-layer <dir> --layer <id> --gerber <path>` now re-renders the expected copper-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-soldermask-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for explicit circular/rectangular pad openings on one selected soldermask layer derived from the nearest copper layer in stackup order, `project validate-gerber-soldermask-layer <dir> --layer <id> --gerber <path>` now re-renders that expected soldermask-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-silkscreen-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for authored board text plus persisted explicit component-silkscreen text, line, arc, and circle geometry on one selected silkscreen layer, using explicit stored text height/stroke-width plus the current fixed stroke-font renderer for board text and component text, explicit stored stroke widths for component lines, deterministic 15-degree chordization for component arcs, and deterministic 24-segment chordization for component circles, `project validate-gerber-silkscreen-layer <dir> --layer <id> --gerber <path>` now re-renders that expected silkscreen-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-paste-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for explicit circular/rectangular pad openings on one selected paste layer derived from the nearest copper layer in stackup order, `project validate-gerber-paste-layer <dir> --layer <id> --gerber <path>` now re-renders that expected paste-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project export-gerber-mechanical-layer <dir> --layer <id> --out <path>` now writes a narrow RS-274X Gerber file for native board keepout polygons on one selected mechanical layer with closed keepouts emitted as filled regions, `project validate-gerber-mechanical-layer <dir> --layer <id> --gerber <path>` now re-renders that expected mechanical-layer Gerber and reports byte-for-byte match/mismatch with CI-usable exit semantics, and `project plan-gerber-export <dir> [--prefix <text>]` still reports the broader deterministic planned artifact set from the persisted native board outline and stackup; full layer-set writing, broader pad geometry beyond the current flashed circle/rectangle subset, package silkscreen shapes beyond the current explicit component-text-line-arc-circle subset, package silkscreen/mask/paste expansion or stencil policy beyond the current narrow slice, broader mechanical documentation geometry, and gerbv validation remain open |
| Drill export | [~] | `project export-drill <dir> --out <path>` now writes a deterministic CSV from the persisted native via inventory in `board/board.json`, `project export-excellon-drill <dir> --out <path>` now writes a narrow Excellon drill file from that same persisted via inventory through `eda_engine::export`, `project validate-excellon-drill <dir> --drill <path>` now re-renders the expected Excellon drill file and reports byte-for-byte match/mismatch with CI-usable exit semantics, `project inspect-excellon-drill <path>` now reads that narrow Excellon file back and reports parsed tool-table and hit-count information, `project compare-excellon-drill <dir> --drill <path>` now compares that Excellon file against the current native via inventory by normalized drill diameter and hit count so drill drift is visible even when byte identity is not the comparison goal, and `project report-drill-hole-classes <dir>` now groups the native via inventory into explicit through/blind/buried hole classes from copper-layer span and reports per-class tool tables plus hit counts; slot/hole classes beyond current via-span semantics and broader fabrication contract details remain open |
| BOM export | [~] | `project export-bom <dir> --out <path>` now writes a deterministic CSV from the persisted native board-component inventory in `board/board.json`, and `project compare-bom <dir> --bom <path>` now compares that BOM against the current native board-component inventory by reference, value, part/package identity, layer, position, rotation, and locked state so BOM drift is reviewable semantically; pool-derived manufacturer fields, schematic-only items, variants, and richer purchasing metadata remain open |
| PnP export | [~] | `project export-pnp <dir> --out <path>` now writes a deterministic CSV from the persisted native board-component inventory in `board/board.json`, and `project compare-pnp <dir> --pnp <path>` now compares that CSV against the current native board-component inventory by reference, placement, rotation, side/layer, part/package identity, value, and locked state so PnP drift is reviewable semantically; richer feeder/origin/variant/manufacturer contract details remain open |
| Gerber comparison | [~] | `project compare-gerber-export-plan <dir> --output-dir <path> [--prefix <text>]` now compares the deterministic planned Gerber artifact set against a directory and reports matched, missing, and extra files, `project compare-gerber-outline <dir> --gerber <path>` now parses the currently emitted outline RS-274X subset and compares it semantically against the native board outline geometry, `project compare-gerber-copper-layer <dir> --layer <id> --gerber <path>` now parses the currently emitted copper-layer RS-274X subset and compares circular/rectangular flashed pads, tracks, vias, and zones semantically against the native layer geometry with deterministic matched/missing/extra reporting, `project compare-gerber-soldermask-layer <dir> --layer <id> --gerber <path>` now parses the currently emitted soldermask-layer RS-274X subset and compares circular/rectangular flashed pad openings semantically against the native openings derived from the nearest copper layer in stackup order, `project compare-gerber-silkscreen-layer <dir> --layer <id> --gerber <path>` now parses the currently emitted silkscreen-layer RS-274X subset and compares authored board-text strokes plus persisted explicit component-silkscreen text, line, chordized-arc, and chordized-circle strokes semantically against the native silkscreen geometry on that layer, `project compare-gerber-paste-layer <dir> --layer <id> --gerber <path>` now parses the currently emitted paste-layer RS-274X subset and compares circular/rectangular flashed pad openings semantically against the native openings derived from the nearest copper layer in stackup order, and `project compare-gerber-mechanical-layer <dir> --layer <id> --gerber <path>` now parses the currently emitted mechanical-layer RS-274X subset and compares native keepout polygons on that selected mechanical layer semantically against emitted filled regions; broader RS-274X feature coverage, broader pad geometry beyond the current flashed circle/rectangle subset, full layer-set comparison, and viewer-level validation remain open |

**M4 overall**: [~] Native-project persistence now has deterministic scaffold creation, minimal native inspect/query/check surfaces including native connectivity nets/diagnostics/ERC/check and engine-backed board diagnostics/unrouted/check parity, focused authored schematic coverage for symbols, text objects, drawing primitives, labels, wires, junctions, hierarchical ports, buses/bus entries, and no-connect markers, the first native board-authored low-risk families plus initial component placement, pad lifecycle with explicit circular/rectangular copper geometry, authored board text with explicit height/stroke-width render geometry, persisted explicit component-silkscreen text/line/arc/circle geometry for the current manufacturing subset, and copper primitives, forward-annotation audit/proposal/export/live-selection-export/artifact-selection/inspect/compare/filter/plan/review-import/review-replace plus keyed single-action apply, a first filtered-artifact apply slice, and a first review-aware batch apply slice, and a materially broader native manufacturing slice: deterministic BOM/PnP export plus semantic compare against live board inventory, deterministic CSV via-drill export plus narrow real Excellon write/inspect/validate/compare and explicit drill hole-class reporting, narrow real RS-274X board-outline, copper-layer (circular/rectangular flashed pads, tracks, vias, and zones), soldermask-layer (circular/rectangular flashed pad openings only), silkscreen-layer (authored board-text strokes plus explicit component-silkscreen text/line/chordized-arc/chordized-circle strokes only), paste-layer (circular/rectangular flashed pad openings only), and mechanical-layer (keepout polygons only) writers through `eda_engine::export`, deterministic Gerber artifact planning from board outline plus stackup, deterministic Gerber plan-vs-directory comparison, and semantic Gerber comparison for the currently emitted outline, single-layer copper, single-layer soldermask, single-layer silkscreen, single-layer paste, and single-layer mechanical subsets; most schematic ops, most board authoring ops, broader ECO apply/reject flow, broader layer-set/geometry-level manufacturing output, and full Gerber feature coverage remain open
