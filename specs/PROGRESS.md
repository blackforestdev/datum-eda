...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   S0 and owner-amended S1/S2 bounded readiness complete. All four native hosts consume shared rendering owners. S3 convergence remains in progress: New Project backdrop retired; Preferences, Revision, Inspector, Project/Layers, terminal and Main menu profiles have bounded shared clipping adoption. Per-adapter qualification and full adoption review remain open. A narrow clarification of S3 HP14/HP16 dependencies on S2 resource work is prepared for owner review, not approved; current S3 exit remains controlling. Fifty S2 predicates retain implementation at S4 and qualification at S5; S1 qualification remains carried. GEFN preserved; resize nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
