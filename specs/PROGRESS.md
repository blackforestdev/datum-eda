...
- **Specify the performance-first shared GUI foundation** (`GUI-PERFORMANCE-SPEC`; `dat-gui-performance-spec-u9n`).
   Owner-directed GPS-C02 through GPS-C06 specification development is active. Resize recovery remains unresolved and pending after the specification decision; no runtime acceptance or implementation grant is implied. *state `in_progress`; authorization `planning`; **CANONICAL NEXT**.*
   *Dependencies:* none. *Unblocks:* dat-gui-performance-implementation-vkq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `research/gui-performance/SHARED_GUI_PERFORMANCE_AUDIT.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`, `docs/reviews/gui-performance/zoom-noop-slice.md`, `docs/reviews/gui-performance/resize-slice.md`, `docs/reviews/gui-performance/rendering-reopening.json`, `docs/reviews/gui-performance/resize-recovery.md`, `specs/GUI_PERFORMANCE_ACCEPTANCE_MATRIX.md`.
...
| `specs/GUI_PERFORMANCE_ACCEPTANCE_MATRIX.md` | GPS-C03 draft: exact schedules, proposed targets and explicit scale/temporal/GPU/memory gaps; no qualification claimed | Complete justified numerical matrix and validated feasible methods before C03 closure |
