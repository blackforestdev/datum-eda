...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   S0 and owner-amended S1/S2/S3 bounded implementation readiness complete. S4 selected for terminal fairness/hidden work, recovery/resource adoption and all 50 carried S2 implementation predicates. Twenty unfinished S3 qualification rows remain required at S5 alongside S1/S2 obligations; original acceptance rules, partial statuses and failed evidence preserved. No full resource-budget or global adoption acceptance. GEFN preserved; resize nonblocking. PM047 permits monitored private text construction; all four remaining accounting/scratch predicates stay open until implemented and verified. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
   *Dependencies:* `dat-gui-performance-spec-u9n`. *Unblocks:* dat-uvt-s5a-build-1wv, dat-gui-p2-cross-probe-27z, dat-gui-p2-full-inspector-0ye, dat-marking-menu-shell-g0w, dat-gui-write-path-qiu, dat-project-preferences-build-tlq. *Governing:* `specs/GUI_PERFORMANCE_RECOVERY_PLAN.md`, `specs/GUI_SHARED_ENGINEERING_CONTRACT.md`, `specs/GUI_PERFORMANCE_IMPLEMENTATION_CONTRACT.md`, `docs/reviews/gui-performance/implementation-map.json`, `docs/reviews/gui-performance/resize-deferral-owner-direction.json`, `docs/decisions/PRODUCT_MECHANICS_047_PRIVATE_TEXT_CONSTRUCTION_GUARD.md`.
...
| `specs/GUI_PERFORMANCE_IMPLEMENTATION_CONTRACT.md` | S0 and amended S1/S2/S3 bounded readiness complete; S4 selected. Twenty unfinished S3 qualification rows required at S5 | Complete S4 implementation and S5 qualification; no partial row marked passed |
