...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   S0 and owner-amended S1 readiness complete. Owner approved the exact S2 exit amendment: bounded S2 readiness remains selected/in progress; unfinished implementation is mandatory at S4 and full qualification at S5. Approval does not complete S2 or select S3. GEFN preserved; resize investigation remains nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
