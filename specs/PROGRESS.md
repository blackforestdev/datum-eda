...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   Owner-ratified specification blocker closed. Resize investigation is pinned separately and nonblocking; shared-engine implementation awaits its existing first owner execution grant. No claim or runtime acceptance. *state `specified`; authorization `owner_decision`; **CANONICAL NEXT**.*
...
| Implementation | Specification item closed; wider implementation awaits its separate first owner execution gate; owner-authorized no-op zoom correction verified within GPS-C01; GPS-C01R rapid-resize geometry reuse shows about 13% lower CPU in both axes with mixed GPU duty, preserved pixels and PTY dimensions; historical partial proof only; resize resource/temporal qualification is pinned separately and no longer gates shared-engine development; broader QA/acceptance open | Individual requirement-to-consumer-to-test/evidence mapping, regression/rollback gates and independent native replay behind a separate owner execution gate |
