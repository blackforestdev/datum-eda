...
- **Implement the approved shared GUI performance specification** (`GUI-PERFORMANCE-IMPLEMENTATION`; `dat-gui-performance-implementation-vkq`).
   S0 and owner-amended S1 bounded implementation readiness complete. All four default hosts share scheduling/surface/queue/lifetime owners. S2 retained geometry/text/encoding/upload migration selected. Thirty initial-scope S1 rows retain unfinished qualification at S5; GEFN preserved and resize investigation nonblocking. *state `in_progress`; authorization `execution`; **CANONICAL NEXT**.*
