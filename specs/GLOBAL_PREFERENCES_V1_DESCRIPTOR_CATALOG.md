...
| Code | Meaning |
|---|---|
| `P` / `W` / `C` | `Presentation`, `WorkflowDefault`, or `Capability` class |
...
|---|---|---|---|---|---|---|
| `datum.console.feedback_duration` | GUI shell | enum `{4s,6s,10s,never}`; `6s` | P; U+S; R,C | live; Console feedback timer | P0; alias `console_duration`; M from current field | `preferences-window.html:90`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:134-146` |
...
| `datum.schematic.theme` | schematic presentation | enum `{dark,light}`; `dark` | P; U+S; R,C | live; schematic canvas only | P0; no legacy; I; PM-036 | `preferences-window.html:98`; `PRODUCT_MECHANICS_036_SCHEMATIC_DRAWING_THEMES.md:18-57` |
...
| `datum.viewport.snap_enabled` | shared viewport | bool; `true` | W; U+S+C; R,C,Pn,L | live; schematic/PCB editors | P0; legacy hard-coded default; I | `preferences-window.html:114`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:182-191` |
| `datum.viewport.grid_presets` | shared viewport | nonempty ordered exact-length list plus selected default; `{0.1mm,0.5mm,1mm}`, `0.5mm` | W; U+S+C; R,C,Pn,L | live; editor grid selectors | P0; no legacy; I | `preferences-window.html:116` |
| `datum.viewport.snap_capture_px` | shared viewport | integer `1..64`; `10` | W; U+S+C; R,C,Pn,L | live; snap resolver | P0; legacy code default; I | `preferences-window.html:118`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:226-231` |
...
| `datum.viewport.crosshair_style` | shared viewport | enum `{full,local,none}`; `full` | P; U+S+C; R,C | live; editor cursor renderer | P0; migrate no stored value: current session remains Session only; I | `preferences-window.html:126`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:243-254` |
...
Crosshair resolution deliberately separates the new persistent opening default
from the existing live Session contribution. Grid pitch/profile tables remain
engine `ViewportProfile` facts; the descriptor stores only the user's preset set
and selection. Exclusive ratified gestures cannot be rebound by this key.
...
| Stable key | Owner | Value schema; factory/no-value | Class; scopes; management | Seed/apply; consumers | Portability; migration; disposition | Evidence |
|---|---|---|---|---|---|---|
| `datum.units.system` | units engine | enum `{metric,imperial}`; `metric` | P; U+S+C; R,C,Pn,L | seed:`ProjectDisplayUnits`; live; all quantity formatters/parsers | P0; splits legacy aggregate Display units; I | `preferences-window.html:137`; `GP_SHARED_UNITS_ENGINE_REQUIREMENT.md:34-82` |
| `datum.units.board_length` | units engine | enum `{follow_system,mm,um,mil,inch}` with system compatibility; `follow_system` | P; U+S+C; R,C,Pn,L | seed:`ProjectDisplayUnits`; live; PCB/layout fields | P0; split aggregate; I | `preferences-window.html:139-140`; `GP_SHARED_UNITS_ENGINE_REQUIREMENT.md:51-82` |
| `datum.units.drill_hole` | units engine | enum `{follow_system,mm,mil,inch}`; `follow_system` | P; U+S+C; R,C,Pn,L | seed:`ProjectDisplayUnits`; live; drill/hole fields | P0; split aggregate; I | `preferences-window.html:140-141`; `GP_SHARED_UNITS_ENGINE_REQUIREMENT.md:51-82` |
| `datum.units.schematic_geometry` | units engine | enum `{follow_system,mm,mil}`; `follow_system` | P; U+S+C; R,C,Pn,L | seed:`ProjectDisplayUnits`; live; schematic geometry | P0; split aggregate; I | `preferences-window.html:141`; `GP_SHARED_UNITS_ENGINE_REQUIREMENT.md:51-82` |
| `datum.units.length_precision` | units engine | enum `{0.1,0.01,0.001,0.0001,exact_nm}`; `0.01` | P; U+S+C; R,C | seed:`ProjectDisplayUnits`; live; length formatters | P0; split aggregate; I | `preferences-window.html:142-143`; `GP_SHARED_UNITS_ENGINE_REQUIREMENT.md:61-82` |
| `datum.units.angle_format` | units engine | struct `{notation:decimal_degrees-or-dms-or-radians,precision}`; decimal degrees/0.1° | P; U+S+C; R,C | seed:`ProjectDisplayUnits`; live; angle formatters/parsers | P0; split aggregate; I | `preferences-window.html:143`; `GP_SHARED_UNITS_ENGINE_REQUIREMENT.md:84-92` |
...
| `datum.pcb.viewport_airwire_culling` | PCB presentation | bool; `false` | P; U+S+C; R,C | live; airwire renderer/performance | P0; correct prototype control/default mismatch before implementation; I | `preferences-window.html:166` |
| `datum.pcb.opening_layer_visibility` | PCB presentation | enum `{all,signal_silk,custom_set}`; `all` | P; U+S+C; R,C | next design open; board viewport | P0; active visibility map stays restartable state; I | `preferences-window.html:168`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:212-221` |
...
| `datum.publish.title_block_template_seed` | documentation | resolvable template-set identity; Datum factory set | W; U+S; R,C,Pn,L | seed:`ProjectDocumentTemplates`; New Project only | P0, missing identity refuses seed; no legacy; I | `preferences-window.html:177-178`; `PRODUCT_MECHANICS_020_PAPER_SPACE_AND_VIEWPORTS.md:18-55` |
| `datum.publish.sheet_format_seed` | documentation | struct `{size,orientation}`; A3/landscape | W; U+S; R,C,Pn,L | seed:`ProjectDocumentTemplates`; New Project only | P0; no legacy; I | `preferences-window.html:178-179` |
| `datum.publish.scale_fraction_style_seed` | documentation | enum `{one_to_n,n_over_one,custom_pattern}`; `one_to_n` | W; U+S; R,C,Pn,L | seed:`AdoptedDraftingStandard`; New Project only | P0; moved from planned organization row; I | `preferences-window.html:179-180`; `PRODUCT_MECHANICS_035_ADOPTED_DRAFTING_STANDARD_AUTHORITY.md:18-51` |
...
| `datum.publish.set_name_prefill` | documentation | struct `{mode:ask-or-pattern,pattern}`; pattern `{project}-{set}` | W; U+S+C; R,C,Pn,L | live pre-fill; publish-set creation | P0; operation still asks; I | `preferences-window.html:182-183` |

The airwire-culling row draws `On` while its description says factory `Off`.
The descriptor default is `false`; GP-C06 must retain this catalog fact and a
bounded Claude reconciliation must correct the comparative visual state before
implementation uses it. Codex does not edit the prototype.

### 2.4 Terminal

| Stable key | Owner | Value schema; factory/no-value | Class; scopes; management | Seed/apply; consumers | Portability; migration; disposition | Evidence |
|---|---|---|---|---|---|---|
| `datum.terminal.launch_profiles` | terminal session | versioned named profiles `{executable,argv,cwd,environment}`; platform default shell | C; U+S; R,C,Pn,L | next terminal launch; PTY/session launcher | PX; replace process-launch inputs only when explicitly selected; I | `preferences-window.html:193`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:305-331` |
...
| `datum.terminal.notifications` | terminal capability | enum `{off,unfocused,always}`; `unfocused` | C; U+S; R,C,Pn,L | live; desktop notification adapter | PX; migrate `DATUM_TERMINAL_NOTIFICATIONS` only with explicit provenance; M | `preferences-window.html:205`; `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:426-441` |
...
| Stable key | Owner | Value schema; factory/no-value | Class; scopes; management | Seed/apply; consumers | Portability; migration; disposition | Evidence |
|---|---|---|---|---|---|---|
| `datum.files.startup_mode` | application shell | enum `{start_page,last_session,empty}`; `start_page` | W; U+S; R,C,Pn,L | next launch; shell/start page | P0; recent list remains restartable state; I | `preferences-window.html:239`; `start-page-study.html:184-249` |
...
| `datum.projects.seed_profile` | Project genesis | resolvable profile identity; Datum factory | W; U+S; R,C,Pn,L | seed:`ProjectSeedSnapshot`; New Project only | P0, missing identity refuses selection; no legacy; I | `preferences-window.html:245`; `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md:132-169` |
| `datum.projects.template_seed` | Project genesis | resolvable template identity or none; Datum starter | W; U+S; R,C,Pn,L | seed:`ProjectTemplateCopy`; New Project only | P0; no legacy; I | `preferences-window.html:247` |
| `datum.projects.unit_policy_seed` | Project genesis/units | struct `{system,display_precision}`; metric/mm | W; U+S; R,C,Pn,L | seed:`ProjectDisplayUnits`; New Project only | P0; no legacy; I | `preferences-window.html:249`; `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md:132-169` |
...
| `datum.revision.visibility` | revision presentation | bool; `true` | P; U+S+C; R,C,Pn,L including approved teaching pin | live; Navigator/projections only | P0; no legacy; I | `preferences-window.html:264`; `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md:1768-1825` |
| `datum.revision.profile_seed` | revision engine | resolvable revision-profile identity; Sequential Alphanumeric (Legacy) | W; U+S; R,C,Pn,L | seed:`ProjectRevisionPolicy`; New Project only | P0; no legacy; I | `preferences-window.html:266`; `GP_C03_PROJECT_SEED_AND_CONTEXT_DECISION_PACKET.md:132-169` |
| `datum.revision.build_presentation_seed` | revision engine | enum `{quiet_v4a,phase_build_v4b}`; `quiet_v4a` | W; U+S; R,C,Pn,L | seed:`ProjectRevisionPolicy.build_presentation`; New Project only | P0; no legacy; I | `preferences-window.html:268` |
| `datum.revision.prototype_transition_seed` | revision engine | enum `{continuous,governed_transition}`; `continuous` | W; U+S; R,C,Pn,L | seed:`ProjectRevisionPolicy.namespace_transition`; New Project only | P0; no legacy; I | `preferences-window.html:270` |
| `datum.agents.authority_level` | agent authority | enum `{inspect,propose,apply_approved,unattended}`; `propose` | C; U only; R,C,Pn,L | next agent launch and live revocation; agent gateway | PX; Project/portable source forbidden; I | `preferences-window.html:274` |
| `datum.agents.unattended_tools` | agent authority | bounded typed map of tool identity to limits; empty | C; U only; R,C,Pn,L | live grant/revocation; agent gateway | PX; Project/portable source forbidden; I | `preferences-window.html:276` |
...
| Active grid selection, crosshair choice, layer visibility map, terminal theme cycle | Session/restartable state | may contribute through approved Session scope; persistence of the distinct opening default does not convert live state | `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:182-254`; `preferences-window.html:126,168,195` |
...
| Dimensioning units, drafting standard, sheet numbering/document conventions, provisional watermark | Project/document authority | shown read-only or seeded once; mutation goes through Project/document authority | `preferences-window.html:147-149,184-186`; `PRODUCT_MECHANICS_035_ADOPTED_DRAFTING_STANDARD_AUTHORITY.md:18-51` |
...
| Terminal executable/argv/cwd/environment supplied to one launch | operation input | explicit launch request wins for that invocation after Capability validation; never silently captured as a profile | `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:305-331` |
| Environment and CLI compatibility inputs | external operation/process inputs | descriptor migration requires an explicit named adapter and provenance; no ambient precedence | `GP_C01_INTERNAL_AUTHORITY_AUDIT.md:426-441`; `GP_C03_TYPED_AUTHORITY_DECISION_PACKET.md:392-467` |
...
| Workspace/input | Mouse/wheel/touchpad mapping; camera/pointer behavior; selection modifiers; cross-probe behavior; hover/tooltip/HUD assistance; numeric spin increments; double-click behavior; 3D input tuning; resource-usage warnings; undo-history/session-journal memory bound | `preferences-window.html:132-133` |
...
| PCB/board | Auto-via on layer change; interactive-routing defaults; routing-objective default; rotation/flip direction; track/arc editing mode; default board primitives; polygon-repour automation; net-name/clearance display; displayed origin/axes; zone display mode | `preferences-window.html:170-171` |
...
| Files/projects | Drafting-standard seed; multi-instance locking; helper applications; update checks; cache maintenance; 3D model search paths; update channel | `preferences-window.html:255-256` |
| Revision | Provisional-watermark presentation descriptor/seed; Git/offline-exchange seed; local-history/session-journal retention; machine VCS integration; user identity for provenance | `preferences-window.html:272` |
| Agents | Persistent agent Project configuration; scripting/agent API access; engine-daemon/MCP endpoint configuration | `preferences-window.html:278-279` |
...
Two historical planned-seed labels are no longer deferred: “Editor appearance
themes” became the ratified schematic theme; “Title-block scale fraction
(per-firm)” became the typed publish-space seed. Historical Project-mirror
candidates remain negative classifications, not deferred descriptors. This
reconciliation accounts for all 59 historical planned labels while admitting
the post-seed editor and completeness-critic rows without pretending the
prototype's old 61+58 banner is a registry count.
...
4. Every other active descriptor is `implement new`. Absence means descriptor
   default; implementation must not manufacture stored defaults during first
...
- personal accessibility carve-outs and keyboard/non-color/reduced-motion
  presentation;
- Capability refusal from portable and Project sources;
- exact preservation of unknowns and one-live-identity alias migration;
- the airwire-culling visual/default mismatch identified in §2.3;
